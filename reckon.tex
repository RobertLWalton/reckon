...
copy of the expression definition is made, and part of this copy is
unified with the expression being evaluated.  This unification defines
variables in the expression definition.  Expression definition
variables that receive no value in this way, but which have default
values, are given the default values.
However, values received from unification may be incomplete, in that they
are expressions which include other variables with unknown values.
When the guards of the expression definition
are matched in turn with other expression
definitions, and unified with parts of copies of these matched definitions,
...
to guards.  The first step in evaluating a block is to evaluate
the values of variables which are marked as being subject to initial
evaluation before they are used.  Then the block is evaluated.  Guard
blocks must return true or false, and if any returns false, the expression
definition match fails, and the search for other matching definitions continues.
Evaluations of variable values just before a block evaluation,
and evaluations of guard blocks, are required to have no visible side effects.
...
In use, an expression definition is just before matching it to an expression,
and then the pattern of the copy is call unified
...
default values, and the matching process continues by searching for
...
in the expression being evaluated.  In matching a definition, more than
...
right, so order of guard matching is not a choice.
...
using any chosen argument order.
If the unification fails, the definition does not match.  If the
unification succeeds, it provides variable values for the variables
in the pattern and in the expression to be matched.
...
Note the variable value expression evaluations of
...
side effects as per \secref{BLOCKS}.
...
\key{group} ::= {\em declaration-group} $|$ {\em statement-group}
			 \\[1ex]
\key{declaration-group} ::= {\em declaration}
