...
A summary of the behavior of standard parsers is:
...
The parsing selector set at the top of the parsing selector set stack is
called the \key{current parsing selector set}, and the parsing stack
definitions it selects are call the \skey{current parsing definition}s.%
...
names a parser.  The most recent current parser definition
...
{\em parser-selectors} ::= [see \pagref{PARSING-SELECTORS}]
\end{indpar}
...
\begin{indpar}\begin{verbatim}
define parser -OPERATOR-PARSER- [operator]
define parser -TEXT-PARSER- [text]
\end{verbatim}\end{indpar}

The difference between these two parsers is that only the operator
parser recognizes operators, and only the text parser recognizes paragraphs,
sentences, etc.
...
(\pagref{NUMBER-UNIT-GROUPING}) can insert implied brackets.

Bracket and operator definitions may specify reformatters
that are functions which reformat subexpressions after they haver
been normally parsed.  These definitions may also specify evaluators
