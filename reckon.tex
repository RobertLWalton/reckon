...
stack.  However, you can place parsing definitions
in the stack
that suppress the effect of particular parsing definitions
(these definitions begin with `\verb|undefine|' instead of `\verb|define|').
...
A \key{parsing selector} is a name consisting of a sequence of
one or more symbols.  Parsing selectors are attached to parsing definitions,
...
but \underline{not} `\verb|operator|',
the \verb|-TEXT-PARSER-| is used.  When the set does \underline{not}
contain `\verb|text|', but contains `\verb|operator|',
the \verb|-OPERATOR-PARSER-| is used.
If the current parsing selector set contained both `\verb|text|'
and `\verb|operator|', both the above definitions would be current,
and the topmost in the parsing definition stack would be used, but
the conventions for using these particular selectors
forbid them from appearing simultaneously in the current
...
Many parts of the parser introduce
...
Other kinds of implicit brackets are introduced by number parsing
and by the text parser.
...
\end{tabular}
\end{center}\label{IMPLICIT-BRACKETS}


For example, the parser turns the number
\begin{center}
\verb|4ft 5 3/4 in|
\end{center}
into
...
\verb|(# 4 * ft + (# 5 + 3/4 #) * in #)|
\end{center}
The operator expression parser with the `operators' parsing selector recognizes
