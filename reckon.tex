...
not matter.  The {\em reorder-mark} (\verb|<:>|), {\em remain\-der-mark}
(\verb|::>|), and {\em optional-qualifier-mark} (\verb|??|)
can only occur in a pattern (\ref{EXPRESSION-MATCHING}).
...
called the matching operation (\ref{EXPRESSION-MATCHING}).
...
there is an arrow label L such that one of the nodes M or N
...
\begin{center}
\verb|[sort x @@ with key weight @@ with comparator < ?? ascending]|
\end{center}
...
`\ttkey{-REQUIRED-QUALIFIER-}'
if the `\verb|@@|' \key{required-qualifier-mark} is used or
`\ttkey{-OPTIONAL-QUALIFIER-}'
if the `\verb|??|' \key{optional-qualifier-mark} is used.
...
two \skey{required qualifier}s, and one \key{optional qualifier arrow}
with its one \key{required qualifier}.
...
reordered argument, and rest argument arrows.
...
given an argument index that is a range of the form
...
is not counted in the `\verb|-ARITY-|'.  Here \verb|-REST-|
...
Conversion from a raw expression to an expression graph is performed
...
and zero or more defaults.  Then all the values of each variable and
all the defaults of each variable are unified, and to produce an
expression graph.

In converting a raw expression to an expression tree, the
...
This expression converts to a tree consisting of a single unlabeled root
node, and this node to the default set of the named variable.

\item[{\em variable-name}~~~(where {\em variable-name} is a word
			     beginning with a capital letter)]~\\
For one kind of conversion, implicit conversion, this is
...
and this conversion may create variables or add nodes of these other
trees to sets of variable values or defaults.  Thus the other
{\em expression-graphs} may provide variable values or defaults that
are disconnected from what will eventually become the part of the
expression graph reachable from its \verb|-ROOT-|.

\end{list}
\end{indpar}

After the raw expression is converted to an expression tree plus
a set of variables, a \ttmkey{-ROOT-}{of conversion} variable is
created whose value is the root of the expression tree and which
has no default.  The unification that is then performed is that
described in \ref{UNIFICATION} except the merge set is initialized
...
before unification is attempted.
...
represents an unlabeled node that is the source of no arrows but is a
value of the variable named L.
For \ikey{explicit conversions}{explicit conversion!to expression graph}
no such special provision is made for labeled nodes.

The exact operation names used in these conversions may be different
from those given above.  The names to be used are given as to the
...
be easily proved that all withnesses of every node are compatible.
...
(\ref{UNIFICATION}) then any two nodes that are merged during
unification are 1-witness compatible.  This is because
step (\ref{UNIFICATION-AMBIGUITY-FAILURE}) of the unification algorithm
prohibits mergers where one node contributes incompatible 1-witnesses
while the other node contributes at least one 1-witness.
...
would be two paths corresponding to the two nodes with the consecutive
nodes on the two paths being merged, because of the nature of
the unification algorithm.  The two paths would end at two nodes that
would have different labels, but would have to be merged, causing
the unification algorithm to fail.
Step (\ref{UNIFICATION-AMBIGUITY-FAILURE}) of the unification algorithm,
requires that along each of the two paths each node sources exactly
one arrow with the next arrow label on the path.
...
we need to avoid this failure, and we do this in \ref{EXPRESSION-MATCHING}
...
\label{EXPRESSION-MATCHING}

\ikey{Expression matching}{expression matching}{matching!expression}
is a variant of expression unification (\ref{UNIFICATION})
that differs from expression
unification when merging nodes that have qualifiers or that have
optional, reordered, or rest arguments.  Expression matching is
...
\ikey{Inference matching}{inference matching}\index{matching!inference}
is a variant on unification (\ref{UNIFICATION}).  What is changed is
the way two nodes are merged if one or both nodes are the source of
an arrow labeled \ttmkey{-ARITY-}{merge}.  The alterred merge algorithm is
called the \key{arity merge algorithm}.

The arity merge algorithm performs \skey{arith check}s on the nodes it
is merging.

A node N passes an arity check if and only if
all of the following are true:

\begin{enumerate}

\item
N is the source of exactly one arrow labeled `\ttmkey{-ARITY-}{check}'.
This arrow is called the \key{arity arrow} of N, and its target is
called the \mkey{arity target}{of node} of N.

\item
The arity target of N is a node labeled A, where A is
a natural number (string of decimal digits with no high order zeroes).
A is called the \mkey{arity}{of node} of N.

\item
N is the source of exactly one arrow labeled `\verb|0|'.
This arrow is called the \key{head arrow} of N, and its target is
called the \mkey{head}{of node} of N.

\item
If an arrow sourced at N has a non-zero natural number label I, then
\verb|1|$\leq$I$\leq$A, and there is exactly one arrow labeled I sourced at N.
These arrows are called the \skey{unreordered argument arrow}s of N,
and their targets are called
\ikey{unreordered arguments}{unreordered argument!of node} of N.
The target of the arrow labeled I is called the \key{I'th argument} of N.

\item
If an arrow sourced at N has a label of the form `J\verb|-|K', for
natural numbers J and K, then 1$\leq$J$<$K$\leq$A, and there are exactly
K$-$J$+1$ arrows sourced at N with this label.
These arrows are called the \skey{reordered argument arrow}s of N,
and their targets are called
\ikey{reordered arguments}{reordered argument!of node} of N.

\item
There is a 1-1 mapping between the integers
1 through A and the unreordered and ordered
argument arrows of N
such that for 1$\leq$I$\leq$A,
I is mapped to an arrow that either
has label I or has label `J\verb|-|K' with J$\leq$I$\leq$K.
Every unreordered and ordered argument arrow must be mapped to
some integer by this mapping.
The arrow mapped to I is called the \key{assigned I'th argument arrow}
of N, and the target of this arrow is called the
\key{assigned I'th argument} of N.
...
\item
There is at most one arrow sourced at N labeled
`\ttmkey{-REST-}{arity check}'.
This arrow is called the \key{rest argument arrow} of N, and its target R is
called the \mkey{rest argument}{of node} of N.

\item
If N has a rest argument R,
R is a node with no label and is the source of no arrows.

\item
There may be any number of arrows sourced at N labeled
\ttmkey{-QUALIFIER-}{arity check}.  These arrows are called the
\skey{qualifier arrow}s of N, and their targets are called the
\ikey{qualifiers}{qualifier!of node} of N.
...
or a qualifier arrow.

\item
The qualifiers of N (targets of qualifier arrows)
satisfy the arity check recursively.  The arity check
...
The \key{arity merge algorithm} makes choices during its execution.  Each
set of possible choices over the whole matching algorithm that contains
arity merges leads to a separate matching algorithm execution that
either succeeds or fails.

The \key{arity merge algorithm} for a merge pair (M,N) in the merge set,
where either M or N or both is the source of an arrow labeled
`\verb|-ARITY-|', and with a particular set of choices, is as follows.
...
\item
If either M or N fails to pass an arity check, the unification algorithm
fails.

\item
If M and N are the same node, terminate the arity merge algorithm.

\item
If both M and N may have a reordered or rest argument, the
unification algorithm fails.

\item
If N has a reordered or rest argument, swithch M and N, so only M
has a reordered or rest argument.

\item
If M has does not have a rest argument R and MA does not equal NA,
the unification algorithm fails.

\item
If M has a rest argument R, let MA be the arity of M and NA be the
arity of N, and run the following \key{remainder merge subalgorithm}.

\begin{enumerate}

\item
If MA is greater than NA, the unification algorithm fails.

\item
For each natural number I from MA$+$1 through NA, an arrow
an arrow labeled I$-$MA is added that has R as its source and the
I'th argument of N as its target.

\end{enumerate}

\item
Choose an argument order assignment for M.  For \verb|1|$\leq$I$\leq$A,
let MI bet the assigned I'th argument of M and NI be the I'th argument
of N, and add the merge pair (MI,NI) to the merge set.
