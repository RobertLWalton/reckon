...
\emkey{name} ::=
	\begin{tabular}[t]{@{}l@{}}
	   ...
	   $|$ {\em decimal-natural} \}$^\star$ \\
        {\em non-special-symbol} \\
        \{ {\em non-special-symbol} $|$ {\em decimal-natural} \}$^\star$
        \end{tabular}
\\[1ex]
...
\begin{indpar}
\begin{list}{}{}
\item[\ikey{Parentheses Rule}{parentheses rule}:]
...
like syntax equations except their right sides ae patterns and not
...
correct order (there are some more details below).
For example,
given the pattern equation:
...
so that the result would be consistent were it a syntax equation between
...
The last sentence of the Pattern Unfolding Rule requires explanation.
...
quickly stop, since the supply of syntax expressions is limited to
...
then the set of equations containing will either imply by deduction
some inconsistent syntax equation, or the pattern equation will be
unsolvable.
