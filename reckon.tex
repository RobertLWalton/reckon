...
There is one last type of datum you can compute with in PCASL, which is
the \key{block}:
...
One can also iterate over data.  Here we use recursion, which tends to
produce more compact code in simple cases.

\begin{indpar}
\verb|> define (the average weight of X)| \\
\verb|+         -: tuple X of persons {| \\
\verb|+     define (the sums of X)| \\
\verb|+             -: tuple of persons X {| \\
\verb|+         # Returns (count,sum) where| \\
\verb|+         # count is the number of persons in the tuple | \\
\verb|+         # and sum is the sum of their weights.| \\
\verb|+         if ( X = () ):| \\
\verb|+             value = (0,0)| \\
\verb|+         else:| \\
\verb|+             value = (1,the weight of (first X))| \\
\verb|+                   + the sums of (rest X) }| \\
\verb|+     (count, sum) = the sums of X| \\
\verb|+     value = sum / count }| \\
\verb|> the average weight of (all persons)| \\
\verb|116.5 lbs|
\end{indpar}

In case you wonder how some of this works, here are some hints.
