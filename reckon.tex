...
\key{number} ::= {\em number-character} {\em number-character}\,$^\star$
\label{NUMBER-LEXEME}
\\[1ex]
\key{quoted-string} ::=
...
\begin{tabular}{l}
\emkey{atom} ::= {\em symbol} $|$ {\em number}
\end{tabular}

\begin{tabular}{l}
\emkey{symbol} ::= see \pagref{SYMBOL-LEXEME}
\end{tabular}
...
meaning in a representation, such as the \verb|:|,
...
be represented by \verb|":"|, \verb|"@"|, or \verb|";"|.

Other than this rule, there is no
distinction between quoted and unquoted representations of symbols.
However, quoted numbers and unquoted numbers are distinct;
...
the numbers are output, they may or may not have commas.
...
The following are generalizations about certain attribute labels.
First, the strictly positive integers \verb|1|, \verb|2|,
\verb|3|, \ldots are used to name the elements of a list.  Second,
some attribute labels beginning with `\verb|.|' have special
...
subexpressions with implicit brackets with sublists, and by
...
object whose only attributes are an attribute labeled {\tt 1} whose value
...
Indentation lexemes are controlled by bracket definitions
