...
There are two standard parsers:
...
operators or separators whose definitions are current, whereas
and the `text' parser has a definition selected
by the `text' selector, does not recognize operator or separator
definitions even if they are current, and does special text parsing
that identifies sections, paragraphs, sentences, and phrases.
...
A \key{parser} is a function which when called with a subexpression that
is just a list of lexemes returns the parsed subexpression, which
...
The most recent current parser definition
...
parser recognizes current operator and separator definitions,
and only the text parser recognizes paragraphs, sentences, etc.
