...
A {\em raw-object-representation} is a logical line beginning
with a {\em raw-object-identifier} lexeme, which consists of `\TT{*@}'
...
\\\hspace*{0.5in}
	\begin{tabular}[t]{rl}
        := {\em bll} & {\em raw-object-identifier} ~
...
\begin{tabular}{l}
\emkey{raw-object-identifier} :::=
	\ttmkey{*}{in {\em raw-object-identifier}}
...
\emkey{atom-name}
    ::= {\em word} $|$ {\em mark} $|$ {\em quoted-string}
                   $|$ {\em unquoted-numeric}
		   $|$ {\em number-name} $|$ {\em label-name} \\
\emkey{unquoted-numeric}
    ::= {\em numeric} but not {\em scientific-numeric} \\
\emkey{number-name}
    ::= {\em scientific-numeric} \\
\end{tabular}
...
\label{RAW-SINGLE-ATTRIBUTE-REPRESENTATION}
::=
\\\hspace*{0.5in}
	...
	{\em raw-single-attribute-value}\STAR{} ~ {\em ell}
	\end{tabular}
\end{tabular}
...
\begin{tabular}{l}
\emkey{raw-single-attribute-value}\label{RAW-SINGLE-ATTRIBUTE-VALUE}
    ::= {\em raw-element}
\end{tabular}
