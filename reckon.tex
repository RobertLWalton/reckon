...
\emkey{separator} ::= {\em separator-character} \\[1ex]
\emkey{name} ::=
...
\emkey{vertical-space-character} ::=
    \key{line-feed} $|$ \key{vertical-tab} $|$ \key{form-feed} \\[1ex]
...
digits.  When a number is input by the compute, stored, and then output,
...
A \key{datum} in CASTLE is either an atom, an object, or an arrow.
...
\begin{tabular}{l}
\emkey{object-name} ::=
	\ttmnbkey{@}{in {\em object-name}} {\em decimal-natural}
\end{tabular}
...
the CASTLE system, and should not be defined by CASTLE users.  {\tt .GLOBAL}
is an example of such a label.

The CASTLE parser (\secref{PARSING})
...
{\em raw-attribute-representations}, or \verb/{| |}/ bracketed and
...
\\[1ex]
{\em expression} ::= see \secref{EXPRESSION-PARSERS}
...
of CASTLE that lists with \verb|{ }| brackets
...
A nofix operator cannot be infix, prefix, postfix or afix.
...
The initial segment is terminated the end of th input list,
...
result.  If there is no lead operator, nothing is done.
...
\item The output list is reformatted and compacted (becoming the
...
\item
If the current operator fixity is prefix, infix, or nofix, then
...
are matched to the expression.  An expression definition may also have
guards, which are expressions that must evaluate to true in order for
the expression definition match to succeed.
...
evaluated by the expression definition.  In addition, there may be other
...
a set of constraint equations are established that must be statisfied to
complete the match.  There are two kinds of constraint equations: syntax
equations, and guards.  Syntax equations involve syntax
variables, which are names beginning with a capital letter optionally
preceded by a \verb|$|.  Guards are just expressions that must evaluate to
\verb|true| in order for the match to succeed.  The syntax equations
are solved during the matching process, to produce values for the
syntax variables.  If the syntax equations are inconsistent and
cannot be solved, of if a guard evaluates to \verb|false|,
the match fails.  Evaluation of the guards in a definition
may result
in additional matches which generate more syntax equations involving
more the syntax variables and also generate more guards.
...
to the evaluator operating mode, as is described below.

A definition may have default values for a syntax variable.  If
the matching process fails to provide a value for the variable, the
default value will be supplied.  In effect the definition is 
a set of different definitions, one for each possible substitution of
default values for their corresponding syntax variables.
...
As an example, if `\verb|foo(2+2)|'
is matched to the pattern `\verb|foo($X)|', then the matching syntax
equation is
\begin{center}
\verb|foo(2+2) :=: foo($X)|
\end{center}
(where \verb|:=:| means `is syntactically equal to'), which yields
the equation
\begin{center}
\verb|2+2 :=: $X|
\end{center}
...
A syntax variable name in the pattern part of a definition that does
not begin with \verb|$| is treated as if it had a preceding \verb|$|.
Such a variable is not allowed to appear with a preceding \verb|$|
anywhere in the definition.  Thus omitting the \verb|$| is both a
convenient shorthand and a promise never to use the unevaluated expression
denoted by the variable name preceded by \verb|$|.

Thus in the above we could have used the pattern
`\verb|foo(X)|' in place of the pattern
`\verb|foo($X)|'.  But if we did that, we could not use \verb|$X|
anywhere in the definition, although using just \verb|X|, as in
\verb|number(X)|, is allowed.

Except for choices of which definitions to match to which expressions,
the entire matching process is monotonic.  This means the order of
matching operations, guard evaluations, and evaluations of unevaluated
syntax variable values does not matter.

In order to make the matching process monotonic, pattern matching must
be monotonic.  This means that if undefined syntax variables in a matched
pattern are given arbirary values later, the pattern match
must still be valid.
...
Lastly, required evaluations of unevaluated syntax variables
...
then the entire matching process fails, just as it
...
\emkey{expression-definition} ::=
	...
	\\[1ex]
\emkey{pattern} ::= \verb|<-|{\em -subexpression}
	~~~ [see \pagref{O-SUBEXPRESSION}] \\[1ex]
\emkey{guard-list-option} ::= {\em empty} $|$ {\em guard-list} \\[1ex]
\emkey{guard-list} ::= {\em guard} \{ \verb|,| {\em guard} \}$^\star$ \\[1ex]
\emkey{guard} ::= \verb|,|{\em -subexpression}
	~~~ [see \pagref{O-SUBEXPRESSION}] \\[1ex]
\emkey{block-option} ::= {\em empty} $|$ {\em block} \\[1ex]
\emkey{block} ::= \verb|{| {\em block-header}\,$^\star$ {\em statement}
                           \{ \verb|;| {\em statement} \}$^\star$ \verb|}|
	    \\[1ex]
\emkey{block-header} \begin{tabular}[t]{rl}
            ::= & \ttkey{default}
	    	\verb|{| {\em default-equation}
		\{ \verb|;| {\em default-equation} \}$^\star$ \verb|}| \\
	    $|$ & \ttkey{constraint}
	    	\verb|{| {\em constraint-equation}
		\{ \verb|;| {\em constraint-equation} \}$^\star$ \verb|}| \\
	    \end{tabular} \\[1ex]
{\em statement} ::= see \pagref{STATEMENT} \\[1ex]
\emkey{default-equation} ::= {\em syntax-variable}
		\ttmnbkey{:=:}{in {\em default-equation}}
		{\em syntax-expression} \\[1ex]
\emkey{constraint-equation} ::= {\em syntax-expression}
		\ttmnbkey{:=:}{in {\em constraint-equation}}
		{\em syntax-expression} \\[1ex]
\emkey{syntax-expression} ::= ~~ \verb|:=:|{\em -subexpression}
				[see \pagref{O-SUBEXPRESSION}]
\\[3ex]
\emkey{pattern-object} ::=
    \begin{tabular}[t]{l}
    {\em pattern-term-option}
        \{ \ttmkey{.}{in pattern} {\em pattern-term} \}$^\star$ \\
    {\em pattern-attribute-representations}
    \end{tabular} \\[1ex]
\emkey{pattern-term-option} ::= {\em empty} $|$ {\em pattern-term} \\[1ex]
\emkey{pattern-term} \begin{tabular}[t]{rl}
            ::= & {\em pattern-clause} \\
            ::= & {\em pattern-clause} \ttmkey{?}{in pattern} \\
            ::= & {\em pattern-clause} \ttmkey{...}{in pattern} \\
            ::= & {\em pattern-clause} \ttmkey{...?}{in pattern} \\
	    \end{tabular} \\[1ex]
\emkey{pattern-clause} ::= {\em pattern-factor} {\em pattern-factor}\,$^\star$
	\\[1ex]
\emkey{pattern-factor} ::= {\em atom} $|$ {\em pattern-object} \\[1ex]
...
A \key{pattern} is just a \verb|<-|{\em -subexpression}
(\pagref{O-SUBEXPRESSION}) that is an object consisting of a list
and other attributes.  The list part is divided into
{\em pattern-terms} and {\em pattern-factors} by the use of
the marks \verb|.|, \verb|?|, \verb|...|, and \verb|...?| which
the operator parser does not recognize.
Subexpressions in a {\em pattern} are also treated as {\em patterns}.
Thus in the {\em pattern}
`\verb|sort ( X, X ...? )|'
the subexpressions `\verb|( X, X ...? )|',
`\verb|X|', and `\verb|X ...?|' are all patterns.

In use, an expression definition being matched to an expression is
copied, and then the pattern of the copy is matched
with the expression.  The act of copying the definition
creates a new set of variables that are distinct from any
previous variables.  If pattern matching fails, the definition
match fails.  If pattern matching succeeds, it generates consistent
syntax equations.
In this case the matching process continues
by generating constraint syntax equations and guards and then searching for
...
\skey{syntax expression}s.  A syntax expression is an syntax variable,
...
The identity `\ttnbkey{:=:}' in a syntax equation is taken to be
symmetric and transitive.  This means that if $e_1$, $e_2$, and
$e_3$ are syntax expressions, then

\begin{indpar}
\begin{list}{}{}
\item [\ikey{Symmetry Rule}{symmetry rule}:]\label{SYMMETRY-RULE}
From $e_1$ {\tt :=:} $e_2$ we deduce $e_2$ {\tt :=:} $e_1$.

\item [\ikey{Transitivity Rule}{transitivity rule}:]%
\label{GENERAL-TRANSITIVITY-RULE}
From $e_1$ {\tt :=:} $e_2$ and $e_2$ {\tt :=:} $e_3$
we deduce $e_1$ {\tt :=:} $e_3$.
\end{list}
\end{indpar}
...
in previous equations.  An algorithm that keeps making deductions until
no new deductions can be made will stop fairly quickly.

Also note that as a technical point, if all we want is to deduce all
syntax equations for which one side as a syntax variable, then we can
restrict the application of the transitivity rule to the case where
$e_2$, the eliminated sytax expression, is a syntax variable.
[TBD: Check this.]
If this variable is denoted by $v$, then we can rewrite transitivity as:

\begin{indpar}
\begin{list}{}{}
\item [\ikey{Transitivity Rule}{transitivity rule}:]\label{TRANSITIVITY-RULE}
...
\emkey{pattern-object} ::=
    ...
    \end{tabular} \\[1ex]
\emkey{pattern-term-option} ::= {\em empty} $|$ {\em pattern-term} \\[1ex]
\emkey{pattern-term} ::= {\em pattern-clause} {\em pattern-marker-option}
	\\[1ex]
\emkey{pattern-clause} ::= {\em pattern-factor} {\em pattern-factor}\,$^\star$
	\\[1ex]
\emkey{pattern-factor} ::= {\em pattern-object} $|$ {\em atom}
		       $|$ {\em syntax-variable} \\[1ex]
\emkey{pattern-marker-option} ::= {\em empty} $|$ {\em pattern marker} \\[1ex]
\emkey{pattern-marker} ::= \ttmkey{.}{pattern marker}
		       $|$ \ttmkey{?}{pattern marker}
		       $|$ \ttmkey{...}{pattern marker}
		       $|$ \ttmkey{...?}{pattern marker}
	\\[1ex]
\emkey{pattern-attribute-representations}
...
{\em pattern} is not preceded by `\verb|.|', it must be first in
the {\em syntax-expression}.

\item A {\em pattern-clause} not followed by `\verb|?|',
`\verb|...|', or `\verb|...?|' in the {\em pattern} must appear exactly
...
means removing the \skey{pattern marker}s,
`\verb|.|', `\verb|?|', `\verb|...|', and `\verb|...?|',
and arranging the pattern clauses in the
correct order.  For example, given the pattern equation:
...
A pattern that contains pattern markers,
`\verb|.|', `\verb|?|', `\verb|...|', and `\verb|...?|',
in its list part outside subexpressions
is said to be `\key{folded}'.  Other patterns are said to be
\key{unfolded}.  Subexpressions of a pattern are also patterns.

A pattern that is unfolded and all of whose subexpressions are
unfolded, that is, a pattern that contains no pattern markers,
even in subexpressions, is said to be \key{completely unfolded}.
...
Note that
the Symmetry Rule (\pagref{SYMMETRY-RULE})
and general Transitivity Rule (\pagref{GENERAL-TRANSITIVITY-RULE})
...
Also note that the Parentheses Rule (\pagref{PARENTHESES-RULE})
and Atom Rule (\pagref{ATOM-RULE}) apply to pattern equations.
...
If a pattern equation asserts the identity of a syntax equation that
is an object with a pattern, and if the pattern can be unfolded unambigously
...
ambigous pattern equation will have been promoted to be a syntax equation,
which may or may not be consistent.

\subsubsection{Default Syntax Variables}
\label{DEFAULT-SYNTAX-VARIABLES}
...
\subsubsection{Repeated Syntax Variables}
\label{REPEATED-SYNTAX-VARIABLES}
...
variable name, each with two \verb|$|'s in it.  Users of CASTLE should
not use \verb|$| inside a syntax variable name in order to avoid conflicting
with this usage.
...
syntax variable names created by the CASTLE system.
...
\\[1ex]
{\em syntax-variable} ::= {\em unevaluated-syntax-variable}
			  ~~~ [as per \pagref{SYNTAX-VARIABLE}]
\end{indpar}
...
syntax varibles it contains replaced by their complete unevaluated values.
...
is the value obtained by evaluating the full unevaluated value of
{\tt \$}$v$ in the context
...
\begin{indpar}\begin{verbatim}
sum ($X) and ($Y) <-- number(X), number(Y) { result = X + Y }
. . . . .
z = sum (1+1) and (5+5)
\end{verbatim}\end{indpar}
...
`\verb|result = X + Y|'.  These last two equations are obtained by
...
Logically evaluated syntax variables are not permitted in patterns.
...
\verb|$|$v$, and in addition, any occurances of $v$ outside patterns
...
\begin{indpar}\begin{verbatim}
increment ($X) . by Y ? <-- number (X), number(Y), Y :=? 1:
    $X = X + Y
\end{verbatim}\end{indpar}
...
[TBD: how should lvalues really be handled?]
...
\emkey{statement} ::= {\em qualified-statement} $|$ {\em empty} \\[1ex]
