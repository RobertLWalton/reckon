...
It causes operands preceeding the operator to be inspected to see if they are
...
\label{POSTFIX},
{\bf nofix}\index{nofix flag@{\tt nofix} flag}%
\label{POSTFIX}~~~~~
These flags determine whether the operator the operator
\key{fixity}.  By default an operator is nofix.

An \key{infix} operator must be between two non-empty operands,
while a \key{prefix} operator must preceed a non-empty operand,
...
Inside an expression, an operator that can be a prefix if it
follows an opening bracket or another operator that is not
postfix.  An operator can be postfix if it preceeds a closing
bracket or another operator that is not prefix.  
An operator can be infix if it does not follow an opening
bracket or an operator that is prefix, and also it does not
preceed a closing bracket or an operator that is postfix.

An operator cannot be both infix and nofix, but can be any other
combination of fixity.  Ambiguities in parsing are possible and
are treated as parsing errors.  For example, if an operator that
can be postfix or infix preceeds an operator that can be infix
or prefix, there is an ambiguity.
...
an optional bracket definition \\
\end{tabular}
\end{center}
...
a list of all lexemes after the found closing bracket \\
\end{tabular}
\end{center}

If no bracket definition argument is given as input, the list
of lexemes returned is empty.
...
The operator parser removes lexemes from the beginning of an
input list, which it initialized to the parser's
lexeme list input argument, and adds {\em expression-items}
to the end of an output list, that becomes the parser's
output parsed expression.  The parser then executes a
\key{parsing loop} that repeats the following:

\begin{enumerate}

\item If there is no bracket definition input argument
and the input list is empty, the parsing loop terminates.

\item If there is a bracket definition input argument, and the
closing bracket from that definition is at the beginning of
the input list, this bracket is removed from the beginning of
the lexeme list, and the parsing loop terminates.

\item If there an opening bracket from a current bracket definition
at the beginning of the input list, then:
...
lexeme list argument and the bracket definition as an argument.

\item The input list is set to the lexeme list returned by the
parser call.

\item If the bracket definition has a reformatter, it is called with
the parsed expression returned by the parser call as it argument,
and the value it returns is pushed to the end of the output list.  Otherwise
the parsed expression returned by the parser call is pushed to the end of the
output list.

\item The parsing selector stack is popped if necessary to restore
it to its state when the bracket was removed from the beginning of
the input list.

\end{enumerate}

\item If there is an operator from a current operator definition
at the beginning of the input list, then:

\begin{enumerate}

\item If this is the first operator encountered, it is made the
\key{lead operator} of the output list, and:


\begin{enumerate}

\item
The operator definition becomes the lead operator definition of
the parser.
...
\item
The operator lexemes are removed from the input list and
\end{enumerate}
