...
An {\tt infix} operator may be \ttkey{left} or \ttkey{right} associative, or
it may have some other associativity that is named by any lexeme other
that {\tt left} or {\tt right}.  The last kind of {\tt infix} operator
is said to have \key{named associativity}.
...
If all operators are of named associativity, all are recognized, and 
...
Then the name of the associativity is prepended to the expression.
Thus if the {\tt infix} operators \verb|<| and \verb|<=| have associativity
\verb|-COMPARE-| and the expression \verb|0 < x - 3 <= 5| will
