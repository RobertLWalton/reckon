...
In this document names of syntactic categories that are expressions defined
by expression definitions are capitalized, while other syntactic category
names are lower case.  The syntactic category {\em EXPRESSION-DEFINITION}
itself corresponds to the expression definition:

\begin{indpar}
\begin{verbatim}
( #PATTERN <-- #GUARD-LIST {#BLOCK} ) <--
\end{verbatim}
\end{indpar}

TBD, how to handle \verb|{ }| as implying a qualifier.
...
may not be modified.  It can be created by the expression

\begin{indpar}
\ikey{IMMUTABLE-CONTEXT}{immutable-context} \\
	\hspace*{0.5in}::= \verb|a context (| {\em CONTEXT-ITEM}
		     		\{ \verb|,| {\em CONTEXT-ITEM} \} \verb|)|
\\[1ex]
\ikey{CONTEXT-ITEM}{context-item} ::= {\em expression}
\end{indpar}

in which each {\em CONTEXT-ITEM} is either an expression definition or
a context.  An {\em IMMU\-TABLE-CONTEXT} is defined by the
...
\key{block} ::= \verb|{| {\em statement}
                         \{ \verb|;| {\em statement} \}\,$^\star$ \verb|}|
			 \\[1ex]
\key{statement} ::= {\em empty-statement}
...
\key{right-side} \
    \begin{tabular}[t]{rl}
    ::= & {\em expression} \\
    $|$ & {\em expression} \ttnbkey{=>} {\em output-variable}
...
an {\em expression-definition}, \pagref{EXPRESSION-DEFINITION})
...
from the statement containing the block.
...
{\em subblock} are variables named in the {\em block}.
...
{\em Output-variables} are variables named after \verb|=>| following
an expression.  In the statements
...
an error if the variable is assigned more than one value.  Each
{\em right-side} and {\em guard} in a {\em block}
is evaluated only when all the input variables it contains have values.
When the {\em right-side} of an {\em assignment-statement} is evaluated,
the variables named on the left side of the statement and any
{\em output-variables} named after a \verb|=>| in the {\em right-side}
are given values.
When a {\em guard} is evaluated, any
...
if the current block iterates because it has not produced a value
for the {\tt value} variable.  The next block starts with these
