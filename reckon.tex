...
on page~\pageref{QUALIFIER-EXAMPLE}.  The root of this expression graph
...
by pure unification (\ref{PURE-UNIFICATION}),
then any two nodes that are merged during
unification are 1-witness compatible.  This is because
step \ref{UNIFICATION-AMBIGUITY-FAILURE} of the node merge algorithm
(page~\pageref{NODE-MERGE-ALGORITHM})
...
used in the expression evaluation algorithm (\ref{EVALUATION-ALGORITHM}).
...
being merged is a call node (\ref{CALL-NODES-AND-CHECK}), and the other
...
an optional, reorderable, or rest argument.  Otherwise the node is said
...
N is always of determinant arity.
...
of MA and NQ be the target of NA.
...
that has the same arrow label as MA (\verb|-REQUIRED-QUALIFIER| or
...
PCASL may have a single memory of nodes and arrows that holds all expession
...
evaluation (\ref{EVALUATION-ALGORITHM}).  To make this more efficient,
