...
Similarly {\em labels} include in named brackets
...
Here the entire bracketted subexpression `\verb/<h(1)|/ \ldots{} \verb/|>/'
...
Also the `\verb/<h>/' operator is programmed to attach its argument,
...
when it closes `\verb/<h(1)|/' in this example.  Its also allowed to
use either an initial or final segment of the name in a closing
named bracket, as in `\verb/<indented bold p|/ \ldots{} \verb/|p>/'
or `\verb/<indented p|/ \ldots{} \verb/|indented>/'.

In the above example,
`\verb/<p>/' is a lower precedence operator than `\verb/<h>/', and
`\verb/<s>/' is lower precedence than `\verb/<p>/'.  Also the sentence
...
Named operators can also be used as named brackets, in which case the
bracketted text becomes the {\tt .title} attribute of the text whose
{\tt .text-type} attribute is set by the
operator.  The `\verb/<h>/' operator above is used in its
bracket form `\verb/<h|/ \ldots{} \verb/|h>/' to attach a {\tt .title} to
the text of {\tt .text-type h}. 
...
no {\.tt .text-type} attribute, it can be assigned a {\tt .text-type}
...
\\[2ex]
-1000+10$n$
	& \ttkey{text-prefix}	& \tttkey{h($n$)}
						& section header \\
-200	& \ttkey{text-prefix}	& \tttkey{li}	& list item \\
0	& \ttkey{text-prefix}	& \tttkey{p}	& paragraph \\
...
\ttmkey{text-prefix}{reformatter} \hfill \OP{h} ~~~ \OP{li} ~~~ \OP{p}
...
The `\verb|<h>|' operator takes an additional argument $n$ (default {\tt 0})
which becomes the {\tt .level} attribute of the reformated list.  Also,
$10n$ is added to the precedence of the operator.
...
the sole element of a new list.  Then the {\tt .text-type} and {\tt .label}
attributes of this list are set from the {\em operator} and the
