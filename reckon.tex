...
\\\hspace*{0.5in}
    {\em raw-object-header} {\em single-attribute-value}$^\star$
...
\begin{tabular}{l}
\emkey{attribute-label-flags-option} ::= {\em empty} $|$ [ {\em symbol} ]
\end{tabular}%
...
\begin{tabular}{l}
\emkey{atom} ::= {\em non-special-symbol} $|$ {\em number}
\end{tabular}
...
\\\hspace*{0.5in}
	 {\em reverse-attribute-label} {\em attribute-label-flags-option}
\end{tabular}
...
the numbers are output, they may or may not have commas.  Thus
\verb|1,000| may become \verb|1000| on output.  This is
not true of symbols, so it would not be true of \verb|"1,000"|,
