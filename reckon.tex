...
We will describe these in order.
...
The \key{optional mark} (\ttkey{?:?}) is used in expression definitions
...
the `\verb|LIST|' and `\verb|INDEX|' argument can be reordered.
...
optional qualifier marks and an optional mark may be inserted into an
...
the optional mark (\verb|?:?|), the reorder mark (\verb|<:>|),
...
affects on parsing.  An informal summary of a few of these is:
...
Parsing is controlled by the \key{parsing context}, which is a stack
that contains definitions of parsers, operators, qualifiers, and
qualifier shortcuts.  An operator may push definitions into the parsing
context.  After the expression containing the operator has been parsed,
these definitions will be popped from the parsing context.

A \key{parser definition} just names a parser.  The most recent parser
definition in the parsing context names the parser used to parse expressions.
There are three standard \ikey{parsers}{parser!standard}:
the \key{marks parser} \ttkey{-MARKS-PARSER-},
where only the optional mark (\verb|?:?|), reorder mark (\verb|<:>|),
...
matching lexemes and the \verb|?:?| optional,
...
not matter.  The {\em optional-mark} (\verb|?:?|),
...
parsing context by some operators.
...
Arguments appearing after the \key{optional mark} \ttkey{?:?} are
...
`$m$\verb|-|$n$', where $0\leq m<n$, $m$ is the number of required arguments,
$n$ is the total number of arguments, and $n-m$ is the number of optional
arguments.
