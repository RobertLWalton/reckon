...
and consists of substeps called \skey{operator scan}s.
An operator scan performed on an expression finds all the
the expression items that are operators, picks from these the operators
of lowest precedence, and inserts implied parentheses around the arguments
of these operators, making new subexpressions.  Operator parsing
is just performing an operator scan of an expression and then performing
operator parsing recursively on all the subexpressions of the result of
the operator scan.

TBD

Operator parsing is done under the control of the \key{lexical context}.
In many cases, the lexical conte
which is defined 
