...
Subexpressions can be \mkey{restructured}{expression}.  Matching lexemes,
such as \verb|`'|, control how a subexpression they surround is
restructured.  They also control the set of operators recognized
in this subexpression.

Certain lexemes, such as `{\tt ,}' and `{\tt ;}', are used to separate
elements of lists.  These are called \skey{element separator}s, and
are treated as if they were operators.

The process of inserting implied parentheses and
restructuring expressions is called
\key{expression parsing}\index{parsing!expression}.
Expression parsing takes an expression as input, and produces as output an
expression that contains nothing that will trigger further parsing.
...
An informal summary of the affects of the standard bracket marks is:

\begin{center}
\begin{tabular}{l@{~~~~~~}l@{~~~~~~}l@{~~~~~~}l@{~~~~~~}l@{~~~~~~}l}
	& Matching  & Turns	  & Turns    & Sets	& \\
In	& Bracket   & Operator	  & Lexical  & Parser	& Turns \\
Parser	& Marks	    & Recognition & Parsing  & To	& Evaluation
\\[1ex]
any & \verb|(|\ldots\verb|)|
...
Other matching bracket marks may be made to affect parsing by introducing
matching bracket definitions (\secref{MATCHING-BRACKET-DEFINITIONS}).
...
names a parser and gives an optional evaluation mode.  The most recent parser
...
\key{PARSER-DEFINITION} \\
	\hspace*{0.5in}::= \verb|define parser| {\em PARSER-NAME}
		{\em evaluation-mode-option} \\[1ex]
\key{PARSER-NAME} ::=
	\verb|[| {\em word} {\em word}\,$^\star$ \verb|]| \\[1ex]
\key{evaluation-mode-option} ::= {\em empty} $|$
	\verb|with| \verb|evaluation| {\em evaluation-mode} \\[1ex]
\key{evaluation-mode} ::= \verb|on| $|$ \verb|off|
\end{indpar}
...
that recognizes operators;
and the \key{text parser}, \ttnbbkey{TEXT PARSER},
where the format-separator (\verb/|/) and punctuation are recognized.
...
Some of the standard bracket marks affect evaluation.
