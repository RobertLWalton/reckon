...
names.  Either a prefix or postfix sequence may be empty.
...
deselected as `\verb|+|' is both an infix \underline{and} a prefix operator,
...
to be the smallest equivalence relation such that two selected infix
operators conflict if they overlap or abut.

\item
De-select any infix operator that is followed by another infix operator whose
first lexeme is not also the first lexeme of a prefix sequence.
De-select any infix operator that is preceded by another infix operator whose
last lexeme is not also the last lexeme of a postfix sequence.
Note that the prefix and postfix sequences considered here may be in the
...
The parse is in error if any equivalence class of
conflicting operators does not have exactly one member that is
\underline{not} deselected by this step.
...
First we define classical parsing.  This is parsing according to
...
{\em expression$_{M+2}$} ::=
	...
	\\[1ex]
{\em expression$_{M+3}$} ::= {\em non-operator} $|$
	\verb|(| {\em expression} \verb|)|
\end{indpar}

We also make the following
\skey{non-overlap assumption}s\index{overlap!non-overlap assumption}:

\begin{enumerate}

\item Two distinct operators cannot overlap.  (This is trivial if
all operators are just a single lexeme).  In applying this
rule one operator may be both prefix and infix, both postfix
and infix, both prefix and postfix, or all three.

\item No operator can overlap a non-operator.
...
Actually this last statement is not true unless we make the following
additional \key{non-allfix assumption}:

\begin{list}{}{}

\item No operator is prefix, postfix, \underline{and} infix (all three).
...
\begin{lemma}
In the classical parse of an {\em expression}:
\begin{list}{}{}
\item[(a)] the {\em expression} is not empty,
\item[(b)] the {\em expression} does not end with a {\em prefix-operator},
\item[(c)] the {\em expression} does not begin with a {\em postfix-operator},
\item[(d)] the {\em expression} does not contain
a {\em postfix-operator} followed by a {\em prefix-operator}.
\end{list}
\end{lemma}

Proof: By simultaneous induction on the syntax equations.

\begin{lemma}
In the classical parse of an {\em expression}:
\begin{list}{}{}

\item[(a)] the {\em expression} does not begin or end with
an {\em infix-operator},
\item[(b)] the {\em expression} does not contain
a {\em prefix-operator} followed by an {\em infix-operator},
\item[(c)] the {\em expression} does not contain
an {\em infix-operator} followed by
a {\em post-operator},
\item[(d)] the {\em expression} does not contain
an {\em infix-operator} followed by
an {\em infix-operator}.
...
Proof: Each statement can be proved by induction using the previous
lemma and previous statements.
...
produce exactly this parse, provided the non-overlap and non-allfix
assumptions hold, and provided the parse stack has the appropriate operator
definitions and is not changed during the parse.
\end{theorem}
...
end of step~\ref{LOWER-PRECEDENCE-STEP} of the operator selection
algorithm.
Each {\em infix-operator$_n$} is preceeded by an
{\em expression$_{n+1}$} that cannot end in a prefix or infix operator
that is not also a postfix operator.
And each {\em infix-operator$_n$} is followed by an
{\em expression$_{n+1}$} that cannot begin with a postfix or infix operator
that is not also a prefix operator.
So no {\em infix-operator$_n$} is deselected by
step~\ref{} of the operator selection algorithm.

No {\em expression$_{n+1}$} can contain an infix operator of precedence
below $n+1$ outside parentheses.  But these subexpressions might contain
...
and we must show that these are deselected by step~\ref{} of the operator
selection algorithm.

Let us show something a bit more general: that every prefix operator of
the classical parse of {\em expression$_n$} that can also be an infix
operator is deselected by step~\ref{}
of the operator selection algorithm.
By Lemma~\ref{} a prefix operator outside parentheses is preceded by
...
by another prefix operator, it must be deselected by step~\ref{},
as by the non-fix assumption a prefix operator cannot be a postfix operator.
If it is preceded by an infix operator that is a postfix operator, when
...
But by Lemma~\ref{} this would make the classical parse of the expression
ambiguous, which is disallowed, so this case cannot happen.

By a similarl argument every postfix operator of the classical parse
of {\em expression$_n$} that can also be an infix operator is deselected
by step~\ref{} of the operator selection algorithm.
