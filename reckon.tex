...
\item\label{TOTAL-OPERATOR-STEP}
...
If zero operators are selected at this point, and if the expression
...
or prefix operator $X$ in the second parse.  But $X$
cannot be an infix or prefix operator in the first parse, so it must be
a postfix operator in the first parse.  Since $X$ is postfix in the first
parse, and must be prefix or infix in the second parse, it must be
infix in the second parse by A5.  So the {\em expression} contains a
...
to the operator selection algorithm.
\end{lemma}
...
an operator (see step~\stepref{TOTAL-OPERATOR-STEP} of the algorithm).
\end{theorem}

Proof: By steps~\stepref{INFIX-SELECTION-STEP}
through~\stepref{INFIX-OVERLAP-STEP}
and Lemma~\ref{SUBEXPRESSION-DESELECTION-LEMMA}
