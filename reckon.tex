...
if a member of the current parsing selector set is attached to the definition.
...
Text parsing is similar to operator parsing except that instead of operators
and brackets there are initiators, terminators, separators,
and brackets, most of which are `named'.
For example, `\verb|<p>|' is a standard paragraph initiator,
and `\verb|<indented p>|' is a standard paragraph initiator
...
The named text initiators, separators, terminators, and brackets
have the syntax:

\begin{indpar}
\emkey{named-initiator} ::=
    \verb|<| {\em name} {\em labels-option} \verb|>| \\[1ex]
\emkey{named-separator} ::=
    \verb|<| {\em name} {\em labels-option} \verb|>| \\[1ex]
\emkey{named-terminator} ::=
    \verb|<| {\em name} {\em labels-option} \verb|>| \\[1ex]
...
Text initiators, separators, terminators, and brackets are collectively
referred to as `\skey{text mark}s'.  For example, `\verb|<p>|' is an initiator
text mark that introduces a paragraph and `\verb/<b|/' and `\verb/|b>/' are
backets that surround bold text.  Text marks have attributes
such as `{\tt style}', `{\tt weight}', `{\tt indent}', and `{\tt adjust}',
that are used to format the text for printing and display.
For example `\verb|<p>|' has a normal weight and `\verb/<b|/' has a
heavy weight.

Text marks
can also inherit attributes from other text marks.  To facilitate this
there are also virtual text marks that are never used in the text,
but can be ancestors of other text marks and serve as repositories of
attributes.  Lastly it is possible to give several names to the same
text mark, and to give names to attribute values (e.g., `{\tt default style}'
is a standard name for a `{\tt style}' attribute value).

Although there are many
standard text marks, each with standard attributes, a large piece of
text typically defines new marks specific to that text and resets some
attributes of the standard marks in a way specific to the particular
text.

A piece of text can be given a {\em label} by including the {\em label}
in the text mark that is the initiator or terminator of the text,
or the separator that ends the text, or a bracket that bounds
the text.  These labels are internal to the document, and not printed.
...
to the labeled bold text and labeled paragraph.
...
The text marks end up as {\tt .initiator},
{\tt .separator}, or {\tt .terminator} attributes of these lists
and sublists.  In addition, some lists acquire a {\tt .title} attribute
which gives text that serves as the printable name
or title of the attributed text, and a {\tt .label} attribute which
gives labels that can be used in references to the text.
The logical structure of the text is determined by the list structure,
the text mark names used as
{\tt .initiator}, {\tt .separator}, and {\tt .terminator} attributes,
and the {\tt .title} and {\tt .label} attribute values.
The attributes of the text mark names
determine how the text is printed or displayed.

\subsubsection{Text Structure}
...
or {\em right-sides}.
Some are variables in the {\em patterns} of {\em assignment-patterns}.
