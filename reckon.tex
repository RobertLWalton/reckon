...
\begin{tabular}{l}
\emkey{non-special-symbol} ::= see \pagref{SPECIAL-SYMBOL}
\end{tabular}
...
\emkey{name} ::= see \pagref{NAME} \\[1ex]
{\em parsing-selectors} ::= see \pagref{PARSING-SELECTORS}
\end{indpar}
...
\emkey{decimal-natural} ::= see \pagref{DECIMAL-NATURAL}
\end{indpar}
...
\emkey{block-header} \begin{tabular}[t]{rl}
	    ...
	    \end{tabular} \\[1ex]
\emkey{statement} ::= see \pagref{STATEMENT} \\[1ex]
...
\emkey{syntax-variable} \begin{tabular}[t]{rl}
			...
			\end{tabular} \\[1ex]
\emkey{name-component} ::= see \pagref{NAME-COMPONENT}
\end{indpar}
