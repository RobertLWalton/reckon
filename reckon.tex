...
\item
N is the source of exactly one arrow labeled
`\ttmkey{-ARITY-}{check}' whose target is a node labeled A, where A is
...
\item
There is at least one 1-1 mapping from the integers
1 through A onto arrows whose labels are either
non-zero natural numbers or are of the form `J\verb|-|K' for natural
numbers J and K, such that integer I is mapped to an arrow that either
has label I or has label `J\verb|-|K' with J$\leq$I$\leq$K.

\item
There is at most one arrow sourced at N labeled
`\ttmkey{-REST-}{arity check}', and the target R of that arrow, if the
arrow exists, is a node with no label that is the source of no arrows.

\item
There may be any number of arrows sourced at N labeled
\ttmkey{-QUALIFIER-}{arity check}.  If Q is the target of such an
arrow, then Q satisfies the arity check recursively.  The arity check
algorithm must allow cycles in the graph, e.g. the case that N and Q are the
same node.
...
\item
If either M or N fails to pass an arity check, the arity merge algorithm
fails.
...
\item
If one of M or N is the source of an arrow labelled `\verb|-REST-|',
the following \key{remainder merge subalgorithm is executed}.  In
our description of this algorithm we will assume that M is the source
of an arrow labeled `\verb|-REST-|' with target node R, that M is
the source of of an arrow labeled `\verb|-ARITY-|' with target node MA,
and that N the source of of an arrow labeled `\verb|-ARITY-|'
with target node NA.  The remainder merge subalgorithm is run with
M and N switched if these assumptions do not apply.

\begin{enumerate}

\item
If N is also the source of an arrow labeled `\verb|-REST-|',
or of an arrow with label of the form `J\verb|-|K',
the arity merge alogrithm fails.

\item
If MA is greater than NA, the arity merge algorithm fails.

\item
For each natural number I from MA+1 through NA, an arrow
an arrow labeled I-MA is added with source R and destination NI,
where NI is the target of the arrow labeled I with source N.
...
\item
A 1-1 mapping is chosen between nodes of N labeled with natural
numbers
For each natural number I from `\verb|0|' through MA,
