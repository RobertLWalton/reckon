...
\emkey{units-subexpression}
	...
	\end{tabular}
\\[1ex]
\emkey{unitless-name} ::= {\em name}\pagnote{NAME} that does not contain
...
message is issued.  Otherwise the subexpression is reformatted
according to the following rules:
\begin{itemize}
\item[(1)] A `\TT{*}' is placed between consecutive {\em unit-primaries},
after any {\em unit-prefix-subexpression}, and before any
{\em unit-postfix-subexpression}.
\item[(2)] Any `\TT{per}' is replaced by `\TT{/}'.
\item[(3)] Then subexpressions containing infix \TT{*}'s are
reformatted as per the operator pass.  This removes infix \TT{*}'s and
puts a \TT{*} at the beginning of the subexpression containing them.
\item[(4)] Lastly nested subexpressions beginning with \TT{*} are
collapsed: e.g.,
\begin{center}
