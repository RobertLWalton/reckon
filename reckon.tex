...
to a preceding {em numeric} lexeme from which they will be automatically
split.  Thus `\TT{5inch}' and `\TT{5 inch}' are equivalent.

TBD


Units can be combined in a {\em unit-expression}
by the operators `\TT{*}' for multiplication,
`\TT{\textasciicircum}' for exponentiation, `\TT{/}' for division,
`\TT{+}' for addition, and `\TT{-}' for subtraction:
\begin{indpar}
...
\emkey{unit-product}
    \begin{tabular}[t]{rl}
    :::= & {\em unit-factor} \\
     $|$ & {\em unit-number-constant} {\em unit-factor} \\
...
\emkey{unit-factor}
    \begin{tabular}[t]{rl}
    :::= & {\em unit-name} \\
     $|$ & {\em unit-name} \TT{\textasciicircum} {\em unit-number-constant} \\
    \end{tabular}
\\
...
{\em Unit-expressions} are single lexemes.
The second operand of `\TT{\textasciicircum}', `\TT{+}', and
`\TT{-}' must always be a {\em unit-number-constant}.  The `\TT{*}'
may be omitted between a {\em unit-number-constant} and a following
{\em unit-factor}.  Note that its is not possible to write factional
exponents.
...
define unit ft = 12inch
define unit yd = 3ft
define unit inch = yd/36
define unit inch = ft/36
define unit sqft = ft^3
define unit cuft = ft^3
\end{verbatim}\end{indpar}

A unit may appear in many such statements in order to describe
its relation to many other units.  The statements must be consistent;
e.g.,
\begin{center}
\tt inch = yd/36 = (3ft)/36 = 3*(12inch)/36 = (36*inch)/36 = 1inch
\end{center}
Equations will be automatically derived from these statements
when such can be done by linear algebra and simple substitution; e.g.
`{\tt yd = 36inch}' and `{\tt sqft = 144inch\textasciicircum 2}'
will be derived.
