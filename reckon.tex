...
%	APE	A Programming Envioronment
...
\key{name} ::=
	\begin{tabular}[t]{@{}l@{}}
	   ...
	   $|$ {\em decimal-natural} \}$^\star$ \\
        ...
        \end{tabular}
\\[1ex]
\key{non-special-symbol} ::= {\em symbol} that is \underline{not}
				specially treated by the parser
\\[1ex]
\key{decimal-natural} ::= {\em number-lexeme} that contains only
...
\key{indentation-separator} ::= see text \\[1ex]
\key{indentation-end} ::= see text
\end{indpar}
...
{\em Indented-subexpressions} are introduced by {\em indentation-marks}
(\secref{INDENTATION-MARKS}), which are lexemes.
Lines in this list are separated by implicit
\skey{indentation-separator}s that are placed at the beginning of each
line but the first.  An \key{indentation-end} is place after
all the lines in an {\em indented-subexpression}.
Although {\em indentation-marks} are lexemes,
{\em indentation-separators}
and {\em indentation-ends} are not lexemes.
...
these can be replaced by indentation using the \verb/:|/ mark according
to a general rule given in \secref{INDENTATION-EQUIVALENCE-RULE}.  Roughly,
...
\begin{tabular}{@{}p{1.7in}@{\hspace*{0.3in}}p{4.3in}@{}}
    ...
    \end{tabular}
    & Explicit brackets are used to group lexemes into subexpressions.
\end{tabular} \\[2ex]
\begin{tabular}{@{}p{1.7in}@{\hspace*{0.3in}}p{4.3in}@{}}
    \begin{tabular}[t]{@{}l@{}}
    Lexeme Replacement\\
...
At the top level, the parser reads and parses one top level line
...
For example, at any given time the topmost current expression parser
definition is used to define the current expression parser.  The
following two expression parser definitions are standardly in the
...
The \verb|+| op adds a selector if it was no previously present.
...
\begin{indpar}\begin{verbatim}
undefine operator "-" [operator]
\end{verbatim}\end{indpar}

further toward the top of the stack will cause the lexeme
`\verb|-|' to be not recognized as an operator.
...
implicit brackets introduced by numbers are \verb|(# #)|.
...
Bracket, indentation mark, operator, sentence terminator, and section
...
\label{BRACKET-NAME}
	\\[1ex]
\key{opening-bracket-name} ::= {\em name} \\[1ex]
\key{closing-bracket-name} ::= {\em name} \\[1ex]
...
\begin{indpar}
\key{indentation-mark-definition} \\
...
\label{INDENTATION-MARK-DEFINITION}
	    \\[1ex]
\key{indentation-mark-name} ::=
	{\em mark-name} $|$ {\em gluing-indicator} {\em mark-name}
\label{INDENTATION-MARK-NAME}
	\\[1ex]
\key{mark-name} ::= {\em mark} $|$ {\em quoted-string}
	\\[1ex]
\key{gluing-indicator} ::= \verb|*|
	\\[1ex]
{\em parsing-selectors} ::= see \pagref{PARSING-SELECTORS}
	\\[1ex]
{\em new-parsing-selectors} ::= see \pagref{NEW-PARSING-SELECTORS}
...
\skey{indented subexpression}s.\label{INDENTED-SUBEXPRESSION}
...
\key{indentation mark} that ends a line.  This mark must
...
\key{indentation-separator} and \key{indentation-end} are used.
An {\em indentation-separator}
occurs just before each line but the first in an indented subexpression.
An {\em indentation-end} occurs at the end of the indented subexpression,
...
The \key{top level},\label{TOP-LEVEL} the file containing the text being parsed,
...
\key{lexeme-dictionary-entry} \\
	...
	\\[1ex]
\key{replaced-lexemes} ::= {\em name}
	\\[1ex]
\key{replacing-lexemes} ::= {\em empty} $|$ {\em name }
	\\[1ex]
...
\key{number-unit-pair} \begin{tabular}[t]{rl}
		     ...
		     \end{tabular} \\[1ex]
\key{prefix-unit-specifier} ::= {\em name} \\[1ex]
\key{postfix-unit-specifier} ::= {\em name} \\[1ex]
\key{unit-specifier} ::= {\em prefix-unit-specifier} $|$
                         {\em postfix-unit-specifier} \\[1ex]
\key{name} ::= see \pagref{NAME}
\end{indpar}
...
a subexpression returns a subpexpression reformatted according to the
...
recognition have all been done, and the subexpression is a sequence
...
\begin{indpar}
\key{expression-parser-definition} ::=\\
\hspace*{0.5in}\verb|define expression parser| {\em parser-name}
				               {\em parsing-selectors} \\[1ex]
\key{parser-name} ::= {\em name} \\[1ex]
\key{name} ::= see \pagref{NAME} \\[1ex]
{\em parsing-selectors} ::= see \pagref{PARSING-SELECTORS}
\end{indpar}
...
\key{operator-definition} \\
	\hspace*{0.5in}::= \ttkey{define operator}
	    ...
	    \end{tabular}
	    \\[1ex]
\key{operator-name} \begin{tabular}[t]{rl}
...
\key{operator-flag} ::= \ttmkey{prefix}{operator flag}
                    $|$ \ttmkey{infix}{operator flag}
                    $|$ \ttmkey{postfix}{operator flag}
                    $|$ \ttmkey{nofix}{operator flag}
                    $|$ \ttmkey{afix}{operator flag}
	\\[1ex]
\key{precedence} ::= {\em sign-option} {\em decimal-integer}
	\\[1ex]
\key{sign-option} ::= {\em empty} $|$ \verb|+| $|$ \verb|-|
	\\[1ex]
\key{reformatter-name} ::= {\em function-name }
	\\[1ex]
\key{function-name} ::= {\em name}
...
\begin{indpar}
\key{temporary-definition} ::= {\em temporary} {\em subexpression} \\[1ex]
\key{temporary} ::= \verb|$| {\em decimal-natural} \\[1ex]
\key{decimal-natural} ::= see \pagref{DECIMAL-NATURAL}
\end{indpar}
...
With this in mind, the reformatters are:
...
returning
\begin{indpar}\begin{verbatim}
@3 = x y { .initiator = "("; .terminator = ")"; .separator = "," }
\end{verbatim}\end{indpar}
...
This just moves the \verb|<--| to the front of the expression:
...
Indented subexpressions are parsed like final \verb|{ }|
...
\begin{indpar}[0.5em]
If there is only one operator and it is at the beginning of the subexpression,
then the subexpression is not changed.

Otherwise the expression cannot have both {\tt AND} and {\tt OR} operators,
the {\tt NOT} operator
can only occur at the beginning of an expression or just after {\tt AND}
or {\tt OR}, and subexpressions cannot be empty, except for subexpressions
before a {\tt NOT}, which must always be empty.

In this case the expression is rewritten using a multi-argument {\tt AND}
or {\tt OR} function and a single argument {\tt NOT} function:

\hspace*{2em}\begin{tabular}{lcl}
\verb|NOT x| & $\Longrightarrow$ & \verb|NOT x| \\
\verb|NOT x OR NOT y OR z| & $\Longrightarrow$ & \verb|OR (NOT x) (NOT y) z| \\
...
\item
If the current operator fixity is prefix, infix, or nofix, then a
...
equations, and guards.  Syntax equations involve syntactic
variables, which are names beginning with a capital letter or with
a \verb|$|.  Guards are just expressions that must evaluate to
\verb|true| in order for the match to complete.  The syntax equations
are solved during the matching process, to produce values for the
syntactic variables.  If the syntax equations are inconsistent and
cannot be solved, the match fails.  Evaluation of the guards in a definition
may result
in additional matches which generate more syntax equations that
involve the syntactic variables in the guarded definition.
...
A definition may have default values for a syntactic variable.  If
...
default values for their corresponding syntactic variables.

There are two kinds of syntactic variable.  Those beginning with a
\verb|$| are set equal to some unevaluated expression, and are called
unevaluated variables.  Those beginning with
a capital letter are set equal to the value of an expression,
and are called evaluated variables.  For example, if `\verb|foo(2+2)|'
is matched to the pattern `\verb|foo($x)|', then \verb|$x :=: 2+2|,
but if it is matched to the pattern `\verb|foo(X)|', then \verb|X :=: 4|,
where \verb|:=:| means `is syntactically equal to'.
...
matching operations, guard evaluations, and evaluated variable expression
evaluations does not matter.

In order to make the matching process monotonic, pattern matching must
be monotonic.  This means that if undefined syntactic variables in a matched
...
executed during the matching process must not have side effects.

Lastly, expressions assigned to evaluated syntactic variables
must be evaluated monotonically in order to give these variables values.
In order to ensure this, these expressions are not evaluated until
all syntactic variables they contain have been given values, so that
the expression to be evaluated does not contain any undefined syntactic
variables.  Furthermore, evaluation of these expressions may not have
side effects.  Again this last means that any blocks executed during
these evaluations must not have side effects.  If an evaluated variable
is assigned an expression containing a syntactic variable that never
becomes defined, then the entire matching process fails, just as it
...
\key{expression-definition} ::=
	...
	\\[1ex]
\key{pattern} ::= {\em pattern-term-option}
		  \{ \ttmkey{.}{in pattern} {\em pattern-term} \}$^\star$
		  \\[1ex]
\key{pattern-term-option} ::= {\em empty} $|$ {\em pattern-term} \\[1ex]
\key{pattern-term} \begin{tabular}[t]{rl}
            ::= & {\em expression} \\
            ::= & {\em expression} \ttmkey{?}{in pattern} \\
            ::= & {\em expression} \ttmkey{...}{in pattern} \\
            ::= & {\em expression} \ttmkey{...?}{in pattern} \\
	    \end{tabular} \\[1ex]
...
\key{block} \begin{tabular}[t]{rl}
            ::= & \verb|{| {\em block-header}$^\star$ {\em statement}
                           \{ \verb|;| {\em statement} \}\,$^\star$ \verb|}| \\
	    $|$ & indented-subexpression introduced by the marker `\verb|* :|'
	    \end{tabular} \\[1ex]
\key{block-header} \begin{tabular}[t]{rl}
            ::= & \ttkey{default}
	    ...
	    \end{tabular} \\[1ex]
\key{default-equation} ::= {\em syntactic-variable}
		\ttmnbkey{:=:}{in {\em default-equation}}
		{\em expression} \\[1ex]
\key{constraint-equation} ::= {\em expression}
		\ttmnbkey{:=:}{in {\em constraint-equation}}
		{\em expression} \\[1ex]
\key{expression} ::= {\em expression-element}$^\star$
		     \\[1ex]
\key{expression-element}
    ::= {\em word} $|$ {\em mark} $|$ {\em number} $|$ {\em quoted-string}
    	$|$ {\em subexpression} \\[1ex]
\key{subexpression} \begin{tabular}[t]{rl}
    ::= & {\em opening-bracket} {\em expression}
          {\em closing-bracket } \\
    $|$ & {\em opening-bracket} {\em expression}
          \{ \verb|,| {\em expression} \}$^\star$
          {\em closing-bracket } \\
    $|$ & {\em opening-bracket} {\em expression}
          \{ \verb|;| {\em expression} \}$^\star$
          {\em closing-bracket } \\
