...
Later we will see that because of this expression heads can be
expression graph variable values, but qualifier heads cannot be.
...
are indicated by a particular graph variable operations which
...
as graph variable names, as if they were preceded implicitly by `\verb|..::|'.
...
and specifies that the default value of the expression graph variable is
...
same graph variable.  If this happens, all the subexpressions represent the
same node, and at most one of the subexpressions may have either a value
or default value for the variable.  That is, at most one of the subexpressions
