...
See \itemref{POSSESSIVES-PARSER-PASS} for details.
...
\begin{indpar}
\emkey{fractional-number} ::= {\em sign}\QMARK{} {\em fractional-numeric}
\\[1ex]
\emkey{sign} ::= \TT{+} $|$ \TT{-}
\\[1ex]
\emkey{fractional-numeric} :::= \{ {\em integral-numeric} \TT{.} \}\QMARK{}
	{\em digit}\PLUS{} \TT{/} {\em digit}\PLUS{}
\\[1ex]
...
The unusual use of `\TT{.}' permits an integer followed by a fraction,
as in `\TT{42~3/8}', to represent two distinct numbers in a list,
whereas `\TT{42.3/8}' with the `\TT{.}' represents just one number,
namely the sum of 42 and 3/8.
However in situations where the units parser pass
(see \itemref{UNITS-PARSER-PASS}) is used on the
subexpression `\TT{42~3/8}', this subexpression is recognized as
representing the single number `\TT{42.3/8}'.
...
\emkey{sign} ::= \TT{+} $|$ \TT{-} \label{SIGN} \\[1ex]
...
\emkey{exponent} ::= {\em exponent-indicator} {\em sign}\QMARK{}
                     {\em integral-numeric} \label{EXPONENT} \\[1ex] 
...
\emkey{concatenated-quoted-string} ::=
    ...
    \label{CONCATENATED-QUOTED-STRING}
\end{indpar}

\subsection{Radix Numbers}
\label{RADIX-NUMBERS}

A \key{radix number} consists of an optional sign, a radix-indicator
followed by a \TT{\#},
a sequence of digits with an optional point and optional commas
inside a concatenated quoted string, and an optional exponent.

\begin{indpar}
\emkey{radix-number} ::=
    {\em sign}\QMARK{} {\em radix-indicator} \TT{\#}
    {\em concatenated-quoted-string} {\em exponent}\QMARK{}
\\[1ex]
\emkey{sign} ::= see \pagref{SIGN}
\\[1ex]
\emkey{radix-indicator} ::= \TT{B} $|$ \TT{O} $|$ \TT{D} $|$ \TT{X}
\\[1ex]
\emkey{concatenated-quoted-string} ::= see \pagref{CONCATENATED-QUOTED-STRING}
\\[1ex]
\emkey{exponent} ::= see \pagref{EXPONENT}
\end{indpar}
...
\\[1ex]
\TT{B}	& binary	& \TT{0}, \TT{1}	& every 3 or 4 digits \\
\TT{O}	& octal		& \TT{0}, \TT{1}, \TT{2}, \TT{3},
			  \TT{4}, \TT{5}, \TT{6}, \TT{7}
			  			& every 3 or 4 digits \\
\TT{D}	& decimal	& \TT{0}, \TT{1}, \TT{2}, \TT{3},
			  \TT{4}, \TT{5}, \TT{6}, \TT{7}
			  \TT{8}, \TT{9}	& every 3 digits \\
\TT{X}	& hexa-decimal	& \TT{0}, \TT{1}, \TT{2}, \TT{3},
			  \TT{4}, \TT{5}, \TT{6}, \TT{7}
			  \TT{8}, \TT{9},	& every 4 digits \\
	&		& \TT{A}, \TT{B}, \TT{C}, \TT{D},
			  \TT{E}, \TT{F}, \\
	&		& \TT{a}, \TT{b}, \TT{c}, \TT{d},
			  \TT{e}, \TT{f} \\
...
B\#"101101"E2	& O\#".7753"   & D\#"97.58"	& X\#".fe8a"e-4 \\
B\#"10,1101"E+2	& O\#"2,3456"  & D\#".123,5"	& X\#"E8A.7CCD,83" \\
B\#"101,101"E2	& O\#"0.2345,6"  & D\#".123,5"	& X\#"E8A,932B.7C" \\
\end{tabular}
\end{indpar}

There may be at most one point.  If commas are used, the number
of digits between successive commas or between a point and its
neighboring comma must be the same, and must be allowed.  However,
commas may be used before the point without being used after the
point, and vice versa.

Just as for {\em scientific-numbers}\pagnote{SCIENTIFIC-NUMBERS}
the {\em exponent} shifts the decimal point right (if +) or left (if -)
by some number of digits (in the given radix).

Characters in the {\em concatenated-quoted-string} other than allowed
digits, a single point, and commas in the correct positions
are ignored (if there is more than one point only the first is not
ignored) and a warning or error message is issued.
...
\emlkey{attribute-representation}{-subparagraph}
    ::= \TT{\{:} {\em attribute-representation-list} \TT{:\}}
\end{tabular}

\begin{tabular}{l}
\emlkey{attribute-representation}{-list}
\\\hspace*{0.5in}
    \begin{tabular}[t]{rl}
    ::= & {\em empty} \\
...
All {\em atom-names} in a raw object representation are dequoted.  Thus
\TT{"X"} and \TT{X} represent the same symbol atom,
whereas \TT{0123} represents the number atom $123$ while \TT{"0123"}
represents the symbol atom \TT{0123}.
...
each stage of the scan.  After the atoms are scanned in this manner,
they are dequoted (symbols that begin and end with \TT{"} have
these characters removed: see page \pagref{DEQUOTE}).
Thus the {\em object-list}
\begin{center}
\tt 5 e + 2 e 3 4 "e" 3 @ 7 e 5
\end{center}
translates as (see \itemref{SCIENTIFIC-NUMBERS}):
\begin{center}
\tt
...
"5" 5 "6" \# "7" \# "8" "\#" "9"
\end{center}
translates as (see \itemref{CONCATENATED-QUOTED-STRINGS}):
\begin{center}
\tt
\begin{tabular}{r@{~~$\Longrightarrow$~~}rl}
...
\end{center}
translates as (see \itemref{RADIX-NUMBERS}):
\begin{center}
\tt
...
dequoted {\em w-type}.  An exception is that a list with {\em w-type} \TT{""}
(the empty {\em quoted-string}) or {\em m-type} `\TT{.}'
is not given any \TT{.type} attribute at all.  Thus `\TT{\{. .\}}'
denotes the empty list with no attributes at all,
but this can also be abbreviated by `\TT{\{\}}'.
This empty list with no attibutes can also be represented more
awkwardly by `\TT{\{""||\}}'.
...
\subsection{Operator Parser Pass}
\label{OPERATOR-PARSER-PASS}

The \key{operator parser pass} identifies operators in a subexpression
...
\subsection{Possessives Parser Pass}
\label{POSSESSIVES-PARSER-PASS}

In computable expressions the phrase `\TT{weight of X}' and
`\TT{X's weight}' are equivalent.  Both are parsed by the
\key{possessives parser pass} which reformats them as
`\TT{.~weight X}'.

The following are standard possessives:
...
This parser first inspects all the {\em words} of the subexpression
and splits any {\em word}-ending possessive suffix off from its
{\em word}.  Thus the subexpression `\TT{weight of george's wife}'
becomes `\TT{weight of george 's wife}'.

After doing this the possessives parser pass checks to see if the
resulting subexpression has any possessive prepositions or
suffixes and also has the syntax:

\begin{tabular}{rcl}
\hspace*{1.7in} \\[-2ex]
\emkey{possessive-subexpression}
    & ::= & \begin{tabular}[t]{l}
            {\em possessive-prepositional-prefix}\QMARK{} \\
            {\em possessive-suffix-prefix}\QMARK{} \\
            {\em non-possessive-name}
	    \end{tabular}
\\[1ex]
\emkey{non-possessive-name} & ::= & \begin{tabular}[t]{l}
                          {\em name}\pagnote{NAME} that does not contain
			  any \\ possessive prefixes or prepositions
			  \end{tabular}
\\[1ex]
\emkey{possessive-prepositional-prefix} & ::= &
    {\em non-possessive-name}  {\em possessive-preposition}
\\[1ex]
\emkey{possessive-suffix-prefix} & ::= &
    {\em non-possessive-name}  {\em possessive-suffix}
\end{tabular}

If the subexpression does not have any possessive prepositions or
suffixes, or does not have the syntax of a {\em possessive-subexpression},
the possessives parser pass leaves the subexpression unchanged.

Otherwise the pass reformats the subexpression by recursively applying
first the rewrite:
\begin{center}
$n$ {\em possessive-preposition} $e$ $\Longrightarrow$
    \TT{.} $n$ $e'$
\end{center}
and then the rewrite rule:
\begin{center}
$e$ {\em possessive-suffix} $n$ $\Longrightarrow$
    \TT{.} $n$ $e'$
\end{center}
where
\begin{center}
$n$ is a {\em non-possessive-name} \\
$e$ is a {\em possessive-subexpression} \\
$e'$ is \TT{\{.} $e$ \TT{.\}} if $e$ has more than one element \\
$e'$ is $e$ if $e$ has just one element
\end{center}

Thus
\begin{center}
\TT{X of Y} $\Longrightarrow$ \TT{.~X Y}
\\
\TT{X of Y 's Z} $\Longrightarrow$
    \TT{.~X \{.~Y 's Z .\}} $\Longrightarrow$
    \TT{.~X \{.~.~Z Y .\}}
\\
\TT{X' Y} $\Longrightarrow$ \TT{.~Y X}
\\
\TT{X 's Y 's Z} $\Longrightarrow$
    \TT{.~Z \{.~X 's Y .\}} $\Longrightarrow$
    \TT{.~Z \{.~.~Y X .\}}
\\
\TT{X of Y 's Z} $\Longrightarrow$
    \TT{.~X \{.~Y 's Z .\}} $\Longrightarrow$
...
\begin{center}
\TT{X of Y 's Z} $\Longrightarrow$
    \TT{.~Z \{.~X of Y .\}} $\Longrightarrow$
    \TT{.~Z \{.~.~Y X .\}}
\\
WRONG ORDER OF RULE APPLICATION!
\end{center}

\subsection{Units Parser Pass}
\label{UNITS-PARSER-PASS}
...
\subsection{Dequoting Parser Pass}
\label{DEQUOTING-PARSER-PASS}
