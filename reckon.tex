...
\tt 7ft	& numeric	& \tt This & word \\
\tt 1	& numeric	& \tt is & word \\
\tt 3/4	& numeric	& \tt ( & (strict) separator \\
\tt in	& unit		& \tt we & word \\
...
\emkey{separator}
    ...
    \end{tabular}
\\[0.5ex]
\emkey{symbol} :::= {\em middle-lexeme} $|$ {\em separator}
                $|$ {\em quoted-string}
\\[1ex]
\emkey{comment} :::=
...
{\em Middle-lexemes}, {\em separators}, and {\em quoted-strings} are 
represent as \skey{symbol}s\label{SYMBOL} which store character strings.
...
\emkey{word} ::=
    ...
    \begin{tabular}[t]{rll}
        & (1) & contains a letter \\
    and & (2) & contains only letters, digits
    and the characters: \TT{`} \TT{'} \TT{.} \TT{-} \TT{\_} \\
    and & (3) & does not have any digit before its first letter \\
    and & (4) & does not begin with \TT{`} or end with \TT{'~.~-~\_} \\
    \end{tabular}
    \end{tabular}
\\[1ex]
\emkey{index} ::=
    ...
    {\em middle-lexeme} that \\
    \begin{tabular}[t]{rll}
        & (1) & contains only digits and the characters: \TT{.~-} \\
    and & (2) & surrounds every \TT{-} by digits \\
    and & (3) & follows every \TT{.} by a digit \\
    \end{tabular}
    \end{tabular}
\\[1ex]
\emkey{numeric} ::=
    ...
    {\em middle-lexeme} that \\
    \begin{tabular}[t]{rll}
    either & (1) & contains a digit not preceeded by a letter \\
    or     & (2) & contains a \TT{\#} and either a letter or a digit
    \end{tabular}
    \end{tabular}
\\[1ex]
...
There are many special kinds of {\em numerics} and {\em marks}:
\begin{indpar}
    \begin{tabular}[t]{lrl@{~~~~~}ll}
  &     & & Section & Example \\[1ex]
\multicolumn{5}{l}{\emkey{special-numeric}~~::=} \\
~~~~
  &     & {\em natural-number}
        & \itemref{NATURAL-NUMBERS}
        & \TT{43} \\
  & $|$ & {\em fractional-number}
        & \itemref{FRACTIONAL-NUMBERS}
        & \TT{4/3} \\
  & $|$ & {\em scientific-number}
        & \itemref{SCIENTIFIC-NUMBERS}
        & \TT{4.3} \\
  & $|$ & {\em date}
        & \itemref{DATES}
        & \TT{4may1903} \\
  & $|$ & {\em time}
        ...
        & \TT{4:30} \\
  & $|$ & {\em radix-number}
        & \itemref{RADIX-NUMBERS}
        & \TT{x\#43}
\\[1ex]
\multicolumn{5}{l}{\emkey{special-mark}~~::=} \\
~~~~
  &     & {\em unit}
        & \itemref{UNITS}
        & \TT{mi/hr} \\
...
{\em Words}, however, are never
special, and can be used to form names of variables, constants, and functions,
which are called `\skey{defined names}':

\begin{indpar}
\emkey{defined-name} ::=
    \{ {\em word} $|$ {\em quoted-string} \}
    \{ {\em word} $|$ {\em quoted-string} $|$ {\em index} \}\STAR{}
\end{indpar}

Some {\em defined-names} have builtin definitions, such as the
...
{\em Indices} are also {\em numerics}.
They may, according to context, be used
as non-initial components of names, or if they contain at most one
`\TT{.}', be used as special {\em numeric} lexemes denoting numbers.
Thus in
...
`\TT{1}', `\TT{3.3}', and `\TT{.5}' are used as {\em indices}
in {\em defined-names}, while `\TT{2}', `\TT{4.4}', and `\TT{.6}'
are used to denote numbers.

{\em Quoted-strings} are used as if they were {\em words}.
For example:
...
defines a variable named \TT{"+"} with value \TT{3}, a variable
name \TT{X} with value \TT{4}, and
a variable named \TT{Y} with value being the text \TT{X + 3},
i.e., the vector whose 3 elements are the symbols
\TT{X}, \TT{+}, and \TT{3}.  \TT{Z} is a variable whose
...
Special lexemes are described in the following sections.
...
(see \itemref{PARSER-UNITS-PASS}) is used on the
...
\\\hspace*{0.5in}
	...
	{\em single-attribute-value}\STAR{}
	\end{tabular}
\end{tabular}
