...
involve the syntax variables in the definitions that match the
guards.
...
denoted by the variable name with preceding \verb|$|.
...
\key{pattern-term} \begin{tabular}[t]{rl}
            ::= & {\em expression} {\em expression}$^\star$ \\
            ::= & {\em expression} {\em expression}$^\star$
	    		\ttmkey{?}{in pattern} \\
            ::= & {\em expression} {\em expression}$^\star$
	    		\ttmkey{...}{in pattern} \\
...
\key{block} ::= \verb|{| {\em block-header}$^\star$ {\em statement}
...
\key{block-header} \begin{tabular}[t]{rl}
	    ...
	    \end{tabular} \\[1ex]
\key{default-equation} ::= {\em syntax-variable}
