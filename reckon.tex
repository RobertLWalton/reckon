...
A line can be \mkey{continued}{line} on the next line by giving that next line
...
line, in which case they are indicated by a announcing the beginning
of the non-continuation line by two \TT{<\$\$>} lexemes instead of just
one.  Thus
...
`\ttkey{<\$BEGIN\$>}' and also specifies that the next non-blank
...
the lexeme squence `\ttkey{<\$END\$>}' is inserted just after
the end of the last lexeme in the last subparagraph line (or
just after \TT{<\$BEGIN\$>} if the subparagraph is empty).
Thus
...
\begin{indpar}\begin{verbatim}
<$$> This is a line
<$ BEGIN $> <$$> And a subparagraph line.
<$$> And another.  <$ END $>
<$$> <$$> And more.
\end{verbatim}\end{indpar}

\TT{<\$END\$>} sequences are added at the end of a file to
match any unmatched \TT{<\$BEGIN\$>} sequences from subparagraphs
...
data is to be automatically written to a file.
...
\\\hspace*{0.5in}
	...
	$|$ & \TT{\{|}
	      {\em raw-attribute-representation}
	      \{ \TT{;} {\em raw-attribute-representation} \}$^\star$
	      \TT{|\}} \\
	$|$ & \begin{tabular}[t]{@{}l@{}}
	      \TT{|:}{\em -indentation-mark} \\
	      ~~~~{\em indentation-beginning}
	          {\em raw-attribute-representation} \\
	      ~~~~\{ {\em indentation-separator}
	         {\em raw-attribute-representation} \}$^\star$ \\
...
\emkey{raw-atom} ::= {\em atom}\pagnote{ATOM} ~
                 but not {\em separator}\pagnote{SEPARATOR} ~or~
		 \TT{|} ~or~ \TT{=} ~or~ \TT{|:}
\end{tabular}
...
Their special syntactic marker is the \TT{=|} sign.
They can
use either \TT{\{|~~|\}} brackets and \TT{;} separators, or
the \TT{|:} indentation mark and {\em indentation-separators}.
To replace \TT{\{|  |\}} and \TT{;} by indentation,
indent appropriately placing the \TT{\{|}, \TT{|\}}, and \TT{;}
brackets and separators at line ends, replace \TT{\{|} by \TT{|:},
and delete the \TT{|\}}'s and \TT{;}'s.
A \TT{=|~~|:} sequence at a line end can be abbreviated by \TT{=|:},
and similarly \TT{>|~~|:} and \TT{>{}>|~~|:} can be abbreviated
by \TT{>|:} and \TT{>{}>|:}.
For example,
...
Note lastly that the \TT{\{| |\}} brackets, which are used in object
representation, are distinct from the \TT{\{ \}} brackets, which are
...
in the \TT{[ ]} brackets.  If \TT{]} is used as one of
these {\em atoms} is must be quoted as in \TT{"]"}.
...
\TT{\{| |\}} bracketed and
\TT{;} separated {\em raw-attribute-representations}, or alternatively by
\TT{|:} and then by indented
{\em raw-attribute-representations}.
