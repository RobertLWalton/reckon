...
{\em bracketed-list} with an \TT{.initiator}.
...
the syntax expression (left side) is an identical atom, or
\item if the unfolded pattern is an object, the syntax expression (left side)
...
this sequence is made into a \TT{(~)} bracketed list
\end{enumerate}
\item the object and the pattern have
