...
\ikey{Lexemes}{lexeme} are defined as follows:
...
\key{separator-character} \begin{tabular}[t]{rl}
		::= &   \verb|(|
	       	    ...
	       	    $|$ \verb|;|
	       	    $|$ \verb|`| \\
		$|$ & \verb|'| not followed by a {\em letter} \\
...
which are always followed by a digit, and commas that are
both preceeded and followed by digits.  A quoted string is
...
and a separator otherwise.  A \verb|.| is a number character
(decimal point) if it
is followed by a digit, and a mark character otherwise.  A
\ttnbkey{,} is a number character if surrounded by digits, and
...
must be quoted (e.g., \verb|","|), and to protect a symbol from
...
Indentation Lexemes (\secref{INDENTATION-LEXEMES}) \\
Radix Numbers (\secref{RADIX-NUMBERS}) \\
Scientific Numbers (\secref{SCIENTIFIC-NUMBERS}) \\
...
algorithm is the input to the indentation lexeme lexical parsing
...
process replaces one lexeme by a string of zero or
...
replace lexemes at the beginning of a lexeme sequence, the definition
...
0 indentation, 0 bracket nesting depth, a pointer to the bracket definition
given above at the beginning of this section,
...
are referred to as `current' (e.g., `current indentation', `current
indentation opening bracket').

Indentation lexemes are then inserted during lexical parsing
by the following \key{indentation lexeme algorithm}.  This algorithm is 
...
2 columns, and cannot be just 1 column.
\end{indpar}
...
ends a lexeme that matches the indentation indicator of some
...
then the indentation indicator is removed from the end of this lexeme
and a new indentation record is pushed into the stack.

If the lexeme from which the indicator as removed becomes empty, it is
deleted from the lexeme stream.

The indentation of the new indentation record is the indentation of
the next lexeme (which is the indentation of the first non-white-space
character on the next non-blank line).
The implicit bracket flag is on.  The record
points at the bracket definition containing the indentation indicator.
\end{indpar}
...
The following are example inputs and outputs from the indentation lexical
parsing algorithm.
...
which is applied to the output of the indentation lexeme algorithm. 
...
be arithmetic operators (except for exponent signs).
...
the decimal point must be followed by
...
To be legal, a decimal number must contain at most one decimal point.
...
be 3 or 4, depending on the {\em radix-indicator}.
...
\tt
\begin{tabular}{l@{~~~~~}l@{~~~~~}l@{~~~~~}l}
B"5,7b63"	& O"2/3456"	& D"-0.1"	& X".E8A932B.7CCD"
\end{tabular}
\end{indpar}
...
{\em parsing-selectors} ::= see \pagref{PARSING-SELECTORS}
	\\[1ex]
\key{function-name} ::= {\em symbol} {\em symbol}\,$^\star$
\end{indpar}
...
When a {\em scientific} is encountered during number lexical parsing,
it is surrounded by \verb|(# #)| implicit brackets.  Thus if
...
\subsubsubsection{Number Unit Grouping}
...
For example, `\verb|7 hr 30 min|' is a {\em number-unit-group} containing
2 {\em number-unit-pairs} that number unit grouping transforms into
`\verb/(# 7 hr + 30 min #)/', where \verb/(# #)/ are implied brackets
(\pagref{IMPLIED-BRACKETS}).

Number unit groups are recognized after scientific numbers have been
recognized.

\subsubsubsection{Unit Multiplier Insertion}
...
Unit multiplier insertion is done after number unit groups have been
recognized.
