...
For example, `\verb|<p>|' is a paragraph initiator,
and `\verb|<indented p>|' is paragraph initiator that is a child of `\verb|<p>|'
and inherits attributes from `\verb|<p>|'.  As another example,
`\verb/<b|/ \ldots{} \verb/|b>/' are the text parser `bold' brackets.

The named text initiators, separators, terminators, and brackets
have the syntax:

\begin{indpar}
\emkey{named-initiator} ::= \verb|<| {\em name} \verb|>| \\[1ex]
\emkey{named-separator} ::= \verb|<| {\em name} \verb|>| \\[1ex]
\emkey{named-terminator} ::= \verb|<| {\em name} \verb|>| \\[1ex]
\emkey{named-left-bracket} ::= \verb|<| {\em name} \verb/|/ \\[1ex]
\emkey{named-right-bracket} ::= \verb/|/ {\em name} \verb|>| \\[1ex]
{\em name} ::= see \pagref{NAME}
\end{indpar}
...
lexemes and sublists.  The text marks end up as {\tt .initiator},
...
and the text mark names.  The attributes of the text mark names
