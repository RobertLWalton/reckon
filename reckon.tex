...
are placed in the parsing definition stack.  The following is a list
...
A parsing definition contains a \key{parsing selector specification}
which lists the parsing selectors attached to the definition.
Some definitions, like bracket definitions, can
contain a \key{new parsing selector specification},
that determines a parsing selector set to push into the
parsing selector set stack.  These have the syntax:

\begin{indpar}

\begin{tabular}{l}
\emkey{parsing-selector-specification} ::= {\em empty}
			     $|$ \verb|[| {\em parsing-selector-list} \verb|]|
\label{PARSING-SELECTOR-SPECIFICATION}
\\[1ex]
...
\begin{tabular}{l}
\emkey{new-parsing-selector-specification} \\
\hspace*{0.3in}\begin{tabular}[t]{rl}
    ...
    $|$ & \verb|[| {\em parsing-selector-op} {\em parsing-selector} \\
	  ...
	  \}$^\star$ \verb|]|
    \end{tabular}
\label{NEW-PARSING-SELECTOR-SPECIFICATION}
\\[1ex]
...
If a parsing definition does not give any \emkey{parsing selector
specification} (i.e., the specification is completely {\em empty}
in the above syntax, with no \verb|[ ]| brackets present), then
the definition is \underline{always} current.  Otherwise the
{\em parsing selectors} listed in the specification are attached to the
definition.

If a definition like a bracket definition gives a \emkey{new parsing
selector specification}, then it contains instructions how to make
...
The \verb|+| op adds the selector if it was no previously present.
The \verb|-| op deletes the selector if it was previously present.
And the \verb|^| op flips the selector, adding it if it was not
...
give any {\em new parsing selector specification}, then
...
Brackets surround a subexpression and specify the parsing
...
Note that expression parsing, which is done after parsing,
can transform the results of parsing significantly.  Thus
...
new current line list.  Then this loop iteration is terminated, without
...
TBD change next paragraph

Note that when a subexpression record is pushed, the bracket or
...
parsing, so these other steps operate on the lexeme sequence
...
\key{expression-parser-definition}
	::= \verb|define expression parser| {\em parser-name}
				            {\em parsing-selectors} \\[1ex]
...
\begin{indpar}\begin{verbatim}
define expression parser -OPERATOR-PARSER- [operator,list]
define expression parser -TEXT-PARSER- [text]
\end{verbatim}\end{indpar}

The difference between these two expression parsers is that only the operator
...
An expression parser is a function that is called with a list of lexemes and
returns a parsed expression.  At any time there can be only one
current expression parser definition, and that is used to select the current
parser.  There are two standard expression parsers:
...
definitions.
Both operator and text parsers recognize brackets and do
parsing, but all standard
parsing definitions do not have the `text' parsing selector,
...
\begin{labpar}[0.1in]{{\bf prefix}, {\bf infix}, {\bf postfix}, {\bf nofix}}%
    ...
    \index{nofix flag@{\tt nofix} flag}\label{NOFIX}
...
the parsing stack.
See \pagref{PARSING-DEFINITION-KINDS} for a list of the kinds of
