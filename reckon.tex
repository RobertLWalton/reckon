...
Some subexpression bracketing matching lexemes affect parsing:
...
\verb|<<>>|  & \verb/|/, text		& eval, operators, qualifiers,
					  \verb|<=>|, \verb|@@@| \\
\verb|`'|  & eval, operators, qualifiers, \verb|<=>|, \verb|@@@|
					& \verb/|/, text \\
\end{tabular}
\end{center}
...
There are three kinds of \skey{parsing context}s:
one where only the reorder mark (\verb|<=>|)
and the qualifier mark (\verb|@@@|) are recognized, one where these
are recognized along with operators and qualifiers,
and one where only the format-separator (\verb/|/) is recognized.
An opening mark may change the parsing context by
pushing a new context into a parsing
context stack whose top member is the effective parsing context.
The corresponding closing mark will pop the parsing context
stack.
...
matching lexemes and the verb|<=>| reorder and 
...
\key{raw-subexpression} ::=
	...
	\verb|[| & {\em raw-expression-head} \\
		 & {\em raw-expression-argument-list} \\
		 & {\em raw-expression-qualifier-phrase}$^\star$ \verb|]| \\
	\end{tabular} \\[1ex]
\key{raw-expression-head} ::= {\em word} $|$ {\em separator} \\[1ex]
\key{raw-expression-argument-list}
	\begin{tabular}[t]{@{}rl}
	::= & {\em empty} \\
	$|$ & {\em raw-expression-argument} \\
	    & \{ {\em reorder-mark-option}
		 {\em raw-expression-argument} \}$^\star$ \\
	\end{tabular} \\[1ex]
\key{reorder-mark-option} ::= {\em empty} $|$ \verb|<=>| \\[1ex]
\key{raw-expression-argument} ::= {\em raw-expression} \\[1ex]
\key{raw-expression-qualifier-phrase} ::=
	\begin{tabular}[t]{@{}ll}
	\verb|@@@| & {\em raw-expression-qualifier-head} \\
		   & {\em raw-expression-argument-list} \\
	\end{tabular} \\[1ex]
\key{raw-expression-qualifier-head} ::= {\em word} $|$ {\em separator}
\end{indpar}

The order of {\em raw-expression-qualifier-phrases}
...
The operator context (\ref{OPERATOR-CONTEXT})
contains a set of \skey{operator definition}s.
...
use to parse its operand subexpressions.  See the second on operator
...
[TBD: add operator def flag to right associative operators.]

[TBD: matchfix operators can turn evaluation, text, operators and qualifiers
on and off.]
