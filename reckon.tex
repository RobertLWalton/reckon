...
\key{variable-assignment-statement} ::=
	{\em variable} \ttkey{=} {\em expression} \\[1ex]
\key{variable} ::=
	{\em word} \{ {\em word} $|$ {\em separator} \}$^\star$ \\[1ex]
\key{pattern-assignment-statement} ::=
	{\em assignment-pattern} \ttkey{\Tilde} {\em expression} \\[1ex]
\key{assignment-pattern} ::= {\em expression} \\[1ex]
\key{guarded-statement} ::= \ttkey{if} {\em statement-guard} {\em block} \\[1ex]
\key{statement-guard} ::= {\em expression}
\end{indpar}
...
After parsing, the {\em variables} in a block are identified.  Each
{\em variable} is a subexpression consisting of a sequence of one or
more {\em words}, but not all such subexpressions are variables.
Some variables may be \mkey{inherited}{variable}
from the statement containing the block.
Some are left sides of {\em variable-assignment-statements}.
Some are parts of {\em assignment-patterns}.  All the {\em variables}
