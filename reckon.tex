...
contains an operator, but the operator name is not the entire expression,
the parse is in error.  This can happen, for example,
...
produce exactly this parse, provided assumptions \AALL hold.
\end{theorem}
...
algorithm is run on the expression are also deselected when the
...
Note that use of assumptions A1-A5 in this proof could be replaced by
...
parsed by the operator selection algorithm under assumptions \AALL,
