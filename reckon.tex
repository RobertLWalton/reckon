...
can never iterate, but {\em temporary-subblocks} can iterate.
...
where {\em variable-name} names a variable whose value is stored in the
{\em object}.
...
When permanently added code can execute no further (\secref{BLOCK-EVALUATION},
...
When code is permanently added to the block, the added code can also
...
is appended to the permanent {\em statement-group}.
Of course either group can be empty.

A statement of the form:

\begin{indpar}
\ttkey{for every}~~{\em type-name} \TT{:} \\
\verb|    |{\em block} 
\end{indpar}
...
\begin{indpar}
\ttkey{update}~~{\em object} \TT{:} \\
\verb|    |{\em block} 
\end{indpar}\label{UPDATE}

adds the code in the
...
`\TT{next}' variables can be used to create iterations of a block.
Each iteration replaces the previous iteration of the block.
