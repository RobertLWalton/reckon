...
\emkey{attributes}
    ...
    $|$ & \TT{::} {\em bip} \{ {\em bll} {\em attribute} {\em ell} \}\STAR{}
          {\em eip} \\
    \end{tabular}
\end{tabular}
...
\begin{center}
\tt
\begin{tabular}{p{2.5in}@{\hspace*{0.5in}}p{3.5in}}
\begin{tabular}[t]{l}
!@53 = \{| X Y Z |\} \\[1ex]
\rm is equivalent to \\[1ex]
!@53 =:: \\
~~~~1 = X \\
~~~~2 = Y \\
~~~~3 = Z \\
...
`\TT{|}', `\TT{>}', `\TT{)}', `\TT{]}', `\TT{'}', `\TT{\}}',
`{\em mark-type}', or `\TT{|} {\em name-type} {\em attributes}\QMARK{}' phrase,
and the corresponding `\TT{\{}' must be followed respectively
by a
`\TT{|}', `\TT{<}', `\TT{(}', `\TT{[}', `\TT{`}', `\TT{\{}',
`{\em mark-type}', or `{\em name-type} {\em attributes}\QMARK{} \TT{|}' phrase,
and if either list opening or list closing has a {\em type}, both must
have the same {\em type}.
As special cases,
`\TT{\{} {\em name-type} {\em attributes}\QMARK{} \TT{|}' can be matched by
`\TT{|\}}' and `\TT{\{|}' can match
`\TT{|} {\em name-type} {\em attributes}\QMARK{} \TT{\}}'.
...
\ttdmkey{type}{of list} attribute equal to their {\em mark-type} or their
dequoted {\em name-type}.

Exceptions are
lists with the form
`\TT{\{|} {\em prefix-0-list} \TT{|\}}'.
and lists with {\em name-type} \TT{""}
(the empty {\em quoted-string}).
These are not given
any \TT{.type}, \TT{.initiator}, or \TT{.terminator}.
Empty lists with no attributes at all are representable by
`\TT{\{""||\}}', `\TT{\{||\}}',
...
{\em Attributes} in a {\em name-typed} bracketed list can be the same as
