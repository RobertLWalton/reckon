...
or by a nofix operator of lower precedence,
or by a lexeme that is not part of a parsing terminator or a current
operator, then
the operator is designated to be postfix.

\item If the output list is non-empty, the operator has the postfix
flag and the infix flag, and the operator is followed by
an operator that can be prefix but cannot be postfix or infix,
then the operator is designated to be infix.
...
\item
If the operator was not determined to be postfix, then a
