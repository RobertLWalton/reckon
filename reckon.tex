...
or a key may be a {\em bracketed-list} that is a {\em pattern-factor}
in the clause whose \TT{.initiator} is not \TT{"("}.
...
syntax expression if that latter does not contain any terminating keys.
For example,
...
syntax equations.  But when we apply the Pattern Consistency Rules
...
The matching algorithm in Section \itemref{MATCHING-ALGORITHM}
...
to see if any match the expression.  This matching process
is as follows.
...
are deduced using propagation and some other rules described below.
...
{\em pattern-equation}
\begin{center}
\verb/sort my data descending :=~ sort ($X) ~ $DIRECTION ?/
\end{center}
the pattern in thus equation can be unfolded to make the {\em pattern-equation}
\begin{center}
\verb/sort my data descending :=~ sort ($X) descending/
\end{center}

that is consistent.  Note that in the {\em pattern} the atoms
`\TT{ascending}' and `\TT{descending}' are terminating keys, and so
