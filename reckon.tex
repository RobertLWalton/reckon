...
(See \pagref{TOP-LEVEL} a few extra details.)
...
Thereafer it can affect the parsing of subsequence top level lines.
There is \underline{no} way to pop definitions off the parsing definition
stack.  However, you can place `\verb|undefine|'s in the stack
that surpress the effect of particular parsing definitions.
...
In addition, parsing (\secref{PARSING}) is performed
by parsers, though parsing definitions standardly do not have
the `text' selector and are therefore standardly not active during
text parsing.  During parsing, number unit grouping
...
to an indented subxpression introduced by an implied indentation mark
before the
...
parsing subalgorithm that recognizes explicitly bracketted
...
parsing defintion stack which defines a gluing mark that is
...
{\bf Bracketted Subexpression Termination.}%
\index{Bracketted Subexpression Termination}
\begin{indpar}
If
\begin{list}{}
	     {\setlength{\itemsep}{0ex}%
...
parsing defintion stack which defines a mark that matches the
...
{\bf Bracketted Subexpression Initiation.}%
\index{Bracketted Subexpression Initiation}
\begin{indpar}
If
\begin{list}{}
	     {\setlength{\itemsep}{0ex}%
...
parsing defintion stack which defines an opening bracket that matches an
