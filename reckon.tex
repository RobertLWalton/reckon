...
for every board:
    ...
    on a board of size S <-- integer S,
                             max ship length <= S:
    ...
    on test position (DIRECTION, START) on THIS
        ...
        <-- pair of integers DIRECTION,
            pair of integers START,
            integer L, 1 <= L <= max ship length:

        (R,C) = START
        if L == 0:
            value = `true'
        else if R < 1 OR R > size:
            value = `false'
        else if C < 1 OR C > size:
            value = `false'
...
\ttmkey{`~'}{{\em time-code}} & a {\em time-format} must begin with \TT{`}
                                and end with \TT{'}; \\
				& these \TT{`~'} are deleted; \\
				& any \TT{'} inside the format must be
				  preceded by a \TT{\BSLASH} \\
...
called \skey{continuation line}s.
...
\TT{={}={}={}={}=}	& implies	& \TT{\{table\}}  & kind is code \\
\TT{`}			& implies	& \TT{\{quoted\}} & kind is text \\
\end{tabular}
\end{center}
...
paragraph,
or begins a new paragraph of the current or a previous paragraph type.
For example, if the current paragraph type is `\TT{code}',
the previous paragraph is continued, whereas if it is `\TT{quoted}',
a new paragraph of the type before the type of the last (`\TT{quoted}')
paragraph is begun.
...
data is written to a file with little attempt to format the data.
...
abbreviates single {\em bracketed-label} atom that can be otherwise
...
means the {\em represented-object-identifier} identifies what
the {\em object-element} denotes, which may be a single
atom, another object identifier, or a bracketed list.
And
...
means the {\em attribute-representations} add to the object
represented by the {\em bracketed-list}.
...
\emkey{mark-type} ::= {\em mark} other than \TT{<} or \TT{>} \\
