...
\verb|[]|  & eval, ops, quals, \verb/|/, text
					& \verb|<=>|, \verb|@@@| \\
\verb|`'|  & eval, ops, quals, \verb|<=>|, \verb|@@@|
					& \verb/|/, text \\
\verb|()|  & nothing			& nothing \\
\verb|{}|  & eval, \verb/|/, text	& ops, quals, \verb|<=>|, \verb|@@@| \\
\verb|<<>>|  & \verb/|/, text		& eval, ops, quals, \verb|<=>|,
					  \verb|@@@| \\
\end{tabular}
\end{center}
...
are recognized along with operators (ops) and qualifiers (quals),
and one where the format-separator (\verb/|/) is recoginized.

Also from the parsing point of view there are several types of
expression restructuring that can occur.  One, \verb|text| restructuring,
is turned on by \verb|`'| quotes, and does things like turn
\verb|`I am!'| into \verb|[sentence I am !]|.  A second kind of
restructuring, which is always on when \verb|text| restructuring is
