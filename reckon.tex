...
code for computing these values.  The code has the following syntax:
...
\key{guarded-statement} ::=
      {\em if-statement}
      {\em else-if-statement}$^\star$
      {\em else-statement-option}\\[1ex]
\key{if-statement} ::=
      \ttkey{if} {\em statement-guard} {\em guarded-block} \\[1ex]
\key{else-if-statement} ::=
      \ttkey{else if} {\em statement-guard} {\em guarded-block} \\[1ex]
\key{else-statement-option} ::= {\em empty} $|$
      \ttkey{else} {\em statement-guard} {\em guarded-block} \\[1ex]
\key{statement-guard} ::= {\em expression} \\[1ex]
\key{guarded-block} ::= {\em block}
\end{indpar}

Blocks are parsed when the statement that contains them (typically
a definition) is parsed, and in addition to {\em statements}, a block
...
After parsing, the {\em variable-names} in a block are identified.  Each
...
Some are parts of {\em assignment-patterns}.  All the {\em variable-names}
...
(\pagref{EXPRESSION-DEFINITION}) the pattern variable names in the
{\em expres\-sion-definition} {\em pattern} are inherited by the {\em block}.
Thus in
...
The pattern variable names in an {\em assignment-pattern} are
{\em variable-names} of the {\em block} containing the
{\em pattern-assignment-statement}.  Thus in the statement
...
{\em pattern-assignment-statement}.  Unlike pattern variables in
{\em expression-definitions}, there is no distinction between
evaluated and unevaluated pattern variables: the
{\em pattern-assignment-statement} pattern variable values are not
further evaluated.
...
{\em guarded-block} is ever evaluated.  An {\em else-if-statement}
{\em guard} is not evaluated until all {\em guards}
in any preceeding {\em if-statement}
or {\em else-if-statements} in the same {\em guard-statement}
have been evaluated to false.  The {\em guarded-block} of an
{\em else-statement} is evaluated if and only if
