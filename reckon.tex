...
\emkey{unguarded-subblock} ::= \TT{:} {\em block}
\\[1ex]
...
\emkey{if-statement} ::=
      ...
      \{ {\em ell}~~{\em bll}~~%
         \ttkey{else if}~~{\em guard}~~{\em unguarded-subblock} \}\STAR{} \\
      \{ {\em ell}~~{\em bll}~~%
         \ttkey{else}~~{\em unguarded-subblock} \}\QMARK{} \\
      \end{tabular} \\[1ex]
...
When identifying block variables, {\em statements} in a
{\em subblock} of the {\em block} are treated as if they
were in the {\em block} proper.  Thus variables named in the
{\em subblock} are variables of the {\em block}.\footnote{
RECKON does not have the notion of name space nesting.}
...
are given values by name in the {\em unguarded-subblock}.  For example,
in:
...
{\em value-expressions} in it have either been evaluated or
...
After the {\em statement-group}
without its default statements finishes executing,
any default statement in the {\em statement-group} that has
...
way are made active at the same time.  Then the {\em statement-group},
which now consists of all non-default statements and
all active default statements,
...
iterates if it produces a value for a {\em next-variable}.
...
the block actually computes (\secref{BLOCK-VARIABLES}).
