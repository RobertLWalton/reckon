...
\emkey{object-list}
    \begin{tabular}[t]{rl}
    ::= & {\em raw-list} \\
    $|$ & other not used in raw representation (see \pagref{OBJECT-LIST})
    \end{tabular}
\end{tabular}

\begin{tabular}{l}
\emkey{raw-list}
    ::= {\em raw-single-attribute-value}\STAR{} \\
\end{tabular}
...
\begin{tabular}{l}
\emkey{raw-single-attribute-value} ::= {\em name-lexeme}
...
{\em Data-representations} are lines beginning with \TT{@}.
For example,
...
These are {\em data-representations} of an object pair that can
...
{\em data-representation} (it may have been defined as the value of an
attribute).  Second, any attribute label represented in the
{\em data-representation}, if it previously exists because it was part of a
...
representation in the {\em data-representation} creates a new attribute value
...
\ttmkey{=>}{in {\em data-representation}}
is like \TT{=>{}>}
except that the {\em data-representation} cannot
...
in the {\em data-representation} must not have previously (before the
{\em data-representation} is read)
...
of this paragraph.  Thus \TT{=>} {\em data-representations} for
...
The RECKON parser (\secref{PARSING}) parses {\em data-representations}.
However, within {\em data-representations} (lines beginning with \TT{@})
...
and \TT{.initiator}.  An alternative raw {\em data-representation}
...
The {\em raw-single-attribute-values} in a {\em raw-list}
within an {\em data-representation}
are assigned as values of the attributes labeled \TT{1}, \TT{2},
\TT{3}, \ldots.  Additional attribute values can be given by
following the {\em raw-list} by a line ending `\TT{:}' that begins an
...
If a {\em data-representation} contains a bracketed list, operators
...
in the {\em data-representation}, that representation is
said to be `cooked'.

The syntax of {\em data-representations} is extended to include
cooked representations by completing the following:
...
\begin{tabular}{l}
\emlkey{cooked-single-}{attribute-representation} ::=
\\\hspace*{0.5in}
	{\em attribute-label} {\em attribute-label-flags}\QMARK{}
        \begin{tabular}[t]{@{}l@{}}
	{\em data-assignment-operator} \\
	{\em cooked-single-attribute-value}
	\end{tabular}
\end{tabular}%
\index{value!cooked attribute}

\begin{tabular}{l}
\emlkey{cooked-single-}{attribute-value}
    \begin{tabular}[t]{rl}
    ::= & {\em name-lexeme} \\
    $|$ & {\em concatenated-quoted-strings} \\
    $|$ & {\em object-identifier} \\
...
\begin{tabular}{l}
\emkey{property-value} ::= {\em raw-list}
\end{tabular}
...
In the cooked {\em data-representation}
the list of {\em raw-single-attribute-values} that could follow
the {\em object-header} has been replaced by
...
Note that here the \TT{\{ \}} brackets surround code and not
{\em attribute-representations}.  They do \underline{not}
play the same role as \TT{\{|  |\}} brackets.
...
\underline{not} play the same role as the \TT{:|} indentation mark.
...
`\TT{<b:} {\em text}\TT{|}{\em text}\TT{|b>}', e.g.,
