...
Note that qualifier heads become arrow labels, while expression heads
become labels of nodes that are targets of arrows labeled `\verb|0|'.
Because expression heads are nodes, they can be
expression graph variable values, but qualifier heads, which are
arrow labels, cannot be variable values.
...
given the same argument index, in this case `\verb|2|'.  The argument
after a \key{remainder mark} \ttkey{::>} is given the special argument index
`\ttkey{*}'.

The empty raw expression, `\verb|[]|', represents the expression tree which
only has one unlabeled node and no arrows:
...
A raw expression that only has qualifier clauses can be used to
represent an expression tree node that is not the source of an arrow
labeled `\verb|0|'.  For example, the raw expression
...
Expression trees can be used to represent expression graphs.  This is
done by defining conversions between expression trees
and expression graphs.
...
involved: the \key{graph variable value operation}, which defaults to
\ttkey{///}, and the \key{graph variable default operation}, which
defaults to \verb|~~~|%
\index{~~~@{\tt \protect\Tilde\protect\Tilde\protect\Tilde}}.
...
The conversion operations can also be done with different names for
the graph variable value and default operations.
...
same node, and at most one of these subexpressions may have either a value
or default for the variable.  That is, at most one of these subexpressions
may have a second argument.
