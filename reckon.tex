...
The {\em opening-mark} lexeme that begins an subexpression
must match (see \ref{MATCHING}) the {\em closing-mark} lexeme that
ends an subexpression.

Besides subexpressions defined in this section, other implied subexpressions
may be induced by operators: see \ref{OPERATORS}.

\subsection{Operators}
\label{OPERATORS}

Expressions in PCASL are evaluated top-down, meaning that the code
that evaluates one expression has control of evaluating subexpressions,
and can choose how each subexpression is evaluated.

Normally one of the first steps in expression evaluation is
\key{operator parsing}.  This scans the expressions for \skey{operator}s
and if any are found, inserts implied parentheses to re-parse the expression,
introducing new subexpressions, and changing the sequence of words, separators,
and subexpressions that constitute the expression.

Operator parsing is
done under the control of the \key{operator context}, which is defined by the
code that is evaluating the expression to be parsed.  The operator context
defines operators and specifies their precedence and other properties.

\subsubsection{Operator Definitions}
\label{OPERATOR-DEFINITIONS}

An operator context consists of a set of \skey{operator definition}s.
...
Expressions can be inferred to be true by inference rules.
...
Inference rules are collected into sets of rules called inference contexts.
An expression has an associated inference context that can be used to infer
and also evaluate the expression.  By default, the inference context
of an expression is the context in which the expression appears as a part
of a code block or as input from a user (what is called the `lexical context'
in programming languages).

Expression inference is done in three stages:

\begin{center}
\begin{tabular}{ll}
(1) & Implicit Parenthesis Insertion
      (\ref{IMPLICIT-PARENTHESIS-INSERTION}) \\
(2) & Subexpression Reduction \\
(3) & Expression Inference \\
\end{tabular}
\end{center}

Note that implicit parenthsis insertion have been covered in a previous
section and that expression evaluation is done after expression inference if
expression evaluation is being done at all.
