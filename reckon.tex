...
Overlapping operators are not treated classically, so in this
section we make the following
\skey{non-overlap assumption}s\index{overlap!non-overlap assumption}:

\begin{list}{}{}

\item[A1.] The representations of two operators as sequences of lexemes
can be identical, but otherwise cannot overlap.  (This is trivial if
all operators are just a single lexeme).

\item[A2.] No operator can overlap a non-operator.

\end{list}

Actually this last statement is not true unless we make the following
additional \skey{fixity assumption}s:

\begin{list}{}{}

\item[A3.] No operator can be an infix operator with more than one
precedence.

\item[A4.] No operator is both prefix and postfix.
...
{\em atomic-expression} ::= {\em non-operator} $|$
...
\begin{lemma}\label{ADJACENCY-LEMMA}
In the classical parse of an {\em expression}:
\begin{list}{}{}

\item[(a)] each {\em prefix-operator} is preceded by the
beginning of the expression, or by an {\em infix-operator}, or
by a {\em prefix-operator},
\item[(b)] each {\em postfix-operator} is followed by the
end of the expression, or by an {\em infix-operator}, or
...
produce exactly this parse, provided the non-overlap and fixity
...
{\em expression$_{n+1}$} that by Lemma~\ref{ENDS-LEMMA}
...
operator outside parentheses that is not deselected by step~\ref{}.
...
and that can also be an infix operator is deselected by step~\ref{}
of the operator selection algorithm.  We will give the argument for
{\em prefix-operators} and leave the symmetical argument for
{\em postfix-operators} to the reader.

By Lemma~\ref{ADJACENCY-LEMMA}
...
as by the fixity
assumptions a prefix operator cannot be a postfix operator.
