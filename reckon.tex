...
\verb|> display (a person named X) as ((oval (0.4,0.2)) labeled X) <--| \\
...
expression graphs (\ref{EXPRESSION-GRAPH-REPRESENTATION}).
...
and therefore designated by \Gvariable{\CurlyBra ROOT\CurlyKet} in our
example.
...
from the root or the value or default of some variable in the graph, all nodes
need not be reachable from the root,  The above example
...
is such a name.  Pseudo-variable names are always boxed, and
never double boxed.
...
bracketted.  Thus the above example can also be rendered as:
...
\begin{Graph}
    \Gdefault{X} \\
    \begin{Gsimplechild}{1}
	\Glabel{foe} \\
    \end{Gsimplechild} \\
    \begin{Gsimplechild}{2}
	\Glabel{fum} \\
    \end{Gsimplechild} \\
\end{Graph}
\hspace{0.3in}
...
\subsection{of nodes Pure Unification}
\label{PURE-UNIFICATION}

Informally, expression graph nodes may be thought of
...
A unification alogithm simply extracts a merge pair (M,N) from the
...
\label{NODE-MERGE-ALGORITHM} applied to the merge pair (M,N) is as follows:
...
\ikey{un-reordered argument indices}{un-reordered argument index} of the
root that label
\skey{un-reordered argument arrow}s of the root, and the targets of these
arrows are the \skey{un-reordered argument}s of the root.  Reordered
...
Each qualifier sources arity, head, and un-reordered argument arrows
...
During expression evaluation (\ref{EXPRESSION-EVALUATION})
the expression being evaluated is unified with a pattern in an expression
definition.  The unification used for this purpose is a
a variant called `call unification' (\ref{CALL-UNIFICATION}).
...
\item
There exists a 1-1 map between the set of un-reordered and reordered
...
is either an un-reordered argument arrow labeled $i$, 
...
arrow, a head arrow, an argument arrow (un-reordered, reordered, or rest),
...
is a node with a label, and any un-reordered non-rest
...
used in the inference algorithm (\ref{EVALUATION-ALGORITHM}).
...
\item
A call node is said to be of \key{indeterminate arity} if it has
an optional, reordered, or rest argument.  Otherwise the node is said
to be of \key{determinant arity}.

If \underline{both} M and N are of indeterminate arity,
the unification algorithm fails.

Otherwise, if just N is of indeterminate arity, switch M and N so that
...
a \ttmkey{\CurlyBra ROOT\CurlyKet}{of conversion} variable is created
...
applies.  If an expression definition has no expression block, the expression
...
An expression may act like a function call, and the called function may
evaluate expressions in the context of its caller, or a context derived
...
In use, the {\em pattern} is call unified (\ref{CALL-UNIFICATION})
...
a {\em block}, that is evaluated and produces a value for the expression.
...
and a context.  The context (\ref{CONTEXTS}) provides a list of expression
...
one argument order assignment (page~\pageref{ARGUMENT-ORDER-ASSIGNMENT})
...
\item[\ttkey{first-value}]~~~~The first definitions in contexts and first
...
\item[\ttkey{all-values}]~~~~All possible choices are tried and the
...
\item[\ttkey{consistent-values}]~~~~All possible choices are tried and the
successful results are collected in a set of values.  Then these results
are tested to see if they are pairwise equal.  If yes, one of these results
is returned as the expression value.  If no, an error values describing
the error are returned as the expression value, and as values of any variables
in the expression.

\item[\ttkey{consistent-reordering}]~~~~Definitions are 
...
in a set of results that is tested for pairwise equality.  If all are equal,
one of these results is returned as the expression value.  Otherwise, an
error value describing the error is returned as the expression value,
and as values of any variables in the expression.

\end{list}

If the search process that is expression evaluation yields no results at all,
...
\item
Make a copy of the expression definition.  In this copy, change each
variable name to a new unique name not used elsewhere to name any
variable.

\item

Call unify (\ref{CALL-UNIFICATION})
the pattern (\ref{EXPRESSION-DEFINITIONS})
from the definition with the expression to be evaluated.
...
visible side effects (\ref{VISIBLE-SIDE-EFFECTS}).

\item
Evaluate each guard of the definition in order.  If any fail to evaluate to
`\verb|true|', the definition does not apply.  These guard evaluations are not
permitted to have visible side effects (\ref{VISIBLE-SIDE-EFFECTS}).
...
is evaluated (\ref{BLOCK-EVALUATION}) to determine the value of the
expression.
