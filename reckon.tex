...
makes a new expression definition whose pattern, guards, and block
are the same as
its argument but whose context part is missing and can be set later.

There are two kinds of contexts: immutable and mutable.  Each kind can
be either a set or a list.
...
\verb|a| \ttkey{context}
    ...
    \\
    \begin{tabular}[t]{@{}rl@{}}
    [ & \ttmkey{is list}{of context!of {\tt context}} {\em IS-LIST-BOOLEAN} \\
    $|$ & \ttmkey{with contexts set}{of context!of {\tt context}}
           {\em WITH-CONTEXTS-SET-BOOLEAN} ]\opt
    \end{tabular}
    \end{tabular}
\end{indpar}
...
a context,
{\em IS-LIST-BOOLEAN} is either `{\tt false}' (the default) or `{\tt true}',
{\em WITH-CONTEXTS-SET-BOOLEAN} is either `{\tt false}' or `{\tt true}'
(the default).  `{\tt is list}' means the context is a list; otherwise
it is a set.  `{\tt with contexts set}' means any expression definitions
that are {\em CONTEXT-ELEMENTs} and that do not have their contexts set
will have them set equal to the context being created.  Thus the default
if given a expression definitions that do not have their contexts set
is to create a context set whose expression defintions have the new
context and therefore can reference each other.
