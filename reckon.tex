...
\emkey{trailing-separator} :::= \TT{'} $|$
				   ...
				   \TT{,}
\\[0.5ex]
\emkey{word} :::= {\em middle-lexeme} that has a letter before any digit
\\[0.5ex]
\emkey{numeric} :::= {\em middle-lexeme} that has a digit before any letter
\\[0.5ex]
\emkey{mark} :::= {\em middle-lexeme} other than {\em word} or {\em numeric}
\\[0.5ex]
\emkey{separator}
    \begin{tabular}[t]{@{}rl}
    ::= & {\em strict-separator} \\
...
{\em Middle-lexemes} are divided into the
the three classes, \emkey{words}, \emkey{numerics}, and \emkey{marks}.
Unlike some other languages, {\em marks} must be separated by
{\em white-space} from {\em words} and {\em numerics}.

\emikey{Numerics}{numerics}
often represent \key{numbers}\label{NUMBER} which are stored
...
Some {\em numerics} may be split into an initial {\em numeric} denoting
a number and a following {\em word} denoting a units measure.  E.g.,
`\TT{5in}' might be split into `\TT{5}' and `\TT{in}' (for inches).
See Section~\itemref{LEXEME-SPLITTING}.
...
that extend to the end of file.  This is \underline{not}
an error.
...
Blank lines are ignored unless they appear just before a non-blank,
non-continuation
