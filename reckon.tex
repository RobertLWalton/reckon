...
\author{Robert L. Walton\thanks{Copyright 2007 Robert L. Walton.
...
\key{lexeme-dictionary-entry} \\
    \hspace*{0.5in}::=
	{\em replaced-lexeme} \verb|==>| {\em replacing-lexemes}
	\\[1ex]
\key{replaced-lexeme} ::=
...
\label{PARSING-SELECTORS-SYNTAX}
...
A \key{subexpression} is a pair of matched lexemes, e.g.~\verb|( )| or
\verb|` '|, and all the lexemes in between.

Expressions and subexpressions can contain lexemes called
...
Certain lexemes, such as `{\tt ,}' and `{\tt ;}', used to separate
elements of lists, are treated as operators during parsing.

Matching lexemes such as \verb|` '| invoke a text parser that
inserts parentheses around sentences, paragraphs, etc., turning these
into lists of words, lists of sentences, etc.

The process of inserting implied parentheses is called
\key{expression parsing}\index{parsing!expression}.
Expression parsing also associates \skey{evaluator}s with some subexpressions.
The evaluator of a subexpression
is a function that is called to evaluate the subexpression.

Matching lexemes, such as \verb|` '|, control the parser used
...
ends a subexpression.  These marks are call the
\skey{bracket mark}s of the subexpression.

The subexpressions identified by this rule have explicit bracket
marks.  Below we introduce subexpressions with operators that have implicit
bracket marks as their opening and closing marks.
...
The bracket marks that surround a subexpression
can have special affects on parsing.
A summary of some of the affects of the standard bracket marks is:

\begin{center}
\begin{tabular}{l@{~~~~~~}l@{~~~~~~}l@{~~~~~~}l@{~~~~~~}l@{~~~~~~}l}
	& Matching  & Sets	&            &            & Does \\
In	& Bracket   & Parser	& Recognizes & Recognizes & Lexical	\\
Parser	& Marks	    & To	& Separators & Operators  & Parsing	\\[1ex]
any & \verb|(|\ldots\verb|)|

		    & --	& --         & --	  & -- \\
any & \verb|{|\ldots\verb|}|
		    & operator	& yes        & yes        & yes \\
operator & \verb|[|\ldots\verb|]|
		    & list	& yes        & no         & yes \\
list & \verb|[|\ldots\verb|]|
		    & list	& yes        & no         & yes \\
text & \verb|[|\ldots\verb|]|
		    & operator	& yes        & yes        & yes \\
any & \verb|`|\ldots\verb|'|
		    & text      & no         & no         & no	\\
any & \verb|``|\ldots\verb|''|
...
The text parser recognizes sentences, paragraphs, and so forth, but not
operators.  The list parser only recognizes separators, as if they were
operators.  The operator parser recognizes both operators and separators,
but not sentences, etc.

Parsing is controlled by the \key{parsing stack},\label{PARSING-STACK}
which contains definitions of parsers and operators, and
lexeme replacement and unit specifier
definitions related to lexical parsing.
...
names a parser.  The most recent parser
...
\key{parser-definition}
	::= \verb|define parser| {\em parser-name}
				{\em parsing-selectors} \\[1ex]
\key{parser-name} ::= {\em word} {\em word}\,$^\star$
\end{indpar}

There are three standard \ikey{parsers}{parser!standard}:
the \ttkey{operator parser},
that recognizes operators and separators;
the \ttkey{list parser},
that recognizes just separators;
and the \ttkey{text parser},
that recognizes the format-separator (\verb/|/) and punctuation.

The \emkey{parsing-selectors}\label{PARSING-SELECTORS} (see
\pagref{PARSING-SELECTORS-SYNTAX} for syntax) select which
parsing stack definitions are active (used),
and which are inactive (unused in the current context).
The topmost {\em parser-definition} in the stack is always active,
and other {\em parser-definitions} in the stack are always inactive.
Other kinds of definition are active if and only if they share
a {\em parsing-selector} with the active {\em parser-definition}.

Thus if the parser definition:

\begin{center}
\verb|define parser operator [operator]|
\end{center}

is active, all non-{\em parser-definitions} with the \verb|operator|
selector will be active, and all without this selector will be inactive.




\ikey{Implicit parentheses}{implicit parentheses} introduced by parsers
...
introduced by operators or separators are represented by \verb@(| |)@.
Other kinds of implicit parentheses are introduced by the text parser.
Here is a complete list of implicit parentheses and the kinds of subexpressions
they surround:
...
The operators parser (\secref{OPERATORS}) recognizes
the operators `\verb|+|', `\verb|-|', and `\verb|*|' in the expression
\verb|x + y - 5 * z| and turns this expression into
\verb@x + y - (| 5 * z |)@.
The list parser is similar, but only recognizes separators.  It turns
\verb|[george, betty sue, bill]| into
\begin{center}
\verb@[ george, (| betty sue |), bill ]@
\end{center}
In addition, lexical parsing (\secref{LEXICAL-PARSING}) is performed
by the operators and list parsers.
The text parser (\secref{TEXT-PARSING}), which is invoked by
\verb|`'| quotes, turns
\verb|``I think!  Therefore, I am!''| into
\begin{center}
\verb@(* <* I think ! *> <* Therefore , <| I am |> ! *> *)@
\end{center}

Some of the standard bracket marks affect evaluation, as is indicated
in the table on~\pagref{CONTEXT-SWITCHES}.  However, evaluation has
nothing directly to do with parsing.


TBD

Evaluation differs from parsing in that it is controlled by
the `wrapper functions' \verb|-EVAL-ON-| and \verb|-EVAL-OFF-|
that are inserted automatically during parsing when the evaluation
mode is changed (\secref{POST-OPERATOR-SELECTION-PROCESSING}).
Thus in a context in which evaluation is on, the \verb|Y| in
\verb|`Let X be [Y].'| will be evaluated but \verb|X| will not be,
because \verb|` '| turns evaluation off, while \verb|[ ]| turns it
back on.  The mechanism for this is that the expression
\verb|`Let X be [Y].'| is parsed to become

\begin{center}
\verb|[-EVAL-OFF- [-SENTENCE- let X be [-EVAL-ON- Y] .]]|
\end{center}

and the evaluator, running during program execution, sees the
\verb|-EVAL-OFF-|, and does not evaluate the following subexpression,
except for parts that have \verb|-EVAL-ON-|.

Note also that if the expression designated by \verb|-EVAL-ON-| evaluates
to a list inside a list, flattening is used to merge the lists.  Thus
if \verb|Y| has the value \verb|`a banana'| when the last expression
is evaluated, the result will be the same as evaluating
\verb|`Let X be a banana.'|, and \underline{not}
\verb|`Let X be (a banana).'|.

Also note that bracket marks that turn evaluation off (or on)
do not produce \verb|-EVAL-OFF-| (or \verb|-EVAL-ON-|) wrappers if
they are used in a context where the evaluation mode is already off
(or on).  Thus if \verb|`Let X be a `[Y]'.'| is evaluated when \verb|Y|
is \verb|`banana'|, the result is the same as evaluating
\verb|`Let X be a `banana'.'|.\footnote{Classical programming
languages would require that {\tt `Let X be a `[ [Y] ]'.'} be evaluated
to get this result.  Two nested {\tt [ ]}'s would be needed to overcome the
two nested {\tt ` '}.}
...
Brackets surround a subexpression and control which the parser is used
to parse the subexpression, and which operators, separators, and brackets
are recognized by that parser.

\subsubsection{Bracket Definitions}
\label{BRACKET-DEFINITIONS}

\ikey{Bracket definitions}{bracket definition} can be pushed onto
the parsing stack, and definitions in the stack
can be used by a parser.
A bracket definition specifies for each bracket pair the following:

\begin{indpar}[1in]
Name and Parsing Selectors \\
Parser and Parsing Selectors \\
Reformatter \\
Evaluator
\end{indpar}
...
\key{bracket-definition} \\
	\hspace*{0.5in}::= \ttkey{define bracket}
	    ...
	    {}[\verb|with|] \verb|parser|
	        {\em parser-name} {\em parsing-selectors} \\
	    {}[ [\verb|with|] \verb|reformatter|
	        {\em reformatter-name} ] \\
	    {}[\verb|with|] \verb|evaluator| {\em evaluator-name} \\
	    \end{tabular}
	    \\[1ex]
\key{bracket-name} ::=
...
the {\em bracket-definition} will be active (see~\secref{PARSING-SELECTORS}).

\subsubsection{Bracket Parser}\ttmindex{parser}{of bracket}
\label{BRACKET-PARSER}

A {\em parser-name}
is the name of the function that is called with an expression
as its single argument in order to parse the expression.  The parser
returns the parsed expression as its result.

The {\em parser-name} and the {\em parsing-selectors} that follow it,
if present, cause the parsing definition:

\begin{center}
{\tt define parser} {\em parser-name} {\em parsing-selectors}
\end{center}

to be pushed into the parsing stack before parsing the subexpression
surrounded by the brackets.  This definition is then popped from the
parsing stack after the subexpression is parsed.

The {\em parsing-selectors} provided with the {\em parser-name} determine
which non-{\em parser-definitions} in the parsing stack are active when
the parser is being used (see~\secref{PARSING-SELECTORS}).

The following are standard parsers:

\begin{indpar}

The \ttkey{operator parser}.  When used with the `{\tt operator}'
and `{\tt separator}' parsing selectors,
this parser recognizes operators and separators.
When used with just the `{\tt separator}' parsing selector,
this parser recognizes only separators.

The \ttkey{text parser}.  Normally used with the `{\tt text}'
parsing selector, although by default no parsing definitions have
this selector.
...
as its single argument in order to reformat the expression.  The reformatter
returns the reformatted expression as its result.

Reformatting occurs immediately after parsing, and can be considered as
a second stage of parsing.  Different reformatters can be used with the
same parser.
...
\key{operator-definition} \\
	\hspace*{0.5in}::= \ttkey{define operator}
	    ...
	    {}[\verb|with|] \verb|precedence| {\em precedence} \\
	    {}[\verb|definitional|] \\
	    {}[ [\verb|with|] \verb|reformatter|
	        {\em reformatter-name} ] \\
	    {}[\verb|with|] \verb|evaluator| {\em evaluator-name} \\
...
wrapper `\verb|[try ...]|', then the expression `\verb|(* x + y *)|'
will be restructured to be `\verb|[try (x + y)]|'.
...
The operator selection algorithm works on an expression viewed as a sequence
of words, separators, and proper subexpressions, possibly surrounded by
opening and closing marks.  The proper subexpressions are bounded
by opening and closing marks: see~\secref{SUBEXPRESSIONS}.
Operators found in the expression
cannot include any of the proper subexpressions.

The operator selection algorithm uses the concepts of prefix sequence and
postfix sequence.
...
\item\label{INFIX-SELECTION-STEP}
...
than the other, deselect the shorter operator.

\item\label{TOTAL-OPERATOR-STEP-1}
If one operator is selected at this point, and the operator name
is the entire expression, de-select the operator and terminate the algorithm.
Thus in the expression `\verb|x = (*)|', `\verb|*|' will be deselected
...
followed by the end of the expression, by an infix operator selected
at the end of the last step,
or by a non-empty part of a postfix sequence that includes
the infix operator.

Thus in the expression `\verb|x + - y|', `\verb|-|' would be deselected
as an infix operator by this step, as it is a prefix operator
preceded by an infix operator that cannot be a postfix operator.
However `\verb|+|' would not be deselected, as it is not preceded by
an operator or expression beginning.

\item\label{INFIX-OVERLAP-STEP}
The parse is in error if two consecutive or overlapping
...
\item\label{PREFIX-POSTFIX-SELECTION-STEP}
...
\item\label{TOTAL-OPERATOR-STEP-2}
If one operator is selected at this point, and the operator name
is the entire expression, de-select the operator and terminate the
algorithm.  Thus
in the expression `\verb|x = (+)|', `\verb|+|' will be deselected
when the subexpression `\verb|(+)|' is parsed.

\item
If one operator is selected at this point, the algorithm terminates
successfully.  Note the operator must be prefix and begin the expression
or postfix and end the expression.  The part of the expression
that is not the operator name is the sole operand, and is implicitly
parenthesized.  Note this operand cannot be empty.

\item\label{MISPLACED-OPERATOR-STEP}
Zero operators are selected at this point.  If the expression
contains any operator the parse is in error.  This can happen, for example,
if the only operator in the expression is an prefix operator not at the
beginning of the expression, or a postfix operator not at the end.
It can also happen in an expression like `\verb|* x|' in which the infix
operator `\verb|*|' was deselected because it begins an expression.
...
then the expression `\verb|x +*+ * y|' would be illegal by
step~\stepref{INFIX-OVERLAP-STEP}
because \verb|*| would not be defined as a prefix operator soon enough.
...
In this section we prove that parsing using the operator selection algorithm of
...
We want to show that the parse of any expression that has an unambiguous
...
\item[A1.]\index{Assumption A1}\label{AONE}
All operators are defined in the parsing stack, and the parsing
stack does not change during the expression parse.

\item[A2.]\index{Assumption A2} There are no matchfix operators
(if these begin and end
with opening and closing marks and do not overlap other operators,
they behave just like parentheses).

\item[A3.]\index{Assumption A3} No two operators can overlap.
(This is trivial if all operators are just a single lexeme).

\item[A4.]\index{Assumption A4} No operator can overlap a non-operator.

\item[A5.]\index{Assumption A5}\label{AFIVE}
No operator is both prefix and postfix.

%% Warning: A1-A5, A4, and A5 are referenced below.

\end{list}
\newcommand{\AALL}{A1-A5\,$^{p\pageref{AONE}}$}
\newcommand{\AFIVE}{A5\,$^{p\pageref{AFIVE}}$}
...
Assumption A5 reflects a minor distinction between
...
\begin{corollary}\label{FIXITY-COROLLARY}
...
\item[(f)] no {\em infix-operator} is preceeded by an {\em infix-operator}
or a {\em prefix-operator},
\item[(g)] no {\em infix-operator} is followed by an {\em infix-operator}
or a {\em postfix-operator},
\item[(h)] an {\em expression$_n$} cannot contain outside parentheses any
...
\begin{theorem}\label{CLASSICAL-PARSE-AMBIGUITY-THEOREM}
Under assumptions \AALL,
if an {\em expression} has a classical parse, the {\em expression} has
two or more distinct classical parses (or in other words, has an
`\key{ambiguous}' classical parse) if and only if the {\em expression}
contains a sequence `$X$ $Y$' of two operators such that $X$ can be
postfix or infix and $Y$ can be prefix or infix.
\end{theorem}

Proof: If the {\em expression} has two classical parses,
these cannot have identical
infix operators, is if they did, the fixities assigned to operators by
the two parses would be the same, since the fixities assigned to an operator
in a {\em primitive-expression} depends solely whether the operator is
before or after the {\em non-operators} in the {\em primitive-expression}.
Then by corollary~\ref{FIXITY-COROLLARY} the two classical parses
could not be distinct.

Therefore if the {\em expression} has two classical parses, some infix
operator in the one parse must not be an infix operator in the other parse.
Without loss of generality assume this operator is outside parentheses,
is an infix operator in the first parse, and is a prefix operator in the
second parse (the postfix case being handled by a symmetric argument).
Take the first operator $Y$ in the {\em expression} that has this property.
$Y$ cannot be at the beginning of the {\em expression}, for it is an infix
operator in the first parse.  Therefore $Y$ must be preceeded by an infix
or prefix operator $X$ in the second parse, by
Lemma~\ref{CLASSICAL-PARSE-CONTEXT-LEMMA}.  But $X$
cannot be an infix or prefix operator in the first parse,
also by Lemma~\ref{CLASSICAL-PARSE-CONTEXT-LEMMA},
so it must be a postfix operator in the first parse.
Since $X$ is postfix in the first
parse, and must be prefix or infix in the second parse, it must be
infix in the second parse by \AFIVE.  So the {\em expression} contains a
sequence of operators `$X$ $Y$' such that $X$ can be postfix or infix and
$Y$ can be prefix or infix.

Conversely, if the {\em expression} has a classical parse
it has a semi-classical parse, and if it has a sequence `$X$ $Y$' of operators
the first of which may be infix or postfix and the second infix or prefix,
it must by Lemma~\ref{SEMI-CLASSICAL-PARSE-CONTEXT-LEMMA} be of the form:

\begin{center}
\ldots {\em primitive-expression} $X$ $Y$ {\em primitive-expression} \ldots
\end{center}

as $X$ must be preceded and $Y$ followed by a {\em primitive-expression}.
Then we can either take $X$ to be an infix operator and $Y$ to be a
prefix operator,
or $X$ to be a postfix operator and $Y$ to be an infix operator, giving
two distinct semi-classical parses, which in turn give two distinct classical
parses.
...
produce exactly this classical parse, provided assumptions \AALL hold.
\end{theorem}
...
step~\stepref{INFIX-DESELECTION-STEP} or
step~\stepref{LOWER-PRECEDENCE-STEP}.
...
But an {\em expression$_{n+1}$} might contain
...
step~\stepref{INFIX-DESELECTION-STEP}, as by assumption \AFIVE
a prefix operator cannot be a postfix operator.
If it is preceded by an infix operator that can be a postfix operator, then
...
But by Theorem~\ref{CLASSICAL-PARSE-AMBIGUITY-THEOREM}
...
\begin{lemma}\label{SUBEXPRESSION-INFIX-SELECTION-LEMMA}
Let the operator selection algorithm be run on an expression to produce
some selected operators and some subexpressions, and let $S$ be one of the
subexpressions.  Then let the operator selection algorithm be run on $S$.
Under assumptions \AALL, both algorithm executions
select the same infix operators in $S$ at the end of their respective
step~\stepref{INFIX-DESELECTION-STEP}s.
\end{lemma}

Proof:  The infix operators in $S$ are the same for both algorithm
executions by the assumptions.  We must prove that
step~\stepref{INFIX-DESELECTION-STEP} deselects the same operators
in $S$ in both algorithm executions.

Suppose an operator is deselected
by step~\stepref{INFIX-DESELECTION-STEP} in the algorithm run for the
whole expression because the operator is a prefix
...
when the algorithm is run for the subexpression,
though any preceding operator may be replaced
by a subexpression beginning.  A symmetrical argument applies to
deselected postfix operators.  So infix operators deselected when the
algorithm is run on the whole expression are also deselected when the
algorithm is run on the subexpression.

Now suppose an operator $Y$ is deselected
by step~\stepref{INFIX-DESELECTION-STEP} in the algorithm run for the
subexpression $S$ because the operator is a prefix
operator preceeded by a prefix operator that cannot be a postfix
operator, or by an infix operator that cannot be a postfix operator,
or by the beginning of the subexpression $S$.  Then this will also be true
when the algorithm is run for the whole expression, except possibly
in the case where the operator is at the beginning of the subexpression
but not at the beginning of the whole expression.  In this case the
operator is preceded in the whole expression by an operator $X$ that
must have been selected by the algorithm run on the whole expression.
Since $X$ is before $Y$, $X$ cannot have been selected as a postfix operator.
Therefore $X$ must be either an infix or prefix operator.  If it is
a prefix operator it cannot also be postfix, by \AFIVE, so $Y$ will be
deselected by the algorithm on the whole expression.  If it is an
infix operator that is also a postfix operator, neither $X$ nor $Y$ would
be deselected by step~\stepref{INFIX-DESELECTION-STEP}
of the algorithm on the whole expression, and that algorithm
would have failed at step~\stepref{INFIX-OVERLAP-STEP}, which it did not
do.  So $X$ cannot be infix
and postfix, and $Y$ will be deselected when the algorithm is run on the
whole expression.  Therefore in all cases $Y$ is deselected when the
algorithm is run on the whole expression.
A symmetrical argument applies when $Y$ is infix and postfix.
So infix operators deselected when the
algorithm is run on the subexpression are also deselected when the
algorithm is run on the whole expression.

QED

Note that use of assumptions A1-A4 in this proof could be replaced by
the assumption that no infix operator that is not deselected
overlaps a fractional part of another operator.  This keeps operators
from crossing the boundaries of subexpressions.

\begin{theorem}
If a sequence of lexemes is successfully
parsed using the operator selection algorithm, and assumptions \AALL hold,
the resulting parse is the unique classical parse of the sequence, provided
the resulting parse does not contain a subexpression consisting of just
an operator (see steps~\stepref{TOTAL-OPERATOR-STEP-1}
and~\stepref{TOTAL-OPERATOR-STEP-2} of the algorithm).
\end{theorem}

Proof: By steps~\stepref{INFIX-SELECTION-STEP}
through~\stepref{INFIX-OVERLAP-STEP}
and Lemma~\ref{SUBEXPRESSION-INFIX-SELECTION-LEMMA}
the algorithm divides the expression
into non-empty subexpressions separated by infix operators.
By steps~\stepref{PREFIX-POSTFIX-SELECTION-STEP}
through~\stepref{END-STEP} each subexpression is a {\em primitive-expression}.
Therefore the parse produced is a semi-classical parse.  And by the various
steps that select only one operator during each iteration of the algorithm,
the result is a classical parse.

The classical parse can be shown to be unique by showing the {\em expression}
does not contain a sequence of operators `$X$ $Y$' such that $X$ can be
infix and postfix and $Y$ can be infix and prefix.  If the {\em expression}
contained such a sequence, both $X$ and $Y$ would \underline{not}
be deselected by step~\stepref{INFIX-DESELECTION-STEP}, and the parse would
be in error by step~\stepref{INFIX-OVERLAP-STEP}.  Since the {\em expression}
cannot contain such a sequence, by
Theorem~\ref{CLASSICAL-PARSE-AMBIGUITY-THEOREM} the classical parse is
unique.
...
How does one know that an expression is a definition, in
\secref{POST-OPERATOR-SELECTION-PROCESSING}.
