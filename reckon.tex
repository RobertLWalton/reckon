...
a sequence of \skey{lexeme}s.  The sequence of lexemes is then subject to
...
that are evaluated (\secref{EXPRESSION-EVALUATION}).  The syntax given
for these categories represents either the syntax recognized by the
expression definitions (\secref{EXPRESSION-DEFINITIONS}) that define
the expressions, or the syntax that can be used to produce a value of
the type required after the expression is evaluated.}
		     \begin{tabular}[t]{rl}
