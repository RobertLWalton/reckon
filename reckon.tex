...
\hline
\end{tabular} \\
\verb|> update x to 45 degrees| \\
\begin{tabular}{|r|r|}
\hline
\multicolumn{1}{|c}{\bf ID} &
\multicolumn{1}{|c|}{\bf value} \\
\hline
\tt @1000044 & 45 degrees \\
\hline
\end{tabular} \\
\verb|> display (a demo with angle X) <-- {| \\
\verb|+     c = {circle 2.0 dotted center (0.0,0.0)}| \\
\verb|+     x-axis = {arrow from (-3.0,0.0) to (3.0,0.0)}| \\
\verb|+     y-axis = {arrow from (0.0,-3.0) to (0.0,3.0)}| \\
\verb|+     line = {line from (0.0,0.0) to <<(cos X, sin X)>>}| \\
\verb|+     arc = {arc-arrow from (0.7,0.0) to <<(0.7*cos X, 0.7*sin X)>>}| \\
\verb|+     value = {column (| \\
\verb|+                overlap (| \\
\verb|+                  <<c>>,| \\
...
\verb|+                  <<line>>,| \\
\verb|+                  <<arc-arrow>> labelled `{Greek T}' ),| \\
\verb|+                label `Depiction of Angle {Greek T}' )}} | \\
\end{indpar}
