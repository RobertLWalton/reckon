...
\key{name} ::= \{ {\em symbol} $|$ {\em number-lexeme} \}$^\star$
               {\em symbol}
               \{ {\em symbol} $|$ {\em number-lexeme} \}$^\star$
\\[1ex]
...
a sequence of {\em symbols} and {\em number-lexemes} containing at least one
{\em symbol}.  {\em Names} are used to name things, such
as functions and variables.

Names are sometimes matched to sequences
of input lexemes.  When this is done, a quoted string in the name has
one pair of \verb|"| quotes removed before the match.  Thus:
...
\key{temporary} ::= \verb|$| {\em natural-number} \\[1ex]
\key{natural-number} ::= see \pagref{NATURAL-NUMBER}
\end{indpar}
