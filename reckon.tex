...
values during unification.  The output of evaluation is both a value for
...
successful results are collected in a set of values.  Note that each
value in this set consists of a value for the expression being evaluated
and values for each variable in that expression.

\item[\ttkey{consistent-values}]~~~~All possible choices are tried and the
successful results are collected in a set of values.  Then these values
are tested to see if they are pairwise equal.  If yes, one of these values
is returned as the expression value.  If no, an error value describing
the error is returned as the expression value.
...
in a set of values that is tested for pairwise equality.  If all are equal,
one of these values is returned as the expression value.  Otherwise, an
...
\item
Evaluate the values of variables which have been assigned values,
unless these variables are marked as holding unevaluated argument values.
