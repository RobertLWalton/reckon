...
\begin{tabular}{l@{~~~~~~}l@{~~~~~~}l@{~~~~~~}l@{~~~~~~}l}
	& With      &           & Sets		& Sets \\
In	& Parsing   & Matching  & Parser	& Parsing \\
Parser	& Selector  & Brackets  & To		& Selectors To \\[1ex]
any     & -	    & \verb|(|\ldots\verb|)|
		                & [no change]	& [no change] \\
any	& -	    & \verb|{|\ldots\verb|}|
				& operator	& operator \\
...
During parsing there are also parsing selectors which determine
which definitions in the parsing stack are used.  The `operator'
parser may run with either the `operator' or `list' parsing selectors.
In the former case, all operators are standardly recognized, whereas
