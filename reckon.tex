...
If an {\em opening-lexeme} $O$ is between a pair $P$ of matched lexemes,
the {\em closing-lexeme} $C$ that matches $O$ must also be between $P$.

If one pair $P_2$ of matching lexemes is between another pair $P_1$
of matching lexemes, $P_2$ is said to be \mkey{nested}{lexemes} inside
of $P_1$.
...
An \key{closing-quote-character} must be part of a sequence of
{\em closing-quote-characters} that is just long enough to be the
matching lexeme for some previous {\em open-quote}.
Otherwise it is a {\em word-character}.
...
resulting from the split is a {\em number} and the {\em separator}
is a period.
...
\key{primary-word} ::= {\em word}
		$|$ {\em word} {\em ambiguous-separator} \\[1ex]
\key{number} ::= {\em real-number} $|$ {\em complex-number} \\[1ex]
...
\key{unsigned-integer} ::= {\em digit} {\em digit}$^\star$ \\[1ex]
\key{exponent} ::= {\em radix-indicator}
		$|$ {\em radix-indicator} {\em integer} \\[1ex]
\key{radix-indicator} ::=     \verb|E|
			  $|$ \verb|e|
			  $|$ \verb|D|
			  $|$ \verb|d|
			  $|$ \verb|X|
