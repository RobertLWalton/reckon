...
\\[1ex]
\emkey{operator-expression-$N$} ::= \begin{tabular}[t]{l}
	an {\em operator-expression} that has no operator \\
	of precedence $N$ or less outside brackets
	\end{tabular}
\label{OPERATOR-EXPRESSION-N}
\\[3.3ex]
...
\emkey{expression-definition} ::=
	...
	\\[1ex]
\emkey{pattern} ::= {\em pattern-term-option}
...
\emkey{default-equation} ::= {\em syntax-variable}
		...
		{\em expression} \\[1ex]
\emkey{constraint-equation} ::= {\em expression}
