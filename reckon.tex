...
\newcommand{\tttbmkey}[2]{{\tt <{\bf #1}\BAR\ldots\BAR{\bf #1}>}%
    \index{#1@{\tt <#1\BAR\ldots\BAR#1>}!#2}}
...
\newcommand{\emxkey}[2]{{\bf \em #1#2}\index{#1@{\em #1}!#2@{\em #2}}}
...
\newcommand{\CLOSE}{{$\}$}}
\newcommand{\BAR}{{\tt \bf |}}
\newcommand{\DOUBLEBAR}{{\tt \bf |\hspace{-0.02pt}{}|}}
\newcommand{\LEFTBRACKET}{{\tt \bf \{}}
\newcommand{\RIGHTBRACKET}{{\tt \bf \}}}
\newcommand{\LEFTSQUARE}{{\tt \bf [}}
\newcommand{\RIGHTSQUARE}{{\tt \bf ]}}
...
\emkey{separator-character} \begin{tabular}[t]{rl}
		::= &   \verb|(|
	       	    ...
	       	    $|$ \verb|`| \\
		$|$ & \verb|,| not surrounded by {\em digits} \\
		$|$ & \verb|'| not followed by a letter or digit \\
		    \end{tabular}\\[1ex]
...
\\\hspace*{0.5in}
{\em number} lexeme (\pagref{DECIMAL-NATURAL})
				 containing only digits and commas
\end{tabular}%

\begin{tabular}{l}
\emlkey{raw-attribute}{-representations}
\\\hspace*{0.5in}
	\begin{tabular}[t]{rl}
        ::= & {\em empty} \\
	$|$ & \LEFTBRACKET\BAR{} \BAR\RIGHTBRACKET \\
	$|$ & \LEFTBRACKET\BAR{}
	      {\em raw-attribute-representation}
	      \{ \verb|;| {\em raw-attribute-representation} \}$^\star$
	      \BAR\RIGHTBRACKET{} \\
	$|$ & \begin{tabular}[t]{@{}l@{}}
	      \BAR:{\em -indentation-mark} \\
	      ~~~~{\em indentation-beginning}
	          {\em raw-attribute-representation} \\
	      ~~~~\{ {\em indentation-separator}
	         {\em raw-attribute-representation} \}$^\star$ \\
...
\begin{tabular}{l}
\emxkey{raw-single-attribute-}{representation} ::=
\\\hspace*{0.5in}
...
\emkey{attribute-label-flags-option} ::=
    {\em empty} $|$
    \LEFTSQUARE{} {\em atom}\,$^\star$ \RIGHTSQUARE{}
\end{tabular}%
...
Their special syntactic marker is the {\tt \bf =|} sign.
They can
use either {\tt \bf \{|~~|\}} brackets and {\tt \bf ;} separators, or
the {\tt \bf |:} indentation mark and {\em indentation-separators}.
To replace {\tt \bf \{|  |\}} and {\tt \bf ;} by indentation,
indent appropriately placing the {\tt \bf \{|}, {\tt \bf |\}}, and {\tt \bf ;}
brackets and separators at line ends, replace {\tt \bf \{|} by {\tt \bf |:},
and delete the {\tt \bf |\}}'s and {\tt \bf ;}'s.
A {\tt \bf =|~~|:} sequence at a line end can be abbreviated by {\tt \bf =|:},
and similarly {\tt \bf >|~~|:} and {\tt \bf >{}>|~~|:} can be abbreviated
by {\tt \bf >|:} and {\tt \bf >{}>|:}.
For example,
...
Note that in this syntax {\em atoms} such as {\tt \bf =} must be
avoided, as they will be confused with symbols recognized by the
parser (to end a {\em label} in this case).
You can use {\tt \bf "="} instead.

A {\em raw-attribute-representation} is ignored if it is {\em empty},
which allows superfluous `\verb|;|'s in a list of
{\em raw-attribute-representations}.

Note lastly that the {\tt \bf \{| |\}} brackets, which are used in object
representation, are distinct from the {\tt \bf \{ \}} brackets, which are
used to bracket executable code.

It is possible to place \skey{attribute flag}s on attribute labels
by putting flag characters in {\tt \bf [ ]} brackets after a label.
The flag characters are those of one or more {\em atoms} enclosed
in the {\tt \bf [ ]} brackets.  If {\tt \bf ]} is used as one of
these {\em atoms} is must be quoted as in {\tt \bf "]"}.
...
in symbols inside {\tt \bf [ ]} brackets that
...
The differences between {\tt \bf =}, {\tt \bf >}, and {\tt \bf >{}>} in the
{\em object-header} relate
to what is done when an object or attribute label previously exists.
Note that with the abreviation convention, these also become differences
between {\tt \bf =|:}, {\tt \bf >|:}, and {\tt \bf >{}>|:}.

\ttmkey{=|}{in {\em object-header}}
(or \ttmkey{=|:}{in {\em object-header}})
...
Thus {\tt \bf >|:} is used to introduce new attributes to an existing object.
...
Because atoms like {\tt \bf =} and {\tt \bf |}
are avoided in attribute labels and values, the parser does
nothing with these.  However after parsing {\em quoted-strings}
...
values {\tt \bf 5} and {\tt \bf "5"} produce vary different values:
...
{\em raw-representations} with `{\em raw-}' replaced everywhere
by `{\em cooked-}' and two syntax equations adjusted as follows:

\begin{indpar}

\emlkey{cooked-object}{-representation} ::=
\\\hspace*{0.5in}
    {\em object-header} {\em expression-option}
    {\em cooked-attribute-representations}
\\[1ex]
\emlkey{cooked-single}{-attribute-value} ::= {\em expression}
\\[1ex]
{\em expression} ::= see section \secref{PARSING}
...
\\[1ex]
\emkey{decimal-natural} \begin{tabular}[t]{rl}
                         ::= & {\em decimal-digits} \\
                         $|$ & {\em decimal-digits}
			       \verb|,| {\em digit} {\em digit} {\em digit}
			 \end{tabular}
...
\ttmkey{define}{reformatter} \hfill \OP{<-{}-} ~~~
	\OP{\LEFTBRACKET \RIGHTBRACKET} ~~~ \OP{:}
...
\emkey{pattern-qualifier} ::=
		       ...
		       $|$ \ttmkey{...?}{pattern qualifier}
	\\[1ex]
\emxkey{pattern-attribute-}{representations}
\\\hspace*{0.5in}
	\begin{tabular}[t]{rl}
        ::= & {\em empty} \\
	$|$ & \LEFTBRACKET\BAR{} \BAR\RIGHTBRACKET{} \\
	...
	\end{tabular}
\\[1ex]
\emxkey{pattern-attribute-}{representation} ::=
\\\hspace*{0.5in}
    {\em required-attribute-representation}
    $|$ {\em optional-attribute-representation}
\\[1ex]
\emxkey{required-pattern-attribute-}{representation} ::=
\\\hspace*{0.5in}
	{\em attribute-label} {\em attribute-label-flags-option} \verb|=|
	      {\em pattern}
\\[1ex]
\emxkey{optional-pattern-}{attribute-representation} ::=
\\\hspace*{0.5in}
	{\em attribute-label} {\em attribute-label-flags-option} \verb|=?|
	      {\em pattern}
\end{indpar}
...
\begin{center}
\begin{tabular}{rcl}
\verb/sort x/ & \tt :=\TILDE & \verb/sort ($X) . ascending ASCENDING ??/ \\
\end{tabular}
\end{center}
...
\emlkey{pattern}{-assignment-statement} ::=
	{\em pattern} \verb|~=~|\index{~=~@{\tt \TILDE=\TILDE}}
	{\em right-side} \\[1ex]
