...
(so the first {\em pattern-clause} cannot be omitted when the
pattern is unfolded: see below).

\item Every {\em pattern-clause} must contain a {\em pattern-factor}
that is either an {\em atom}, an {\em alternative-syntax-variable}
(which will be replaced by one of a finite set of atom sequences: see
below), or a {\em pattern} that is a
{\em bracketed-list} with an \TT{.initiator} other than `\TT{(}'
(so the \TT{.initiator} can be matched just as atoms are matched).
...
\item If a {\em pattern} is not an object (it is a single atom or
syntax variable) it is replaced by an object containing as its
sole attribute a single list element equal to the original pattern
...
on the left.  The rules governing the formation of the matching are:
...
\TT{"("} is matched to a single element of the of the syntax expression
that is also a bracketed list whose \TT{.initiator} and \TT{.terminator}
equal the \TT{.initiator} and \TT{.terminator} of the pattern bracketed list.

\item Each bracked list of the pattern whose \TT{.initiator} \underline{is}
\TT{"("} is either matched as in (4), or is matched to a (possibly empty)
sequence of elements of the syntax expression which is followed by
(a) the end of the syntax expression, or (b) an element matched using
rules (3) or (4).
...
Then the pattern consistency rule is simply:
