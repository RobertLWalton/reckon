...
no attributes except a list element equal to the atom
\end{enumerate}
...
The \key{pattern matching} algorithm matches an expression to a pattern.
Both the expression and the pattern are viewed by the algorithm as
data, that is, as atoms or objects, and \underline{not} as lexeme
sequences.  The matching algorithm has one set of rules for matching
the list parts of object, and another set for matching the non-list
parts.
