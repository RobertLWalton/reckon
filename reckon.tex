...
reordered, as in \verb|element LIST <:> INDEX|, and is used in
expression definitions.
The \key{remainder mark} (\ttkey{::>}) preceeds a final argument in a
definition that represents the list of remainding arguments in a use
of the definition, as in \verb|max ::> ARGS| where \verb|ARGS| represents
the list of all the arguments given to \verb|max|.
The \key{qualifier mark} (\ttkey{@@@}) signals that the following
expression item is a qualifier, as in \verb|sort x @@@ ascending|,
...
restructured.  The only special marks in a raw expression
are the reorder mark (\verb|<:>|)
and the qualifier mark (\verb|@@@|).
Translating
an expression into a raw expression is called \key{parsing}.

Subexpression bracketing matching lexemes may affect parsing.
...
operator definitions with controls (\ref{OPERATOR-CONTROL}).

Parsing is controlled by the \key{parsing context}, which consists
of a \key{parsing mode stack} and a \key{parsing definition stack}.

There are three kinds of \skey{parsing mode}s:
the \ttkey{marks} mode where only the reorder mark(\verb|<:>|),
remainder mark(\verb|::>|), and the qualifier mark (\verb|@@@|) are recognized,
the \ttkey{operators} mode where these marks
are recognized along with operators and qualifiers,
and the \ttkey{text} mode where only the format-separator (\verb/|/)
is recognized.
An opening mark may change the parsing mode by
pushing a new parsing mode into a parsing
mode stack whose top member is the effective parsing mode.
The corresponding closing mark will pop the parsing mode
stack.

From the parsing point of view there are several types of
expression restructuring that can occur.  One, text restructuring,
is turned on by \verb|`'| quotes, and does things like turn
\verb|`I am!'| into \verb|[sentence I am !]|.  A second kind of
restructuring, which is on if and only if text restructuring is
off, enforces the rule that the first item of an expression with no
...
\verb|(-EMPTY- (integral x) from 0)|.  Lastly, operators can cause
major restructuring of an expression (\ref{OPERATORS}),
...
\subsubsubsection{Fixity}\ttmindex{fixity}{of operator}
...
An \verb|infix| operator is placed between its two operands;
a \verb|prefix| operator is placed before its one operand;
a \verb|postfix| operator is placed after its one operand;
and a \verb|matchfix| operator surrounds its one operand.
Operands of \verb|prefix|, \verb|infix|, and \verb|postfix|
operators may not be empty.
An \verb|infix| operator may not begin or end an expression;
a \verb|prefix| operator must begin an expression;
a \verb|postfix| operator must end an expression;
and a \verb|matchfix| operator has two parts: an opening operator
that must begin an expression and a closing operator that must end
the same expression.

\subsubsubsection{Name}\ttmindex{name}{of operator}
...
name, they must begin and end the name.  When a subexpression is tested
...
the matchfix operator `\verb/[| ... |]/' from being recognized.

When an expression is checked for operators,
only infix operators in the expression that have lowest precedence
among all the infix operators in the expression are considered, and
other infix operators are suppressed.
All prefix operators that begin the expression are considered,
all postfix operators that end the expression are considered,
and all matchfix operators that begin and end the expression are
considered.  Ambiguity is possible, and, unless resolved by one of the
following rules, is an error.

{\bf Matchfix Rule.}\index{Matchfix Rule}
If two different matchfix operators surround an expression, but the
first has longer opening and closing names, then the second is suppressed.
If a matchfix operator and a prefix operator apply, but the opening operator
name of the matchfix operator is longer than the name of the prefix operator,
then the prefix operator is suppressed.
If a matchfix operator and a postfix operator apply, but the closing operator
name of the matchfix operator is longer than the name of the postfix operator,
then the prefix operator is suppressed.
If a matchfix operator and in infix operator apply, the infix operator is
suppressed.

Thus if `\verb/[| ... |]/' and `\verb/[ ... ]/' are two matchfix operators
that both apply to an expression, the second will be suppressed.  If
`\verb|- < ... > -|' is a matchfix operator and `\verb|-|' is a prefix
operator, and both apply to an expression, the prefix operator will be
suppressed.
However, if `\verb|-< ... >-|' is a matchfix operator and `\verb|-<|'
is a prefix operator, and both apply to an expression, neither will be
suppressed, and their ambiguity will cause an error.

{\bf Prefix/Postfix Rule.}\index{Prefix/Postfix Rule}
If an expression begins with a consecutive sequence of prefix operators,
any infix operator overlapping this sequence is suppressed.
If an expression ends with a consecutive sequence of postfix operators,
any infix operator overlapping this sequence is suppressed.

Thus if `\verb|+|' is both a prefix and infix operator,
the expression `\verb|+ + y|' is equivalent to `\verb|+ (+ y)|'.

{\bf Infix Precedence Rule.}\index{Infix Precedence Rule}
If an expression contains any infix operators that are not be suppressed by the
previous rules (Matchfix and Prefix/Postfix), then any prefix operators
beginning the expression are suppressed, and any postfix operators
ending the expression are suppressed.

{\bf Consecutive Infix Rule.}\index{Consecutive Infix Rule}
Any consecutive sequence infix operators not suppressed by the
Matchfix or Prefix/Postfix Rules must permit all operators in
the sequence but one to be suppressed so that the sequence of
suppressed infix operators before the unsuppressed operator can be
interpreted as a sequence of postfix operators,
and the sequence of suppressed infix operators after the
unsuppressed operator can be interpreted as a sequence of prefix operators.
If there is not a single unique way of doing this, it is an error.

Thus if `\verb|+|' is both a prefix and infix operator,
the expression `\verb|x + + y|' is equivalent to `\verb|x + (+ y)|'.
But if `\verb|+|' were a prefix, infix, and postfix operator, this
expression would have erroneous ambiguity.
...
used to parse its operand subexpressions.  See the section on operator
control below (\ref{OPERATOR-CONTROL}).

\subsubsubsection{Precedence}\ttmindex{precedence}{of operator}
...
only infix operators of precedence $P$ are considered when parsing the
expression.  In this case prefix and postfix operators are not considered
in parsing the expression.  However, after implied parentheses
...
\subsubsubsection{Associativity}\ttmindex{associativity}{of operator}
An infix operator has an {\em associativity} that
a sequence of lexemes.
...
all the operators that appear must have the same associativity, and this
associativity must not be {\tt none}\ttmindex{none}{associativity}.

If all operators are {\tt left} associative%
\index{left associative@{\tt left} associative},
all the infix operators but the rightmost are suppressed,
...
all the infix operators but the leftmost are suppressed,
so it is as if operators to the right had
higher precedence than those to the left.

If all operators are of \key{named associativity}, all are recognized, and 
...
\subsubsubsection{Control}\ttmindex{control}{of operator}
...
{\bf operators}\index{operators flag@{\tt operators} flag}\\
{\bf marks}\index{marks flag@{\tt marks} flag}\\
{\bf text}\index{text flag@{\tt text} flag}\\
At most one of these flags may be given.  These flags push a new
parsing context into the parsing context stack, parse the operands
of the current expression, and then pop the parsing context stack.
They are usually used only for matchfix operators, but may be used
for other operators.

The {\tt operators} flag turns on
operator, reorder mark(\verb|<:>|),
remainder mark(\verb|::>|),
and qualifier mark(\verb|@@@|) parsing, and turns off text parsing.

The {\tt marks} flag turns on parsing of
reorder marks(\verb|<:>|), remainder marks(\verb|::>|),
and qualifier marks(\verb|@@@|), and turns off operator and text parsing.

The {\tt text} flag turns on
text parsing, and turns off operator, reorder mark(\verb|<:>|),
remainder mark(\verb|::>|), and qualifier mark(\verb|@@@|) parsing.
...
\subsubsection{Operator Context}
