...
\key{primary-lexeme}
	...
	\end{tabular}
	\\[1ex]
\key{primary-word} ::= {\em word-character} {\em word-character}$^\star$ \\[1ex]
...
The following sections give rules involving primary lexemes.

\subsubsection{Matching and Context}
\label{MATCHING}
...
\mkey{context}{of matched lexemes} of a pair $P$ of matched lexemes
...
Several characters in primary lexemes are ambiguous in the
...
A {\em format-separator-character} must be in the context of
...
{\em White-space} lexemes are only primary lexemes; they are deleted
when primary lexemes are converted to lexemes.  The rules that
...
The sequence of primary lexemes is converted to a sequence of lexemes
...
The rule for spliting a {\em primary-word} into a {\em word}
...
\key{real-number} ::= {\em unsigned-real-number} $|$ {\em signed-real-number}
		      \\[1ex]
\key{signed-real-number} ::= {\em sign} {\em unsigned-real-number} \\[1ex]
\key{sign} ::= \verb|+| $|$ \verb|-| \\[1ex]
\key{unsigned-real-number} ::= {\em unsigned-decimal}
		$|$ {\em unsigned-decimal} {\em exponent} \\[1ex]
\key{unsigned-decimal} ::= {\em unsigned-integer}
		$|$ {\em unsigned-integer} {\em fraction}
		$|$ {\em fraction} \\[1ex]
\key{fraction} ::= \verb|.| {\em unsigned-integer} \\[1ex]
\key{unsigned-integer} ::= {\em integer-digits}
		       $|$ {\em unsigned-integer} \verb|,| {\em integer-digits}
		       \\[1ex]
\key{integer-digits} ::= {\em integer-digit} {\em integer-digit}$^\star$
			 \\[1ex]
\key{integer-digit} ::= {\em digit}%
			$|$\verb|A|%
			$|$\verb|B|%
			$|$\verb|C|%
			$|$\verb|D|%
			...
			$|$\verb|f|
			\\[1ex]
\key{exponent} ::= {\em radix-indicator}
		$|$ {\em radix-indicator} {\em integer} \\[1ex]
\key{radix-indicator} ::= {\em decimal-radix-indicator} \\[1ex]
\key{radix-indicator} \begin{tabular}[t]{rl}
		     ::= & {\em binary-radix-indicator} \\
		     $|$ & {\em octal-radix-indicator} \\
		     $|$ & {\em decimal-radix-indicator} \\
		     $|$ & {\em hexadecimal-radix-indicator} \\
		     \end{tabular} \\[1ex]
\key{binary-radix-indicator} ::= \verb|B| $|$ \verb|b| \\[1ex]
\key{octal-radix-indicator} ::= \verb|O| $|$ \verb|o| \\[1ex]
\key{decimal-radix-indicator} ::= \verb|D| $|$ \verb|d|
			      $|$ \verb|E| $|$ \verb|e| \\[1ex]
\key{hexadecimal-radix-indicator} ::= \verb|X| $|$ \verb|x| \\[1ex]
\key{integer} ::= {\em unsigned-integer} $|$ {\em signed-integer} \\[1ex]
\key{signed-integer} ::= {\em sign} {\em unsigned-integer} \\[1ex]
...
Some lexemes defined according to the above equations to be {\em numbers}
are not in fact {\em numbers}.  To be a {\em number}, a lexeme must
pass the following additional test:

{\bf Number Digit Rule.}\index{Number Digit Rule}
{\em Integer-digits} that are not {\em digits} are not permitted in a
{\em number} unless a {\em hexadecimal-radix-indicator} is present
in the {\em number}.

Note that decimal points and commas in a {\em number}
must be followed by a digit.
Also, although the hexadecimal digits \verb|A|-\verb|F| will be prohibited
from non-hexadecimal numbers, digits outside the range of the radix are
not prohibited from binary or octal numbers.  Use of such digits will
result in an erroneous lexeme,
but would not be an error of lexeme formation.
Similarly, incorrect numbers of digits between commas will result in an
erroneous lexeme, but will not be an error of lexeme formation.

\subsubsection{Ambiguous Separators}

Informally, an ambiguous separator is a one character separator that immediately
follows a word, and could be part of the word.  Examples are the period
and the exclamation point.  The Ambiguous Separator Rule
given below tells when to split a {\em primary-word} that ends with an
ambiguous separator character into a {\em word} and a one character
...
\key{separator} ::= {\em format-separator}
		$|$ {\em ambiguous-separator} \\[1ex]
\key{ambiguous-separator} ::= {\em ambiguous-separator-character} \\[1ex]
\key{ambiguous-separator-character} ::=     \verb|!|
				        $|$ \verb|?|
			                ...
			                $|$ \verb|.|
					\\[1ex]
\key{primary-word} ::= {\em word}
		$|$ {\em word} {\em ambiguous-separator}
\end{indpar}

The following rule controls when to split a {\em primary-word}
into a {\em word} and an {\em ambiguous-separator}.

{\bf Ambiguous Separator Rule.}\index{Ambiguous Separator Rule}
A {\em primary-word} that ends with an {\em ambigous-separator-character}
is split into a {\em word} and an {\em ambiguous-separator} if the {\em word}
does \underline{not} contain a copy of the {\em ambiguous-separator} character,
or if the {\em ambiguous-separator} is a period
and the {\em word} is a {\em number}.
Otherwise the {\em primary-word} is not split and becomes a {\em word}.
...
Primary-Word		& \multicolumn{2}{@{}l@{}}{Lexeme Sequence}
							& Splits?	\\[1ex]
