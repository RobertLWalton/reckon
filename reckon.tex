...
on a board of size S ~ with square size SQSIZE ?
            <-- integer S, max ship length <= 
                SQSIZE :=? 0.25in,
                number SQSIZE, 0.05in <= SQSIZE:
...
for every board:
    ...
    on make display for (THIS):
        owner plane = a plane with depth 3
        ...
        while rc < size:
            ...
            next rcd = rcd + SQSIZE
            next rc = rc + 1
        opponent plane = copy owner plane
    ...
    on display ( THIS ) for PLAYER <--
            ...
            PLAYER :=? owner:
        if owner plane == none:
            make display for ( THIS )
        if PLAYER == `owner':
            value = owner plane
        else:
            value = opponent plane
\end{verbatim}
