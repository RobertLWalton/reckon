...
\verb|+                overlap (| \\
\verb|+                  <<c>>,| \\
\verb|+                  <<x-axis>> labelled `X Axis',| \\
\verb|+                  <<y-axis>> labelled `Y Axis',| \\
\verb|+                  <<line>>,| \\
\verb|+                  <<arc>> labelled `<<theta>>' ),| \\
...
\ikey{LEXEME-DICTIONARY-DEFINITION}{lexeme-dictionary-definition} \\
	\hspace*{0.5in}::= \begin{tabular}[t]{@{}l@{}l@{}}
		     ...
		     \end{tabular} \\[1ex]
\ikey{DICTIONARY-NAME}{dictionay-name} ::=
	\verb|[|
...
placing cancelling undefinitions in the parsing stack.  These have the
syntax:
...
This dictionary also protects irregular singluar forms that might be
...
can be a singluar word refering to a group of people, but PCASL will
...
parsing stack changes to produce anomolous seeming results.  For
...
recoginized by text processing, while
...
\subsection{Paragraph and Sentence Parsing}
\label{PARAGRAPH-AND-SENTENCE-PARSING}

If the text being parsed does not contain any format separators,
the text is lexically just a string of non-whitespace lexemes
which is parsed into a list of paragraphs and sentences.  Paragraphs
are separated by blank lines, and sentences end with sentence
terminators, such as the period or question mark.
...
In use, the {\em pattern} is call unified (\secref{CALL-UNIFICATION})
