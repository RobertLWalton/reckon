...
the default is to create an immutable context set whose expression defintions
...
code may execute in that iteration to give the component a value.
...
that is part of the code of an object.
The method is callable from code inside or outside the block.
...
When the method definition is called, the method code block executes
in a particular object.  More specifically, the method code is added
temporarily to the object, as for the {\tt update} function
(\pagref{UPDATE}).

If a method definition has an argument named \ttkey{SELF}, then when the
definition is called, that argument becomes the object in which
the method code executes.  This argument must have the same type as
the object whose code contains the method definition.
If there is no {\tt SELF} argument, a new method of the right type
is created by the method execution, and the variable name
{\tt SELF} may be used in the method code to refer to this object,
even though there is no argument with this name.
Usually {\tt SELF} is returned as the value of the execution of such
a method, but the language does not require this.
...
and output side effects do not execute but are instead put on a
{\tt todo} list.
