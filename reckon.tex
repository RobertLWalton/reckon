...
(\secref{BRACKET-DEFINITIONS}).

The subexpressions identified by this rule have explicit
brackets.  Below we introduce subexpressions with operators that have implicit
brackets.
...
The brackets that surround a subexpression
...
\begin{tabular}{l@{~~~~~~}l@{~~~~~~}l@{~~~~~~}l}
	& With      &           & Sets \\
In	& Parsing   &		& Parsing \\
Parser	& Selector  & Brackets  & Selectors To \\[1ex]
{}[any] & -	    & \verb|(|\ldots\verb|)|
		                & [no change] \\
{}[any]	& -	    & \verb|{|\ldots\verb|}|
				& operator \\
operator & operator & \verb|[|\ldots\verb|]|
				& list \\
operator & list	    & \verb|[|\ldots\verb|]|
		    		& list \\
text    & text      & \verb|[|\ldots\verb|]|
				& operator \\
{}[any]	& -	    & \verb|`|\ldots\verb|'|
		    		& text     \\
{}[any]	& -	    & \verb|``|\ldots\verb|''|
...
A summary of the behavior of parsers is:
...
During parsing, parsing selectors are used to determine
which definitions in the parsing stack are used.  The `operator'
parser standardly runs with either the `operator' or `list' parsing selectors.
In the former case, all operators and separators
are standardly recognized, whereas
in the later case, only separators are standardly recognized.
The text parser standardly runs with the `text' selector.
...
but not sentences, etc.  Both parsers recognize brackets and do lexical
...
contains definitions of parsers, brackets, operators, lexeme replacements,
and unit specifiers.
...
a sequence of one or more words used to name sets of parsing definitions.
A \key{parsing selector set}
is a set of parsing selectors that names the set of parsing stack definitions
...
set determined by the bracket definition may be pushed into the parsing
...
Some of the standard brackets affect evaluation, as is indicated
...
Also note that brackets that turn evaluation off (or on)
