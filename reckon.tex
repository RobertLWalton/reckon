...
These lists are then further parsed to find operators, separators,
...
is an object, and more specifically, its elements, which are atoms or
other objects, and the following attributes: \TT{.type}, \TT{.initiator},
\TT{.terminator}, \TT{.separator}.
...
Parsing at this point is a top down process.  First operators and
subexpressions are discovered, as part of operator parsing.  Subexpressions
are then parsed independently (except for context, see below),
and then a reformatter is invoked to reorganize the object.

A subexpression may not have any operators.  In this case other kinds of
parsing are applied to it.  


However, as a last step in parsing, atoms are dequoted.

{\em Bracketted lists} are parsed after they have been read.  There are
several types of parsing as determined by the brackets:

\begin{center}
\begin{tabular}{l@{\hspace*{0.3in}}l@{\hspace*{0.3in}}l@{\hspace*{0.3in}}l}
\bf Brackets & \bf Selectors & \bf Context Selectors & \bf Usage \\[1ex]
\TT{\{*\ldots*\}} & comment & all & comment \\
\TT{`\ldots'} & text & all but comment & text \\
\TT{\{+\ldots+\}} & operator, code & all but comment & unevaluted code \\
\TT{\{\{\ldots\}\}} & operator & all but comment & unevaluated expression \\
\TT{[\ldots]} & operator & text & evaluated expression in text \\
\TT{[\ldots]} & inherited & all but comment and text & subexpression \\
\TT{(\ldots)} & inherited & all but comment & subexpression \\
\TT{(|\ldots|)} & inherited & all but comment & subexpression \\
\end{tabular}
\end{center}
