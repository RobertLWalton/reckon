...
\begin{verbatim}
for every board:
    on a board of dimension X <--:
        size = X
        size-of-value-array = (X,X)
        foreach subscript (I,J):
            value (I,J) = `empty'
    on build from (I1,J1) to (I2,J2) <--:
        dI = I1 - I2
        dJ = J1 - J2
        if (     abs(dI) != abs(dJ)
             AND dI != 0
             AND dJ != 0 ):
            value = error `illegal build XXX'
        type = max ( abs(dI), abs(dJ) )
        if ( dI > 0 ): dI = 1
        else if ( dI < 0 ): dI = -1 
        if ( dJ > 0 ): dJ = 1
        else if ( dJ < 0 ): dJ = -1 
        for I = I1 to I2 step dI:
...
\caption{Code for the Battleship Game}
\label{CODE-FOR-BATTLESHIP}
\end{boxedfigure}
...
Other rules of CASTLE (\secref{LEXEMES})
...
categories represent CASTLE expressions defined by expression
...
this document syntactic categories representing CASTLE {\em EXPRESSIONs}
