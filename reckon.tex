...
\emkey{natural-index}\label{NATURAL-INDEX}
    :::= \TT{0} $|$ {\em non-zero-digit} {\em digit}\STAR{}
			   ~~~ [there can be at most 15 {\em digits}]
\end{indpar}
...
$1.7976931348623158\times 10^{+308}$ are represented a $+$ or $-$
infinity, which can also be represented by the builtin constants
\TT{+INF} and \TT{-INF}: see Figure~\itemref{BUILTIN-CONSTANTS}.

Internally $+0$ and $-0$ are distinct, though in most computations
they behave the same.  On output both are represented by \TT{+0}.
...
\end{center}
\vspace*{-3ex}
\caption{Builtin Constants}
\label{BUILTIN-CONSTANTS}
\end{boxedfigure}
...
\begin{tabular}{l@{~~~}l@{~~~}l@{~~~}l}
ft		& inch	& m	& kg \\
lb*ft		& inch\CIRCUM 3    & kg*m & kg*m\CIRCUM 2 \\
...
Note that `\TT{*}' takes precedence over `\TT{/}', and `\TT{\CIRCUM}'
...
\emkey{glued-lexeme}
    \begin{tabular}[t]{rl}
    :::= & {\em scientific-numeric} \TT{\_}\QMARK{} {\em unit}
           \{ \TT{\_}\QMARK{} {\em scientific-numeric}
...
\emkey{unit-factor} ::= {\em unit-name}
    \{ \TT{\CIRCUM} {\em natural-numeric} \}\QMARK{}
\\
...
\begin{tabular}{l}
\emkey{represented-object-identifier} :::= {\em raw-object-identifier} $|$ {\em cooked-object-identifier} \\
...
\emkey{number-name} \begin{tabular}[t]{rl}
    ::= & {\em integral-numeric} $|$ {\em fractional-numeric} $|$
          {\em scientific-numeric} \\
    $|$ & {\em radix-numeric} $|$ {\em date} $|$ {\em time}
    \end{tabular} \\
...
handle operators, numeric units, and possessives.  For a
...
\emkey{atom}\label{ATOM-REPRESENTATION}
    ::= {\em atom-name} \\[1ex]
\emkey{object}\label{OBJECT-REPRESENTATION}
    ::= {\em bracketed-list} with \TT{\{|~|\}} possibly omitted:
                                                             see text
\end{indpar}
...
\item Asserts identity between two objects which have equal
\TT{.initiator} and \TT{.ter\-min\-a\-tor} attributes, but no other
non-list-element attributes,
and one or both of the objects has a single
object list element that is a syntax variable.
\end{enumerate}
...
Thus the following syntax equations are consistent:

\begin{center}
\begin{tabular}{r@\TT{~:=:~}l@{~~~~by Consistency Rule (}l@{)}}
\verb/5 + 6/ & \verb/$X/ & 1 \\
\verb/5/ & \verb/6/      & 2 \\
\verb/[ 5 6 7 ]/ & \verb/` A B C '/      & 3 \\
\verb/[ 5 6 7 ]/ & \verb/[ $X ]/         & 4 \\
\end{tabular}
\end{center}
...
A syntax expression object in a syntax equation
whose \TT{.initiator} is \TT{"("} and whose
...
\begin{center}
\begin{tabular}{r@\TT{~:=:~}l@{~~~same~as~~~}r@\TT{~:=:~}l}
\verb/5 + 6/ & \verb/( 8 + 9 )/ & \verb/5 + 6/ & \verb/8 + 9/ \\
\verb/5/ & \verb/{| 5 |}/ & \verb/{| 5 |}/ & \verb/{| 5 |}/ \\
\verb/5/ & \verb/( 5 )/ & \verb/{| 5 |}/ & \verb/{| 5 |}/ \\
\end{tabular}
\end{center}

The following rule is applied when two objects are consistent: 

\begin{indpar}
\begin{list}{}{}
\item[\ikey{Propagation Rule}{propagation rule}:]\label{PROPAGATION-RULE}
From a consistent syntax equation asserting the identity
of two objects new equations are deduced as follows:
\begin{enumerate}
\item
If consistency was deduced according to Consistency Rule (3),
...
identical.
\item
If consistency was deduced according to Consistency Rule (4),
a new equation is deduced by deleting from the syntax equation
all \TT{.initiator} and \TT{.terminator} attributes and replacing
any list of a single syntax variable by the syntax variable itself.
\end{enumerate}
\end{list}
\end{indpar}
...
since both occurances of \TT{\$X} are identified by
Restricted Transitivity (b).  Lastly, if
we started instead with:
...
\emkey{pattern-equation} ::= {\em syntax-expression} \ttkey{:=\TILDE}
			    {\em pattern}
\\[1ex]
\emkey{pattern} ::= {\em object} [see \pagref{OBJECT-REPRESENTATION}]
		    \begin{tabular}[t]{l}
		    whose list portion is a {\em pattern-list} \\
		    and whose attribute values are
		    {\em pattern-factors} \\
		    \end{tabular}
\\[1ex]
\emkey{pattern-list} ::=
    \begin{tabular}[t]{l}
    {\em pattern-term}\QMARK{}
...
\begin{enumerate}
\item An unfolding of a {\em pattern} is a sequence of
{\em pattern-clauses} from the {\em pattern} taken in any order,
\underline{except} the first {\em pattern-clause} in the
{\em pattern} must be first in the unfolded {\em pattern}.
All {\em pattern-qualifiers} are removed from the {\em pattern-clauses}
...
and this may be omitted in the unfolded {\em pattern}.
...
\item An attribute that does \underline{not} have the \TT{?} flag
must appear exactly once in the unfolded {\em pattern}.

\item An attribute that \underline{does} have the \TT{?} flag
and must appear exactly once or not at all
in the unfolded {\em pattern}.

\end{enumerate}

There are also some extra rules for {\em pattern} syntax:

\begin{enumerate}

\item The first {\em pattern-clause} of a {\em pattern}
may not have any {\em pattern-qualifier} other than `\TT{...}'.
Thus the first {\em pattern-clause} may not be omitted in
any unfolding of the {\em pattern}.

\item Every {\em pattern-clause} must contain a {\em pattern-factor}
that is either an {\em atom} or a {\em pattern} that is a
{\em bracketed-list} with an \TT{.initiator} other than `\TT{(}'.

\end{enumerate}

Pattern equations are roughly similar to syntax equations but
obey significantly different rules:

\begin{indpar}
\begin{list}{}{}
\item[\ikey{Pattern Consistency Rule}{pattern consistency rule}:] ~\\
A pattern equation is \key{consistent} if its {\em pattern}
has a unique unfolding such that:
\begin{enumerate}
\item if the unfolded {\em pattern} is a single atom,
the syntax expression (pattern equation left side) is an identical atom, or
\item if the unfolded pattern is an object, the syntax expression (pattern
equation left side)
is an object such that:
\begin{enumerate}
\item there is a \underline{unique}
(left to right) order preserving matching of list elements
of the pattern to list elements or list element sequences
of the object such that:
\begin{enumerate}
\item each pattern atom is matched to an identical object atom
\item each pattern bracketed list is matched to a object bracketed
list with identical \TT{.initiator} and \TT{.terminator}, except
that a pattern bracketed list with \TT{.initiator} \TT{"("} and
\TT{.terminator} \TT{")"} and no other non-list-element attributes
may be matched to a non-empty sequence of object elements after
this sequence has been made into a \TT{(~)} bracketed list
\end{enumerate}
\item and the object and the pattern have
the same non-list-element
attribute labels, and all these attributes in both object and pattern
each have exactly one value
\end{enumerate}
\end{enumerate}

\item[\ikey{Pattern Propogation Rule}{pattern propogation rule}:] ~\\
Given a consistent pattern equation with the unfolded {\em pattern}
being an object
and not an atom, a new pattern equation is deduced for every bracketed
list in the unfolded {\em pattern}.  The right side of this is
the bracketed list in the unfolded {\em pattern}, and the left side
is the bracketed list in the object this is matched to by the
consistency rule.

If the right side of any deduced pattern equation is not a {\em pattern},
the pattern equation is made into a syntax equation.

\end{list}
\end{indpar}

One way in which pattern equations differ from syntax equations
is that \TT{(~)} parentheses cannot be omitted in the pattern,
but they can be omitted in the syntax expression when they
surround a sequence of several atoms that is bounded by atoms
named in the pattern.  For example,
...
\begin{center}
\begin{tabular}{rcl}
\verb/sum (1+1) and (5+5)/ & \TT{:=\TILDE}
			      & \verb/sum ($X) and ($Y)/ \\
\verb/$X/ & \tt :=: & \verb/(1+1)/ \\
\verb/$Y/ & \TT{:=:\TILDE} & \verb/(5+5)/ \\
\verb/X/ & \tt :=: & \verb/2/ \\
\verb/Y/ & \TT{:=:\TILDE} & \verb/10/ \\
\end{tabular}
\end{center}
...
to create a new expression definition whose pattern, qualifiers, and block
...
\emlkey{pattern}{-assignment-statement} ::=
	{\em pattern} \TT{\TILDE=\TILDE}\index{~=~@{\TT{\TILDE=\TILDE}}}
	{\em right-side} \\[1ex]
