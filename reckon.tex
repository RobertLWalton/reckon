...
\emkey{pattern} \begin{tabular}[t]{@{}rl}
		...
		\end{tabular}
\\[1ex]
\emkey{pattern-object} ::= {\em object} [see \pagref{OBJECT-REPRESENTATION}]
		           \begin{tabular}[t]{l}
			   all of whose attributes have single values, \\
		           ...
		           \end{tabular}
\\[1ex]
\emkey{pattern-indicator} ::=
	{\em pattern-separator} $|$ {\em pattern-qualifier}
\\[1ex]
\emkey{pattern-list} ::=
    \begin{tabular}[t]{l}
    {\em pattern-term}
...
is a {\em pattern-factor} that is an {\em atom-name} in the clause, an
{\em atom-name} that is in one of the replacement values
for an {\em alternative-syntax-variable} that is in the clause,
or a {\em bracketed-list} in the clause
whose \TT{.initiator} is not \TT{"("}.
An `\mkey{initial key}{of a pattern clause}' is a key of the clause that
can be the first key of the clause (this includes the first atoms of
{\em alternative-syntax-variable} replacements if the
{\em alternative-syntax-variable} appears before any other key in the clause).
An `\mkey{initial key}{of a pattern}'
of a {\em pattern} is any initial key of one of its {\em pattern-clauses}.
Given this, we may state some extra rules for {\em pattern} syntax:
...
\TT{.initiator} and \TT{.terminator} values.  The pattern has every
non-list-element attribute that the syntax expression has.
The syntax expression has every non-list-element attribute that
...
initial key of the original folded pattern, and no element that is a syntax
variable, and the sequence is
...
\item
Its right
side is a {\em pattern-object} that
...
When a pattern equation is consistent, the following are applied
to deduce new pattern equations or syntax equations.
