...
is a directed possibly cyclic graph with labels on all arcs and on some
nodes, plus a set of graph node valued variables.  Labels and variable
names must be single words or
separators.  Labelled nodes cannot be the source of any arc.
Several arcs with the
same source node may have the same arc label.
...
of more than one arc, the graph is a tree.  In this case it is called
...
\newcommand{\Gleaf}[1]{{\tt #1}}
\newcommand{\Glabel}[1]{\fbox{\tt #1}}
\newenvironment{Graph}%
    {\[ \left[ \begin{array}{l}}%
    {\end{array} \right] \]}
\newenvironment{Gchild}[1]%
    {{\tt #1}{\tt :} \left[ \begin{array}{l}}%
    {\end{array} \right]}
