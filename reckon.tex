...
\begin{indpar}
\key{LEXEME-DEFINITION}\,\footnote{Capitalized syntactic
...
\key{UNIT-SPECIFIER-DEFINITION} \\
	\hspace*{0.5in}::= \verb|define unit specifier|
	    ...
	    \end{tabular} \\[1ex]
\key{UNIT-SPECIFIER-FIXITY}
	\verb|[| \ttkey{prefix} \verb|]|
	$|$ \verb|[| \ttkey{postfix} \verb|]| \\[1ex]
\key{UNIT-SPECIFIER-NAME}
...
\begin{indpar}
\key{UNIT-SPECIFIER-UNDEFINITION} \\
...
\key{PARSER-DEFINITION} \\
	\hspace*{0.5in}::= \verb|define parser| {\em PARSER-NAME} \\[1ex]
\key{PARSER-NAME} ::=
	\verb|[| {\em word} {\em word}\,$^\star$ \verb|]|
\end{indpar}
...
\begin{indpar}
\key{PARSER-UNDEFINITION} ::= \verb|undefine parser| {\em PARSER-NAME-option}
	\\[1ex]
{\em PARSER-NAME-option} ::= {\em empty} $|$ {\em PARSER-NAME}
\end{indpar}
...
in the stack.  The optional {\em PARSER-NAME}, if present,
must match the name of the canceled parser, and is only used for error
checking.
...
and the \key{text parser}, \ttnbbkey{TEXT PARSER},
...
When a subexpression is surrounded by brackets, the barckets
