...
If an expression is to be examined for operators, \verb|matchfix|
operators are looked for first, \verb|infix| operators second,
and \verb|prefix| and \verb|postfix| operators are looked for
simultaneously third.

\subsubsubsection{Lexemes}\ttmindex{lexemes}{of operator}
...
of lexemes must begin with the {\em opening-mark} that begins a subexpression
and the closing sequence of lexemes must end with the {\em closing-mark}
that ends the subexpression.  An example sequence of lexemes for a
{\tt matchfix} operator is `\verb/[ | ... | ]/' that permits subexpressions
like `\verb/[| x - 5 |]/'.

Two different operators may have the same sequence of lexemes provided
they have different fixities and neither is \verb|matchfix|.  For example,
`\verb|-|' is the lexeme sequence of a {\tt prefix} operator and also of
a distinct {\tt infix} operator.  No three operators may have the same
set of lexemes.
...
than X.  For example,
\verb|[ ... ]| is more general than \verb/[| ... |]/.  
