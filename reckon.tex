...
a left to right spelling scan to produce a final set of lexemes
...
Decimal numbers are sequences of digits with an optional decimal point
and an optional exponent.
Decimal ratios are sequences of digits separated into a numerator
and a denominator by a `\verb|/|'.
The syntax equations defining {\em decimal-numbers} and
{\em decimal-ratios} are as follows.
...
\key{radix-number-mark} ::= {\em digit} $|$ {\em letter}
                      $|$ \verb|.| $|$ \verb|,|
\end{indpar}
...
B\verb|#|10110100\verb|#|	& O\verb|#|77534201\verb|#|
	& D\verb|#|19758\verb|#|	& X\verb|#|FE8A932B\verb|#| \\
B\verb|#|101101\verb|#|		& O\verb|#|0.7753\verb|#|
	& D\verb|#|197.58\verb|#|	& X\verb|#|0.FE8A932B\verb|#| \\
B\verb|#|10,1101\verb|#|	& O\verb|#|12,3456\verb|#|
	& D\verb|#|0.123,5\verb|#|	& X\verb|#|FE8A,932B.7CCD,83\verb|#| \\
\end{tabular}
\end{indpar}
...
separated by a slash (`\verb|/|'), with an optional prefix sign.

\begin{indpar}
\key{ratio} ::= {\em unsigned-ratio}
	    $|$ {\em sign} {\em unsigned-ratio} \\[1ex]
\key{ratio} ::= {\em numerator} \verb|/| {\em denominator} \\[1ex]
...
123\verb|^|0 & -123\verb|^|+2 & -0.123,456e-3 & -1,234.567890\verb|^|6 \\
X\verb|#|A\verb|#|e0 	& B\verb|#|1011\verb|#|e-3
	& O\verb|#|0.7753\verb|#|e-5
	& X\verb|#|0.FE8,A93,2BE\verb|#|\verb|^|+5 \\
\end{tabular}
\end{indpar}
...
\key{unit-number} \begin{tabular}[t]{rl}
		  ::= & {\em pre-unit-indicator} {\em base-number} \\
		  $|$ & {\em sign} {\em pre-unit-indicator}
		  		   {\em unsigned-base-number} \\
		  $|$ & {\em base-number} {\em post-unit-indicator}
		  \end{tabular} \\[1ex]
\key{base-number} ::= {\em decimal-number} $|$ {\em ratio} \\[1ex]
\key{unsigned-base-number} ::= {\em unsigned-decimal-number}
...
Expression matching treats call nodes differently than unification
does.  A \key{call node} is a node that has
an \mkey{arity arrow}{of call node}.
That is, it is the source of an arrow labeled
\ttmkey{-ARITY-}{of call node}.  More specifically, expression
matching replaces the node merge algorithm
...
are natural numbers and $m<n$.  In either case $n$ is called the
\key{maximum arity} of the call node N.
...
arrow, the rest argument of N is a node with no label
...
is a variant of expression unification (\ref{PURE-UNIFICATION})
...
described in \ref{PURE-UNIFICATION} except the merge set is initialized
