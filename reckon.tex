...
paragraph if the current paragraph type,
...
line ending `\TT{:}' also has type \TT{\{code\}}, unless the
`\TT{:}' is preceeded by special indentation marks.
...
\begin{center}
\begin{tabular}{ll@{\hspace*{1in}}ll}
\emkey{bcp}	&	begin code paragraph &
\emkey{ecp}	&	end code paragraph \\[0.5ex]
\emkey{btp}	&	begin text paragraph &
\emkey{etp}	&	end text paragraph \\[0.5ex]
...
with the brackets becoming the \TT{.initiator} and \TT{.terminator}
...
\emkey{bracketed-list}\label{BRACKETED-LIST}
    ...
    $|$ & \TT{\{:} {\em prefix-0-list} \TT{:\}} &
    $|$ & \TT{\{\{} {\em prefix-0-list} \TT{\}\}} \\
    $|$ & \TT{\{} {\em m-type} {\em prefix-0-list}
                               {\em reflected-m-type} \TT{\}} \\
...
\emkey{reflected-m-type} ::= reflection of the matching {\em m-type}
                             [see text] \\
\end{tabular}
