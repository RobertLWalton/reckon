...
{\em WITH-CONTEXTS-SET-\EOL BOOL\-EAN} is either `{\tt false}' or `{\tt true}'
...
object.  Each iteration replaces the previous iteration as the object
...
is implied for every {\em component-name} of an object that is not
defined by an explicit `{\tt default next}' statement in the current
(permanent plus temporary) code of the object.
Thus components not explicitly changed propagate unchanged from
one iteration of an object to the next.
The statement
...
the next object iteration is created, in which case other permanent
code may immediately execute to define the object.

TBD: what does the last sentence mean?
...
Operations that have side effects: input, output, object update
(which is like object IO).
