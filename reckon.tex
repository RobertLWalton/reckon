...
or after units and `{\tt +}' operators with parentheses are inserted
between consecutive numbers, to produce from the input the
following string of 12 lexemes:
...
and {\em horizontal-space-characters} cannot be deleted by the rules.
...
are not distinguishable from explicit lexemes.  The indentation
lexemes are the post-separator
...
After pre-lexemes are scanned to produce lexemes, lexemes that are
not in the lexical context of a pair of matched {\em quotes} are
...
Note that lexical parsing is \underline{not} performed on lexemes that
are in the lexical context of a pair of matched {\em quotes}.
