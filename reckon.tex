...
In this case the first four equations are consistent and the fifth is not.
...
[TBD: $e_2$ here denotes a unique subexpression of the original
expressions, and not a surface subexpression.]
...
Note that two subexpressions of the previous equations that are syntactically
identical by accident are not equated by this algorithm.  Thus given

\begin{center}
\verb/sort ($X, $Y) :=: sort ( 5 + 6, 5 + 6 )/
\end{center}
...
but \underline{not} \verb|($X) := ($Y)|, even though this is consistent for
this specific case.

Also note that as a technical point, if all we want is to deduce all
syntax equations for which one side is a syntax variable, then we can
...
Proof Sketch: Let S~=~all syntax equations deducible from Propagation, Symmetry,
and Restricted Transitivity.  Assume S is consistent.  Let S'~=~all
syntax equations deducible from S and (unrestricted) Transitivity.
Then prove by induction on the size of the syntax equations
that (1) S' is consistent, and (2) S' is closed under Propagation,
Symmetry, and (unrestricted) Transitivity.
\end{quote}
