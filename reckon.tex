...
name, they must begin and end the name.
...
must have the same associativity, and this
...
An operator wrapper is an expression containing the lexeme `\verb|...|'.
If operator selection for an expression selects an operator with a
wrapper, the expression is restructured to be a copy of the wrapper
with its `\verb|...|' lexeme replaced by the parenthesized expression
being restructured.
...
If operator sepelction for an expression selects several infix operators,
and one has a wrapper, all must have that same wrapper, else it is an error.
...
than one operator selected, it is an error, and if there are no operators
selected, the expression is not restructured.

The operator selection rule uses the current parsing definition stack
to determine which operators are defined.  The important parts of each
...
\item
If more than one matchfix operator is still selected, it is an error.
...
matchfix operator, then it is an error.
...
at this point, which selecting any other operators.
...
\item
Deselect any selected infix operator that overlaps or immediately follows
a prefix sequence that begins the expression.
Deselect any selected infix operator that overlaps or immediately precedes
...
For example, in the expression `\verb|+ x * y|' the `\verb|+|' is
...
It is an error if the is an equivalence class of conflicting operators all
...
\item
It is an error if two conflicting infix operators are still selected at
this point.
...
\item
Select any prefix operator that begins the expression, and any postfix
operator that ends the expression.
...
\item
If more than one operator is selected at this point, it is an error.
...
used to parse its operand subexpressions.  See the sections on operator
control flags (\ref{OPERATOR-CONTROL-FLAGS}),
operator subdefinitions (\ref{OPERATOR-SUBDEFINITIONS}),
and operator parsers (\ref{OPERATOR-PARSER}) above.
