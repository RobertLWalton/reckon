...
\key{variable-assignment-statement} ::=
	{\em variable-name} \ttkey{=} {\em right-side} \\[1ex]
\key{variable-name} ::=
	{\em word} \{ {\em word} $|$ {\em separator} \}$^\star$ \\[1ex]
\key{right-side} ::= {\em expression} \\[1ex]
\key{pattern-assignment-statement} ::=
	{\em assignment-pattern} \ttkey{\Tilde\Tilde} {\em right-side} \\[1ex]
\key{assignment-pattern} ::= {\em expression} \\[1ex]
\key{guarded-statement} ::=
      {\em if-statement} {\em else-if-continuation}$^\star$
      $|$ {\em else-continuation-option}\\[1ex]
\key{if-statement} ::=
      \ttkey{if} {\em guard} {\em guarded-subblock} \\[1ex]
\key{else-if-continuation} ::=
      \verb|;| \ttkey{else if} {\em guard} {\em guarded-subblock} \\[1ex]
\key{else-continuation-option} ::= {\empty} $|$
      \verb|;| \ttkey{else} {\em guard} {\em guarded-subblock} \\[1ex]
\key{guard} ::= {\em expression} \\[1ex]
\key{guarded-subblock} ::= {\em block}
\end{indpar}
...
a different sense: see \secref{BLOCK-VARIABLES}).
...
{\em variable-name} is a subexpression consisting of a sequence of one or
more {\em words}, but not all such subexpressions are {\em variable-names}.
Some {\em variable-names} may be \mkey{inherited}{variable-name}
from the statement containing the block.
Some are left sides of {\em variable-assignment-statements}.
...
sides of {\em assignment-statements}.

When identifying {\em variable-names} in a {\em block}, {\em statements} in a
{\em guarded-subblock} of the {\em block} are treated as if they
were in the {\em block} proper.  Thus variables named in the
{\em guarded-subblock} are variables named in the {\em block}.
...
Pattern variable names are not permitted to begin with
\ttmkey{\#}{pattern variable name}.
...
is evaluated only when all ths variables it contains have values.
When the {\em right-sides} of {\em assignment-statements} are evaluated,
the variables named on the left sides of the statements are given values.

None of the expressions or statements in an {\em if-statement}
{\em guarded-block} are
evaluated until the block's {\em guard} has been evaluated to true;
if the guard evaluates to false, no expression or statement in the
...
Then if no variable that could be assigned by the {\tt default}
...
where \verb|X| is an inherited variable name.  If \verb|X > 5| is true,
the value of the block is `{\tt yes}' and the {\tt default}
...
the value of the block is `{\tt no}'.
...
variable and if it does execute a {\tt next} qualified
{\em assignment-statement}.  The sequence of blocks are called the
...
the {\em statement} becomes visible with the rest of the {\tt default}
{\em statements} if none of the variables that can be assigned by the
{\em statement} have values for the next iteration.
...
variables are normally propogated unchanged from on iteration to the
next.
...
\begin{center}
\verb|next| {\em variable-name} = {\em variable-name}
\end{center}

for every {\em variable-name} that can be assigned by the
{\tt once} qualified {\em statement}.  The intention is that the
{\tt always} qualified {\em statement} will assign values on the first
iteration and these values will then be propogated to subsequent iterations.
