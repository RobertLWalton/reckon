...
is implied for every inherited {\em variable-name}.  Thus inherited
variables are normally propagated unchanged from one iteration to the
next.
...
When code becomes part of a block value, the code executes as much as it
can to define variable values that are then components of the block value.
When the code can execute no further (\secref{BLOCK-EVALUATION}), any
...
TBD:  next default v = v implied for permanent code variables

TBD: next x = nothing causes x to become undefined for the next iteration
