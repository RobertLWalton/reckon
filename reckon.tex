...
`{\tt all persons}' denotes the list of all persons.
...
For this reason PCASL lists are sometimes called `\skey{flat list}s'.
...
For example, the input `{\tt x = 7' 5.5";}' contains the pre-lexeme
`{\tt 5.5";}', which is split into 3 lexemes `{\tt 5.5 " ;}', and
then during lexical parsing numbers and units are grouped
using parentheses to produce from the input the following string of 9 lexemes:
...
categories represent PCASL expressions defined by expression
definitions (\secref{EXPRESSION-DEFINITIONS}).}
		     \begin{tabular}[t]{rl}
...
\begin{indpar}
\key{number-unit-group} ::= {\em number-unit-pair}\,$^\star$ \\[1ex]
...
Lexical parsing recognizes \skey{unit-number-group}s and places
parentheses around them.  This is done after lexeme replacement
(\secref{LEXEME-REPLACEMENT}).

TBD: This needs to be done during expression parsing.
...
this document syntactic categories representing PCASL {\em EXPRESSIONs}
...
\begin{tabular}{l@{~~~~~~}l@{~~~~~~}l@{~~~~~~}l@{~~~~~~}l}
		...
		& operators		& \verb/|/	& \verb|<:> ::> ::?| \\
Brackets	& and qualifiers	& text		& \verb|@@  ??|
								& eval \\[1ex]
...
qualifier & \ref{QUALIFIER-DEFINITION} \\
qualifier shortcut & \ref{QUALIFIER-SHORTCUT-DEFINITION} \\
...
\begin{indpar}
\ikey{PARSER-UNDEFINITION}{parser-undefinition} \\
	\hspace*{0.5in}::= \verb|undefine parser| ~ {\em PARSER-NAME}
\end{indpar}
...
can only occur in a pattern (\secref{PATTERN}).
...
define operator FIXITY NAME
       ...
       <-- missing(PRECEDENCE) or integer(PRECEDENCE)
\end{verbatim}
\end{indpar}
...
However, in PCASL an operator can change the parsing stack
used to parse its operand subexpressions, and this allows PCASL to
to use arbitrary grammars for subexpressions.\footnote{In practice
...
permitting matchfix operators to change grammars.  But PCASL currently
...
For example, in the expression `\verb|+ x * y|' the infix operator
`\verb|+|' is
deselected, in the expression `\verb|+ + + x * y|' all infix `\verb|+|'s are
deselected, and in the expression
`\verb|+ / x * y|' the `\verb|/|' would be deselected as an infix operator,
and in consequence the expression as a whole might not parse.
...
Thus in the expression `\verb|x + - y|', the `\verb|-|' would be deselected
as an infix operator by this step, as `\verb|+|'
conflicts with `\verb|-|' and is not a postfix operator.  The `\verb|+|'
would not be deselected, as although `\verb|-|' conflicts with it,
`\verb|-|' is a prefix sequence.
...
In PCASL things can be arranged so the nodes to be evaluated,
...
is the expression defined by an expression definition.  However, in PCASL
arrows may be added to an expression being evaluated (Step~\ref{ADD-ARROW-STEP}
\pagref{ADD-ARROW-STEP}).
So in PCASL probably no immutable node can
point directly at another immutable node.\footnote{PCASL differs from
...
Luckily PCASL is not intended to run large programs.  However, it should be
...
of the expression definition are bound to subexpressions of the expression or
to default values for the variables.
Then some of these subexpressions or defaults are evaluated
...
In use, the pattern of an expression definition and the expression being
evaluated are both copied and the two copies are then call unified
(\secref{CALL-UNIFICATION}).
If unification fails, the definition does not apply.  If
unification succeeds, variables not bound to values are given their
default values, subexpressions bound to some of the variables in
the {\em pattern} are evaluated and replaced by their values, and then
the {\em guards} are evaluated.  If any of the {\em guards} evaluate
...
or of a variable are equal, one of these values is returned as the value
of the expression or variable.  If some of the values are unequal, an
error value giving the context of the evaluation and the unequal values
is returned as the value of the expression or variable.
...
It is an error if a null node is to be evaluated.
...
is evaluated (\secref{BLOCKS}) to determine the value of the
expression.
...
An expression is evaluated in a context.  A \key{context} is just a list
of expression definitions and other contexts.  The context is searched
...
The search of a context is in list order.  If a list element that is another
context is encountered, that context is searched completely before
continuing to the next element of the list.

An \key{expression definition} consists
of an {\em EXPRESSION-DEFINITION} (\pagref{EXPRESSION-DEFINITION}) and
a context in which the expression defined is executed.  An
{\em EXPRESSION-DEFINITION} expression computes an expression definition
object of type \ttkey{expression-definition} that has no context.
A context can be added to such an object later by executing:

\begin{indpar}
\verb|set| {\em EXPRESSION-DEFINITION} \verb|context| {\em CONTEXT}
\end{indpar}

Once a context has been added to an expression definition, this context
cannot be changed.
...
\begin{indpar}
	\verb|an expression-definition|
	      {\em EXPRESSION-DEFINITION}
	      {\em CONTEXT}
\end{indpar}

The new expression definition has a different context, given by
the second argument, than the first argument does.  The expression
...
makes a new expression definition whose definition part is the same as
its argument but whose context part is missing and can be set later.

There are two kinds of contexts: immutable and mutable.

An \key{immutable context}\label{IMMUTABLE-CONTEXT}
may not be modified.  It can be created by:

\begin{indpar}
\verb|a context (| {\em CONTEXT-ITEM} \{ \verb|,| {\em CONTEXT-ITEM} \} \verb|)|
\end{indpar}

in which each {\em CONTEXT-ITEM} is either an expression definition or
a context.  If any of the {\em CONTEXT-ITEMs} are expression definitions
that do not have their contexts set, their contexts are set to the context
being created.  Thus a set of expression definitions can be created
which can reference each other.

The expression

\begin{indpar}
\verb|a context (| {\em CONTEXT-ITEM} \{ \verb|,| {\em CONTEXT-ITEM} \} \verb|)|
\verb|with context setting false|
\end{indpar}

makes an immutable context without setting the contexts of any expression
definition {\em CONTEXT-ITEMs}.  This gives the flexibility to set these
contexts later.

TBD problem with conflicts among definitions in one list
...
an expression-definition {
    on an expression-definition EXPRESSION-DEFINITION CONTEXT
       <-- expression-definition EXPRESSION-DEFINITION,
           context CONTEXT
    on an expression-definition EXPRESSION-DEFINITION
       <-- expression-definition EXPRESSION-DEFINITION
    on set SELF context CONTEXT <-- context CONTEXT
}
a context {
    on a context CONTEXT-ITEM-TUPLE <--
              list CONTEXT-ITEM-TUPLE of context-items
}
context-item X <-- expression-definition X
context-item X <-- context X
context-item X <-- mutable-context X
\end{verbatim}
\end{indpar}
...
\key{block} ::= \verb|{| {\em statement}
                         \{ \verb|;| {\em statement} \}\,$^\star$ \verb|}|
			 \\[1ex]
\key{statement} ::= {\em expression-definition}
		$|$ {\em empty-statement}
		$|$ {\em qualified-statement}
			 \\[1ex]
\key{expression-definition} ::= {\em EXPRESSION-DEFINITION} 
				( see \pagref{EXPRESSION-DEFINITION}) \\[1ex]
\key{empty-statement} ::= {\em empty} \\[1ex]
...
\key{output-variable} ::= {\em variable-name} \\[1ex]
...
\key{unguarded-subblock} ::= {\em block} \\[1ex]
...
\key{guarded-subblock} ::= {\em block} \\[1ex]
\key{subblock} ::= {\em guarded-subblock} $|$ {\em unguarded-subblock}
\end{indpar}
...
except for {\em assignment-patterns}, each of which becomes a separate
...
PCASL does not have the notion of name space nesting, because the
...
Evaluation of a {\em block} is driven by availability of variable values
and not by the order of the {\em statements} in the {\em block}.
Each variable in a block can be assigned at most one value: it is
an error if the variable is assigned more than one value.
...
Each {\em right-side-expression} in a {\em block}
...
Block evaluation terminates when no further evaluation is possible.

When block evaluation terminates, the value of the block is the
...
It is an error if a variable is assigned a value by more than one
{\em assignment-statement} during block evaluation.

Block evaluation is modified by {\em statement-qualifiers}
(\secref{STATEMENT-QUALIFIERS-AND-ITERATION}).

\subsection{Statement Qualifiers and Iteration}
\label{STATEMENT-QUALIFIERS-AND-ITERATION}

{\em Statement-qualifiers} modify block evaluation.

A {\em statement} qualified by the
\ttkey{default} {\em statement-qualifier}
is invisible, as if it did not exist, until all variables that can be
given a value by other statements in a block have been given values.
Then if \underline{no} variable that could be assigned by the {\tt default}
qualified {\em statement} has
been given a value, the statement is made visible, and can execute.

All {\tt default} qualified {\em statements} become visible at once,
so it is possible for an error to occur if two such statements assign
the same variable.
...
{\em assignment-statement} remains invisible, but if \verb|X > 5| is false,
the {\tt default} {\em assignment-statement} becomes visible and
...
variable and if it does execute a \underline{non-{\tt default}}
{\tt next} qualified {\em statement}.  The sequence of blocks are called the
...
The variable values computed by a \ttkey{next} qualified
{\em statement} are not variable values in the current
block, but are rather values for the current block's next block,
if the current block iterates.  The next block starts with these
variable values, so these variables must not be assigned in the next
block.

If a {\em statement} is qualified by both {\tt next} and {\tt default},
the {\em statement} becomes visible after all {\tt default}
{\em statements} have become visible and executed if at this point
the block is going to iterate, and if at this point none of the variables
that can be assigned by the {\em statement} have values for the next iteration.

A statement of the form

\begin{center}
\verb|default| \verb|next| {\em variable-name} = {\em variable-name}
\end{center}

is implied for every inherited {\em variable-name} that is not
defined by an explicit `{\tt default next}' statement in the code.
Thus inherited variables are normally propagated unchanged from
one iteration to the next.

If a `{\tt next}' qualified statement sets a variable to the value of
...
\begin{center}
\verb|default| \verb|next| {\em variable-name} = \verb|UNDEFINED|
\end{center}

for an inherited {\em variable-name} will keep the inherited
variable from being propagated from one iteration to the next.
The expression `{\tt UNDEFINED}' cannot be used to set a variable
outside a `{\tt next}' qualified statement.
...
for every {\em variable-name} that can be assigned by the
{\tt always} qualified {\em statement}.  The intention is that the
...
TBD: should {\tt next} qualify a variable name and not a statement?



\section{Typed Blocks}
\label{TYPED-BLOCKS}

A typed block is a block that has a type, that can be used as a value,
and that inherits code from its type.

A \key{block value} can be created by a {\em block-value} expression:


\begin{indpar}
\key{block-value} ::= \{ \ttkey{a} $|$ \ttkey{an} \} {\em type-name}
		      {\em block} \\[1ex]
\key{type-name} ::= {\em word}
\end{indpar}

The code in the {\em block} becomes a permanent part of the block value.
The variable values defined by this code become \skey{component}s of the value.
...
Blocks are updated by adding code to them either permanently or temporarily.
Whenever code is added to a block value, the code executes as much as it
can to define variable values that are then components of the block value.
When the code can execute no further (\secref{BLOCK-EVALUATION},
\secref{STATEMENT-QUALIFIERS-AND-ITERATION}), any
variables that do not have values are undefined components.  If some of
these are given values later, any code that is a permanent part of the
block may restart and produce additional variable values that define
...
The components of a block value are more precisely the syntactic variables
of the code added permanently to the block value, whether or not these
variables have values defined by code added permanently or temporarily
to the block value.  Variables of code added temporarily that are not
syntactically variables of code added permanently are
\underline{not} components of the block value,
...
adds the code in its block permanently to every block value of the given type,
including both existing block values and block values that are
created in the future.  The code in the block is associated with the
type and is said to be `\skey{inherit}{ed}' by every block of that type.

A statement of the form:

\begin{indpar}
\ttkey{update} {\em EXPRESSION} {\em block} 
\end{indpar}

where {\em EXPRESSION} evaluates to a block value adds the code in the
{\em block} temporarily to the block value.  The code is added and all
code in the block value executes until it can do nothing more.  Then
the temporary code is removed from the block value.

When code is added to a block value and executes, the statement qualifier
{\tt default} acts when execution stops to enable execution of {\tt default}
statements, as per \secref{STATEMENT-QUALIFIERS-AND-ITERATION}.  Similarly
the statement qualifier {\tt next} can be used to create iterations of the
block value.  Each iteration replaces the previous iteration as the block
value wherever the block value is referenced.  Thus if two variables equal
the block value, and one is used to cause the value to iterate, both variables
get the new iterated value.

A statement of the form

\begin{center}
\verb|default| \verb|next| {\em component-name} = {\em component-name}
\end{center}

is implied for every {\em component-name} of a block value that is not
defined by an explicit `{\tt default next}' statement in the current
(permanent plus temporary) code of the block value.
Thus components not explicitly changed propagate unchanged from
one iteration of a block value to the next.
The statement

\begin{center}
\verb|default| \verb|next| {\em component-name} = \verb|UNDEFINED|
\end{center}

(\pagref{UNDEFINED})
can be used to change this behavior and make a component undefined when
the next block value iteration is created, in which case other permanent
code may immediately execute to define the value.

When an iteration of a block value is created by temporary code, the
sequence of actions is:

\begin{center}
\begin{tabular}{lp{5.5in}}
(1) & Add the temporary code to the block value. \\
(2) & Execute all block value code until it terminates. \\
(3) & If any {\tt next} variable values were defined by the
      execution in step (2), create
      the next iteration of the block value, giving this
      iteration only the permanent code of the block value. \\
(4) & In the next iteration of the block value execute all
...
Operations that have side effects: input, output, typed block update
(which is like typed block IO).
