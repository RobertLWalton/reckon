...
A \TT{.} is a numeric character (decimal point) if it
...
is just the {\em quoted-string} lexeme with its surrounding quotes (\TT{"})
removed.  Thus, for example, \TT{fie} and \TT{"fie"} represent the
same symbol, as do \TT{+} and \TT{"+"}.
...
to the parser, \TT{+} and \TT{"+"} are not the same when input, but when
output they are the same.
For instance, given \TT{x + y} as input the parser will output \TT{+ x y},
...
\emkey{name} := {\em name-lexeme}\STAR{} ~ {\em non-numeric-name-lexeme}
                ~ {\em name-lexeme}\STAR{}
\label{NAME}\\[1ex]
\emkey{name-lexeme} ::= {\em lexeme} other than {\em non-name-lexeme}
\label{NAME-LEXEME} \\[1ex]
...
[TBD should marks be excluded from names (so quotes are required)?
Then flag-lexemes would be words, decimal-naturals, marks, and
quoted strings, but not separators.]
...
\emkey{non-zero-digit} := {\em digit} other than \TT{0}
\end{indpar}

Given a non-negative integer {\em number} atom, there is a unique
{\em decimal-natural} representing it.  Note that
{\em decimal-naturals} cannot contain commas, as if they could,
either all {\em decimal-natural}
representations of an integer would have to contain commas,
or the representations would not be unique.
...
\emkey{atom-value}
    \begin{tabular}[t]{rl}
    ::= & {\em name-lexeme} \\
    $|$ & {\em concatenated-quoted-strings} \\
...
\begin{tabular}{l}
\emkey{flag-lexeme}
    ::= {\em name-lexeme}\pagnote{NAME-LEXEME}
\end{tabular}
...
\\\hspace*{0.5in}
	\TT{<*>} {\em attribute-label} {\em attribute-label-flags}\QMARK{}
\\\hspace*{1.0in}
	\begin{tabular}[t]{@{}l@{}}
...
Raw object representations do not make any use of the syntax
elements indicated above; specifically, {\em bracketed-lists} are
...
add new values to previously existing attributes of the object begin
represented.
More precisely, the {\em attribute-label} of any {\em attribute-representation}
in the {\em object-representation} must not have previously (before the
{\em object-representa\-tion} is read)
been an attribute label of the represented object.
...
of this paragraph.  Thus \TT{=>} {\em object-representations} for
different objects may each contain {\em double-attribute-representations}
with the same {\em double-attribute-value} and {\em reverse-attribute-label}.
...
\emkey{bracketed-list}\label{BRACKETED-LIST}
    ...
    $|$ & \TT{\{|} {\em prefix-0-list} \TT{|\}} \\
    $|$ & \TT{\{} {\em w-type} {\em properties}\QMARK{}
          \TT{|} {\em prefix-0-list}
          \TT{|\}} \\
    $|$ & \TT{\{} {\em w-type} {\em properties}\QMARK{}
          \TT{| } {\em prefix-0-list}
          \TT{|} {\em w-type} {\em properties}\QMARK{} \TT{\}} \\
    \end{tabular}
\end{tabular}

\begin{tabular}{l}
\emkey{m-type} ::= {\em mark}
\end{tabular}

\begin{tabular}{l}
\emkey{w-type} ::= {\em name}\pagnote{NAME} other than {\em mark}
\end{tabular}
...
\begin{tabular}{l}
\emkey{property-value} ::= {\em single-attribute-value}
\end{tabular}

\end{indpar}

{\em Prefixes}, {\em types}, and {\em properties} are explained
below.  First we give some examples involving just parsed untyped
{\em bracketed-lists} without {\em properties}.  In some of these
...
`\TT{\}}' must be proceeded by a `\TT{|}', `{\em m-type}', or
`\TT{|} {\em w-type} {\em properties}\QMARK{}' phrase, and the
corresponding `\TT{\{}' must be followed by a `\TT{|}', {\em m-type},
or `{\em w-type} {\em properties}\QMARK{} \TT{|}' phrase respectively
...
The two-{\em symbol} sequence `{\tt \{\}}'
is an abbreviation for the empty list,
and is equivalent to `{\tt \{|~|\}}'.

`{\tt ||\}}' is an abbreviation of `{\tt |~|\}}' when the latter is used to
denote an empty list that (optionally) has a {\em type} and {\em properties}.

{\em Lists} may be parsed after they have been read.  Operators, including
separators such as the semi-colon `{\tt ;}'
and comma `{\tt ,}', are recognized by such a parse.  See
...
More algorithmically, during a left to right scan of a {\em prefix-0-list},
...
sublist.  The type and properties of this {\em prefix} become the
type and properties of the sublist.
...
The untyped lists, such as the parenthesized lists, are actually
equivalent to typed lists as follows:
\begin{center}
\begin{tabular}{ll}
Untyped List Form	& Equivalent Typed List Form
\\\hline
{\tt (} \ldots{} {\tt )}	& {\tt \{"("|} \ldots{} {\tt |\}} \\
{\tt [} \ldots{} {\tt ]}	& {\tt \{"["|} \ldots{} {\tt |\}} \\
{\tt `} \ldots{} {\tt '}	& {\tt \{"`"|} \ldots{} {\tt |\}} \\
...
Thus the type of a `{\tt (~)}' parenthesized list is {\tt "("} and
the type of a `{\tt \{|~|\}}' bracketed list is just `{\tt .}'.

A {\em property} is a label/value pair.
Lists can have elements which are in a sequence and properties which
have labels.  Actually, the list elements are automatically assigned
...
A {\em property-value} that consists of more than one {\em value-symbol}
is equivalent to a {\tt \{|~|\}} list of these {\em value-symbols}.
Thus `{\tt kind = prefix infix}' is equivalent to
`{\tt kind = \{|prefix infix|\}}'.

If no {\em property-value} is given, it is implicitly `{\tt true}'
if the {\em property-label} does not begin with `{\tt no}',
`{\tt no-}', or `{\tt no~}'.
Otherwise the `{\tt no}', `{\tt no-}', or `{\tt no~}' is removed
from the {\em property-label} and the {\em property-value}
is implicitly `{\tt false}'.  For example,
\begin{center}
\tt \{my type:~format, no-capitalize, noadjust, no tweak\}
\end{center}
is equivalent to
\begin{center}
\tt
...
If no {\em property-value} is given,
it is an error if the {\em property-label} begins with
`{\tt no}', `{\tt no-}', or `{\tt no~}' and is not still a {\em property-label}
after the `{\tt no}', `{\tt no-}', or `{\tt no~}' are removed (e.g.,
`{\tt no`set}' or `{\tt no}' by itself would be in error).
