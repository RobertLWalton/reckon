...
Pattern equations are similar to but different from syntax equations.
They obey the following rules:

\begin{indpar}
\begin{list}{}{}
\item[\ikey{Pattern Consistency Rule}{pattern consistency rule}:] ~\\
A pattern equation, after its pattern has been unfolded,
is \key{consistent} if it:
\begin{enumerate}
\item asserts identity between two identical atoms, or
\item asserts identity between an object and a pattern such that:
\begin{enumerate}
...
that is made into a \TT{(~)} bracketed list
\end{enumerate}
