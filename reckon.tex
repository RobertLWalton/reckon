...
a symmetric and transitive relation on subexpressions of the original
syntax equation.  This means, however, that two different subexpressions
...
So in addition to the Propagation Rule, we apply the following, where
...
Specifically, the set of variables $v$ for which an equation of the
form $v$ {\tt :=:} $e$ where $e$ is not a variable would not expand.
...
either $e_i$ {\tt :=:} $e_{i+1}$
or $e_{i+1}$ {\tt :=:} $e_i$ is in $S'$.  $n=1$ is allowed.
...
if such an equation exists in $S'$.
...
consequent must be an equation in $S''$.
\end{quote}
