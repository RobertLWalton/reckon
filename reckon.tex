...
for every board:
    on a board of size S:
        ...
        size = S
        `state(I,J) is `safe' or `hit.''
        array hit of size (S,S) with initial element `safe'
        `vessel(I,J) is ID of vessel at (I,J) or `none'.'
    ...
    on a vessel of length L on board B:
        ...
        location loop:
            ...
            check loop:
                first p = position
                first len = 1
                if vessel(p) of board != `none':
                    continue location loop
                if len == length:
                    finish check loop
                next p = p + direction
                next len = len + 1
...
All {\em dates} are {\em numerics}.  Some examples are:
