...
A nofix operator cannot be infix, prefix, or postfix, but otherwise
any combination of fixity is permitted.
Ambiguities in parsing are possible and
are treated as parsing errors.  For example, if an operator that
can be postfix or infix precedes an operator that can be infix
...
During the execution of the algorithm below, a \key{closing bracket}
...
if that is given.
A \key{parsing terminator} is defined to be a closing bracket, a terminating
operator, or the end of the input list.

Also during the execution of this algorithm a list of {\em expression-items}
...
\item If a closing bracket is at the beginning of
the input list, this bracket is removed from the beginning of
the input list, and the parsing loop terminates.

\item If a terminating operator at the beginning of the input list,
...
\item If the bracket definition has a reformatter, it is used to reformat
the parsed expression returned by the parser call.  Then this parsed
expression is pushed as a single {\em expression-item} to the end of the
output list.

\item If a {\em new-parsing-selector-set} from the bracket definition
was previously pushed onto the parsing selector stack,
...
the entire output list becomes an operand, and the output list is
...
then an error infix operator is inserted at the \underline{beginning}
of the \underline{input list}, a parsing error is announced, and
the error operator replaces the operator previously at the beginning
of the input list in what follows.
...
flag but not the infix flag, the operator is designated to be postfix.

\item If the output list is non-empty, the operator has the postfix
flag and the infix flag, and the operator is followed by a parsing
terminator or by another operator that cannot be prefix or nofix,
the operator is designated to be postfix.
...
list, the prefix operator precedence as its operator precedence argument,
and a copy of the closing bracket definition argument.

\item Upon return from the parser call
the returned lexeme list becomes the input list and the returned
parsed expression becomes the current operand.

\item If the prefix operator definition has a reformatter, it is
used to reformat the operand.

\item The operand is pushed into the output list.

\item If the output list is not empty or the operator can be a nofix
operator, then:
...
\item
If the operator can be nofix or postfix and it is 
is pushed into the output list.

\item
The operator at the beginning of the input list is made into the
lead operator, and its definition becomes the lead operator
definition.
