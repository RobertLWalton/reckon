...
\emkey{pattern-term} \begin{tabular}[t]{rl}
            ::= & {\em pattern-factor} {\em pattern-factor}$^\star$ \\
            ::= & {\em pattern-factor} {\em pattern-factor}$^\star$
	    		\ttmkey{?}{in pattern} \\
            ::= & {\em pattern-factor} {\em pattern-factor}$^\star$
	    		\ttmkey{...}{in pattern} \\
...
As a general rule, a syntax equation is \key{inconsistent} if it

\begin{enumerate}
\item asserts identity between distinct atoms, or
\item asserts identity between objects one of which has an attribute
label that the other does not have, or
\item asserts identity between objects one of which has an attribute
with more than one value, or
\item asserts identity between an atom and an object
\end{enumerate}

However, there are some exceptions to these rules.  These are:

\begin{enumerate}
\item an object whose {\tt .initiator} is {\tt "("} and whose
{\tt .terminator} is {\tt ")"} is treated as if it had no
{\tt .initiator} or {\tt .terminator} attributes,
\item an atom may be treated as if it were an object that has
...
label shared by the two objects.  Given such an attribute label, the values
of the attribute for the two objects are asserted to be syntactically
identical.  For example, from
...
However, in this case the first two equations are consistent, and
from the second we can deduce:
...
\begin{enumerate}
\item Symmetry: from $e_1$ {\tt :=:} $e_2$ we deduce $e_2$ {\tt :=:} $e_1$.

\item Transitivity: from $e_1$ {\tt :=:} $e_2$ and $e_2$ {\tt :=:} $e_3$
we deduce $e_1$ {\tt :=:} $e_3$.
\end{enumerate}
...
When a pattern equation identifies a syntax expression that is an
object, and not an atom or a syntax variable, with a pattern, then
the pattern is `\key{unfolded}' to become a syntax expression.  It must
be possible to do this unambiguously, or the pattern equation is
inconsistent.

Consider the pattern equation
...
The {\em pattern} here contains three {\em pattern-terms} separated
by `\verb|.|'s.  
