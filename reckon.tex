...
{\tt matchfix} operators, which have two sequences of lexemes, an
...
If opening and closing marks are part of a {\tt matchfix} operator
name, they must begin and end the name.  When a subexpression is tested
for {\tt matchfix} operators, any opening and closing marks that begin and
...
the {\tt matchfix} operator `\verb/| ... |/' could be invoked by
...
only {\tt infix} operators in the expression that have lowest precedence
among all the {\tt infix} operators are considered,
all {\tt prefix} operators that begin the expression are considered,
all {\tt postfix} operators that end the expression are considered,
and all {\tt matchfix} operators that begin and end the expression are
considered.  Ambiguity is possible, and unless resulved by one of the
following rules, is an error.

{\bf Matchfix Rule.}\index{Matchfix Rule}
If two different matchfix operators surround and expression, but the
...
{\bf Non-Empty Operand Rule.}\index{Non-Empty Operand Rule}
Any selection of operators in a expression that would produce an empty
operand for an infix, prefix, or postfix operator is suppressed.

This if \verb|+| is both a prefix and infix operator, the
expression `\verb|x + + y|' 

TBD

Two different \verb|matchfix| operators X and Y may be such that the beginning
lexemes of X are a proper prefix of the beginning lexemes of Y, and the
ending lexemes of X are a proper prefix of the ending lexemes of Y.  In this
case we say that X is more general than Y, and Y is more specific
than X.  For example,
More specific \verb|matchfix| operators are tested for before more general
\verb|matchfix| operators, so the most specific \verb|matchfix| operator
that works for a given expression will be the one recognized.
...
An {\tt infix} operator has an {\em associativity} that is named
by a single word or separator lexeme.
Three words, {\tt left}, {\tt right}, and
{\tt none} have special meaning.  If an {\tt infix} operator has some
...
Thus if the {\tt infix} operators \verb|<| and \verb|<=| have associativity
