...
faithful to their input, though they can suffer differences
related to quoting.
An unquoted symbol is stored exactly as it is represented.
A quoted symbol not containing \BACKSLASH{}
is similarly stored.  An unquoted symbol not containing \BACKSLASH{}
is stored exactly the same way as the same symbol surrounded by
quotes.  Thus \verb|fie| and \verb|"fie"| store the same
thing, as do \verb|+| and \verb|"+"|.

However, some symbols are recognized and treated specially by the parser.
These \skey{special-symbol}s must be quoted if you want to prevent such
...
{\em non-special-symbol}.  {\em Names} are used to name things, such
...
the subexpression.  Non-blank lines
indented by more than the subexpression indentation
are continuations of the preceding non-blank line.
