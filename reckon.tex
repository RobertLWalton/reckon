...
\begin{tabular}{l}
\emkey{raw-atom-list} ::= \TT{|} {\em raw-atom}\STAR{} \TT{|} \\
\end{tabular}
...
\begin{tabular}{l}
\emkey{flag-atom} ::= {\em raw-atom} but not {\em numeric}
\end{tabular}
...
The \TT{<\#>} {\em attribute-representations} is ignored; that is,
blank lines amoung the {\em attribute-representations} are ignored.

It is possible to place \skey{attribute flag}s on attribute labels
by putting flag characters in \TT{[ ]} brackets after a label.
The flag characters are those of a {\em raw-atom} enclosed
...
\begin{tabular}{l}
\emkey{type} ::= {\em word}\PLUS{} $|$ \TT{.}
\end{tabular}

\begin{tabular}{l}
\emkey{list-opening} ::= \TT{(} $|$ \TT{[} $|$
                         \TT{`}\PLUS{} $|$ \TT{\{|} $|$ 
			 \TT{\{}{\em type}
		         \{ \TT{:} {\em properties} \}\QMARK{} \TT{|} \\
\end{tabular}

\begin{tabular}{l}
\emkey{list-closing} ::= \TT{)} $|$ \TT{]} $|$
                         \TT{'}\PLUS{} $|$ \TT{|\}} $|$ 
                         \TT{|} \{ {\em properties} \TT{:}  \}\QMARK{}
			 {\em type}\TT{\}} \\
\end{tabular}
...
\emkey{prefix}\label{PREFIX}
    & ::= & \TT{\{}{\em type} \{ \TT{:} {\em properties} \}\QMARK{} \TT{\}}
\\[1ex]
\multicolumn{3}{c}{All {\em prefix-n}'s in a {\em prefix-n-list}
                   have the same type.}
\end{tabular}
...
\hspace*{1.7in} \\[-2ex]
\emkey{list-element}
    & ::= & {\em word} $|$ {\em data-separator} $|$ {\em list} \\
\emkey{data-separator}\label{DATA-SEPARATOR}
    & ::= & {\em separator} not in {\em non-data-separator} \\
\emkey{non-data-separator}
    & ::= & {\em strict-separator} $|$ \TT{`}\PLUS{} $|$ \TT{'}\PLUS{}
\end{tabular}

\begin{tabular}{rcl}
\hspace*{1.7in} \\[-2ex]
\emkey{properties} & ::= & {\em property}
		           \{ \TT{,} {\em property} \}\STAR{} \\
\emkey{property} & ::= & {\em property-label} \TT{=} {\em property-value} \\
                 & $|$ & {\em property-label} \\
                 & $|$ & \TT{no}{\em property-label} \\
                 & $|$ & \TT{no-}{\em property-label} \\
                 & $|$ & \TT{no~}{\em property-label} \\
\emkey{property-label} & ::= & {\em property-label-word}\PLUS{} \\
\emkey{property-label-word} & ::= & {\em word} other than `\TT{=}' \\
\emkey{property-value} & ::= & {\em value-symbol}\PLUS $|${\em list} \\
\emkey{value-symbol} & ::= & {\em symbol} other than {\em non-value-symbol} \\
\emkey{non-value-symbol} & ::= & `\TT{=}' $|$ `\TT{,}' $|$
			         `\TT{`}\PLUS' $|$ `\TT{'}\PLUS' $|$
		                  {\em strict-separator}
