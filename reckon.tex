...
\emkey{word} ::=
    ...
    (1) contains a letter \\
    (2) contains only letters, digits
    and the characters: \TT{`} \TT{'} \TT{.} \TT{-} \TT{\_} \\
    (3) does not have any digit before its first letter \\
    (4) does not begin with \TT{`} or end with \TT{'~.~-~\_} \\
    \end{tabular}
\\[1ex]
\emkey{index} ::=
    \begin{tabular}[t]{l}
    ...
    (2) surrounds every \TT{-} by digits \\
    (3) follows every \TT{.} by a digit \\
    \end{tabular}
\\[1ex]
\emkey{numeric} ::=
    \begin{tabular}[t]{l}
    {\em middle-lexeme} that \\
    (1) contains a digit or \TT{\#} \\
    (2) either contains a \TT{\#} or does not have a letter before the
        first digit
    \end{tabular}
\\[1ex]
...
(see Section~\itemref{SPECIAL-NUMERICS})
and {\em marks} (see Section~\itemref{SPECIAL-MARKS}): for
example, \TT{05may1999} is a date {\em numeric} and \TT{/5.2/} is
a format mark.  {\em Words}, however, never
special, and can be used to form names of variables, constants, and functions.
Some of these names, however, have builtin definitions, such as the
words `\TT{ft}' and `\TT{in}' which in some contexts are units
(see Section~\itemref{UNITS}),
and `\TT{PI}' which is some contexts denotes the mathematical
constant $\pi$.  {\em Indices} may, according to context, be used
as non-initial components of names, or if they contain at most one
`\TT{.}', may be used as a {\em numeric} denoting a number.
