...
Expression definitions are searched for in a context list, which is just
a list of contexts.  Each context is a list of expression definitions.
Each point in the program has a lexical context, which operates like
a stack.  There is also a global context to which definitions may be added
or from which they may be deleted.  An expression may act like
a function call, and the called function may evaluate expressions in the
context of its caller, or a context derived from that of its caller.
?????
...
\key{block} ::= \verb|{| {\em statement}$^\star$ \verb|}| \\[1ex]
\end{indpar}
...
A goal is an expression that must be inferred to make
...
their values before the goals are inferred.
...
Design: Input checkpointing is used to record all inputs to a computation
...
to be repeated once or a few times so the compute can turn the history
