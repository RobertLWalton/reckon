...
pre-lexeme (\secref{TEXT-PARSING}).
...
\key{lexeme-definition} ::=
	...
	{\em lexeme-function-definition} \\[1ex]
\key{lexeme-dictionary-definition} \\
	\hspace*{1in}::= \verb|define lexeme dictionary|
				{\em lexeme-dictionary-entries} \\[1ex]
\key{lexeme-dictionary-entries} \begin{tabular}[t]{rl}
...
\key{lexeme-dictionary-entry} ::=
	{\em replaced-lexeme} \verb|=>| {\em lexeme-replacement} \\[1ex]
\key{replaced-lexeme} ::= {\em word} $|$ {\em separator} \\[1ex]
\key{lexeme-replacement} ::= {\em replacing-lexemes} \\[1ex]
\key{replacing-lexemes} ::=
	\{ {\em word} $|$ {\em separator} \}
	\{ {\em word} $|$ {\em separator} \}$^\star$ \\[1ex]
\key{lexeme-function-definition} \\
	\hspace*{1in}::= \verb|define lexeme function|
...
An operator may push definitions into the parsing
...
\key{operator-definition} \\
	\hspace*{1in}::= \verb|define operator| {\em operator-name}
	\\[1ex]
\key{lexeme-dictionary-entries} \begin{tabular}[t]{rl}
                     ::= & \verb|(| {\em lexeme-dictionary-entry-list} \verb|)|
		     \end{tabular} \\[1ex]
...
operator parsers (\secref{OPERATOR-PARSER}),
operator control flags (\secref{OPERATOR-CONTROL-FLAGS}),
and operator subdefinitions (\secref{OPERATOR-SUBDEFINITIONS})
above.
