...
complete the match.  There are four kinds of constraints: syntax
equations, pattern equations, default equations, and guards.
Syntax equations identify expressions that involve syntax variables,
which are names beginning with a capital letter
preceded by a \verb|$|.  Pattern equations are like syntax equations but
involve a pattern which can represent one of many possible syntax
expressions.  Default equations assign values to variables that are
discarded when a pattern is unfolded to become the expression
it will actually represent.  Guards are just expressions that must evaluate to
...
the matching process to become the expressions they represent.
Guards are evaluated, and in some cases a syntax variable
value is evaluated.
...
\item A {\em pattern-clause} not followed by a {\em pattern-marker}
other than `\verb|.|' in the {\em pattern} must appear exactly
...
means removing the \skey{pattern marker}s, `\verb|.|',
...
\subsubsection{Optional Truth-Value Syntax Variables}
\label{OPTIONAL-TRUTH-VALUE-SYNTAX-VARIABLES}
...
$v$ {\tt :=:} {\tt false}
\end{center}
is asserted.  However, if the clause is not omitted from the unfolded
pattern, but the variable $v$ at the end of the clause is omitted from
the unfolded pattern, then the syntax equation
\begin{center}
$v$ {\tt :=:} {\tt true}
\end{center}
is asserted.
...
(\pagref{EVALUATED-SYNTAX-VARIABLE}), but it may be
...
So expression evaluation is a search process to find a choice
...
the outcome of the evalution process is uniquely determined, and does not
...
or of a syntax variable are equal, one of these values is returned as the value
of the expression or syntax variable.  If some of the values are unequal, an
error value giving the context of the evaluation and the unequal values
is returned as the value of the expression or syntax variable.
...
a context {
    on a context CONTEXT-ELEMENT-LIST
       . is list LIST ??
       . is mutable MUTABLE ??
       . with context missing CONTEXT-MISSING ??
