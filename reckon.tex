...
{\em Numerics} represent \key{numbers} which are stored as strings of binary
...
{\em Special-character-representatives} inside the \TT{"}'s of a
quoted string may be used to represent various characters: for
example, the {\em quoted-string} \TT{"<Q>Help!<Q>"} represents the
7-character symbol \TT{"Help!"}.


Symbols and numbers are referred to as \skey{atom}s.  The equations
...
A \key{name} is a sequence of atoms represented by a sequence of
{\em word}, {\em numeric}, and {\em quoted-string} lexemes:

\begin{indpar}
\emkey{name} := {\em name-lexeme}\PLUS{}
\label{NAME}\\[1ex]
\emkey{name-lexeme} ::= {\em word} $|$ {\em numeric} $|$ {\em quoted-string}
\label{NAME-LEXEME}
\end{indpar}
...
A name is said to be a \key{natural-number}\label{NATURAL-NUMBER}
if and only if it consists
of just one non-negative integer number atom.

The lexemes of a name are sometimes combined with each other to
represent an atom of the name.  So for example, the names
`\TT{1e5}', `\TT{100000}', and `\TT{100,000}' all represent
the same one atom {\em natural-number} name (here `\TT{1e5}'
is the 3-lexeme name `\TT{1 e 5}' which is scientific notation
for a single number atom).

A \key{simple-name} is a name that represents a single atom
in a direct way, such as `\TT{100,000}' and `\TT{1e5}'.
A \key{compound-name} is a name that is parsed to represent
an expression, such as the name `\TT{george's weight}' which
is parsed to represent `\TT{george 's weight}' where \TT{'s}
has become an operator (like the `\TT{.}' operator in C, C++,
...
A \key{simple name} is a {\em name} that represents a single
atom in a direct way without use of dictionaries (see \pagref{DICTIONARY}).
...
\emkey{object-identifier} ::=
	...
	{\em natural-number}\pagnote{NATURAL-NUMBER} \\
\end{tabular}
...
\ikey{Lists}{list}
can have elements which are in a sequence and properties which
...
it is an error if the {\em property-label} is
`{\tt no}' by itself.
