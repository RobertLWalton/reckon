...
A description is an object with a type and inherits code from its type.
...
\begin{indpar}
\emlkey{description}{-update-statement} ::=
...
{\em description-update-statement} may also define `\skey{update variable}s'
...
Code can be temporarily added to an description by invoking methods.
...
of the description code (it can be either permanent or temporary).
The method is callable from code inside or outside the description code block.
...
the method definition.  The description whose code this is
...
equal to the description whose code contains the method definition.

If there is no \TT{SELF} argument, a new description
is created when the method is called, and the permanent code plus
method code is executed for that description.  The permanent code must
contain the method definition in this case.  Such a method is
called a \TT{constructor}, and the description created is said to be
constructed by the method execution.  Constructor executions usually return
the description they construct, but this is not required by the language.
Constructors usually
have patterns that begin with `\{ \TT{a} $|$ \TT{an} \} {\em type-name}',
where {\em type-name} names the type of description they construct, but
