...
a sequence of {\em character representatives} surrounded by
the double quote character (\verb|"|), where most characters
are their own representatives (\verb|A|, \verb|B|, \verb|0|,
etc.) but other characters are represented by a sequences of
characters that begin with a backslash (e.g., \verb|\n| represents
a new line character).  A \key{white-space} lexeme is a sequence
of white-space characters (single space, horizontal tab, form
feed, etc.).

A \verb|'| is a word character if it is followed by a letter,
and a separator otherwise.  A \verb|.| is a number character
(decimal point) if it
is followed by a digit, and a mark character otherwise.  A
\verb|,| is a number character if surrounded by digits, and
a separator otherwise.


A \key{white-space} lexeme does not have exactly the same characters
that were input to create it, unlike other lexemes.  The
...
If the lexeme contains a {\em carriage-return} but no
...
of the {\em white-space} lexeme, assuming that each
...
After these rules are applied, a {\em white-space} lexeme consists
...
empty {\em white-space} lexeme; the lexeme either has a
...
to the lexeme cannot have a {\em carriage-return},
