...
\key{block} ::= \verb|{| {\em group}
                         \{ {\em break} {\em group} \}\,$^\star$ \verb|}|
			 \\[1ex]
\key{break} ::= \ttnbkey{---}\verb|-|$^\star$
...
{\em Blocks} are divided into \skey{statement group}s by
{\em sequence-breaks}.
Evaluation of a statement group is driven by availability of variable values
and not by the order of the {\em statements} in the group.
However each group in a block is completely evaluated before any subsequent
groups in the block are evaluated.
...
Each {\em right-side-expression} in a statement group
