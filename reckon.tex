...
There are several different types of matching lexemes
that are used to bracket subexpressions:
...
A left matching lexeme may change the parsing context by `turning on'
a new kind of context by pushing the new kind of contest into a parsing
context stack whose top member is the effective parsing context.
The corresponding right matching lexeme will pop the parsing context
stack.
...
stack and not a counter; if evaluation is
...
incremented (decremented) by a left matching lexeme
that turns evaluation on (off), and decremented (incremented) by the
corresponding right matching lexeme.  This counter sticks at +1: matchings
...
lexical context by some operators.
...
and a \verb|matchfix| operator has two parts: a beginning part
that must begin an expression and an ending part that must end
the same expression.
...
The two sequences of lexemes for a matchfix
...
An example sequence of lexemes for a
matchfix operator is `\verb/[ | ... | ]/' that permits subexpressions
...
the expression `\verb/x + (| y |)/'.  Inclusing of extra pairs of parentheses
\verb|()| therefore has no affect on the raw representation of an expression
or its meaning.
...
But if `\verb|+|' were a prefix, infix, and postfix operator, these
expressions would both have erroneous ambiguity.
...
An infix operator has an {\em associativity} that is named
by a single word or separator lexeme.
Three words, {\tt left}, {\tt right}, and
...
only the rightmost operator is used to
...
only the leftmost operator is used to
...
operators as well as betweeen the beginning of the expression and the
first operator and between the last operator and the end of the expression.
Then the lexeme naming the associativity is prepended to the expression.
Thus if the infix operators \verb|<| and \verb|<=| have associativity
\verb|-COMPARE-|, then the expression \verb|0 < x - 3 <= 5| will
