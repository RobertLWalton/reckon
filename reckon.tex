...
In order to be recoginzed as a {\em unit-specifier} a {\em word} sequence
...
A matchfix operator name must begin with an opening mark other than
\verb|(|, and end with a matching closing mark.  These must be the only
opening/closing marks in the matchfix operator name.  Non-matchfix
...
begin and end a subexpression, and are thus invisible.
...
to use non-operator-precedence grammars for subexpressions.
...
would not be delesected, as although `\verb|-|' conflicts with it,
...
\key{right-side} \
    \begin{tabular}[t]{rl}
    ::= & {\em expression} \\
    $|$ & {\em expression} \ttkey{=>} {\em output-variable}
...
\key{unguarded-subblock} ::= {\em block}
...
in any preceeding {\em if-statement}
...
all {\em guards} in any preceeding {\em if-statement}
...
If a block finishes without producing a value for a variale named
...
variables are normally propogated unchanged from one iteration to the
next.
