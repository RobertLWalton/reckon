...
In a data paragraph (beginning with \TT{\{data\}}, \TT{\{raw data\}},
or equivalent: see \pagref{DATA-PARAGRAH}) logical lines end as for code
except that the separator `\TT{;}' lexeme is not recognized.
...
a blank line, or with an end-of-file lexeme.
...
object can only be empty if the \TT{.terminator} is a `\TT{;}' separator.
...
In text,
a physical line whose indent is greater than or equal to
the current paragraph indent
is a \key{continuation line} that
continues the current logical line, and blank lines 
terminate the current logical line.

In code empty logical lines and blank lines within a logical line
are ignored
...
end of the first logical line, the line end after the `\TT{;}' at the
end of the second logical line is ignored, and the blank line within
the first logical line is ignored.
