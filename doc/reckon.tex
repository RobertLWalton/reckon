...
a {\em word}, {\em numeric}, or {\em quoted-string} (but \underline{not}
by a {\em mark} or {\em separator})\pagnote{LEXEME-SYNTAX}.
A {\em word} and the same {\em word} quoted represent the same string:
e.g., \TT{X} and \TT{"X"} represent the same string.  Similarly
a {\em numeric} and the same {\em numeric} quoted represent the same
string.

A \key{number} is an IEEE 64-bit floating point
value that can be represented by a {\em number} lexeme\pagnote{NUMBERS}.

A \key{label} is a sequence of 0 or more atoms, and can be represented
by a {\em name}\pagnote{NAME}.
...
whose elements can be atoms, special values, or pointers to objects.
An object can also contain a set of \skey{attribute}s, each with an
attribute label and one or more attribute values which can be atoms,
special values, or pointers to objects.

The elements of a vector are referred to by the \ikey{indeces}{index!of vector}
\TT{0}, \TT{1}, \TT{2}, \ldots{}.

An attribute is referred to by an \key{attribute label} which is
a label represented by a {\em name} not beginning with a {\em number}.
More specifically, an attribute label is either a string or is a
label atom whose elements are strings or numbers and whose first element
is a string.

There are two kinds of pointers to object: single and double.

A \key{single pointer} is just a normal pointer to the target object.
It is named by the vector element index containing the pointer, or by
the attribute label of the attribute one of whose values is the pointer.

A \key{double pointer} is a double headed pointer between two objects,
and acts like a pair of pointers, one in each direction.  The difference
between a double pointer and a pair of single pointers is that 
it is possible to delete only one direction of a pair of single pointers, but 
when deleting a double pointer, both directions are deleted.

A double pointer must be an attribute value at each of its ends, and
has an attribute label for each of its directions.  From the point
of view of an object at one of its ends, it is named by the pair
of \underline{both} attribute labels.  The attribute label of the
attribute of the object which has the pointer as one of its values
is called the \key{forward attribute label}, and the attribute label
going in the other direction is called the \key{reverse attribute label}.

An attribute may have multiple values.  Except for double pointers,
each of these values is named by the attribute label of the attribute.
For double pointers, each is named by the pair consisting of the
forward and reverse attribute labels.

\ikey{Attribute flags}{attribute flag} may be attached to the
attribute labels of an object.
More precisely, a set of attribute flags is defined for each object
and each attribute label, and these flags apply to all attribute values
with the of the object that have the given label (in the case of
double pointer values, this means the forward attribute label).
The standard flags are the \key{hidden flag} (\ttmkey{H}{hidden flag}),
and the \key{maybe flag} (\ttmkey{G}{maybe flag}).
Attributes with a hidden flagged are not to be output when their object
is output.
...
if they cannot be reached without traversing an pointer whose label
has a maybe flag.
...
\begin{tabular}{l}
\emkey{element-list} ::=
	{\em raw-element-list} $|$ {\em cooked-element-list} \\
\emkey{raw-element-list} ::= {\em raw-element}\STAR{} \\
\emkey{cooked-element-list} ::= {\em expression}
                                ~~~~~ [see \pagref{EXPRESSION}] \\
\end{tabular}

\begin{tabular}{l}
\emkey{raw-element}\label{RAW-ELEMENT}
    \begin{tabular}[t]{rl}
...
There are two single attributes of object \TT{@<X>} (arrows sourced
at \TT{@<X>}),
...
the set to two elements, \TT{@<X>} and \TT{@<Y>}.
