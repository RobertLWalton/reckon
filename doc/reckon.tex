...
A {\em number} is an IEEE 64-bit floating point number.
{\em Number} and {\em numeric-word} lexemes translate to
numbers, and a {\em numeric} lexeme that is the same as
a {\em number} lexeme with commas every three integer digits
also translates to a number.

When a lexeme translates to a number, the type of the lexeme
is reset.  If the number is not finite, the type is set to
{\em numeric-word}; examples are {\tt +NaN} and {\tt 1e400}.
If the number is an integer in the range $[0,10^9)$, the type
...
\end{center}
are exactly represented in numbers.
