...
If its attribute label has the \TT{M} flag, it can have
a multiset of values, and if its label has the \TT{S} flag, it can have
...
that are \underline{not} pointers to object.  \TT{[<~>]}
bracketed subexpressions are used to represent
label atoms (see \pagref{LABELS}),
...
\begin{indpar}
\emkey{insertion}\label{INSERTION} ::= \TT{[} {\em code-expression} \TT{]} \\
\emkey{code-expression} ::= {\em expression} parsed in the \TT{code} context \\
...
{\em code-expression}\pagnote{CODE-EXPRESSION} in a
\TT{label} or \TT{text} context.  Examples in \TT{label} context
are in \itemref{LABEL-VALUES}, and in \TT{text} context are in
