...
ends only at:
\begin{itemize}
\item The next blank line (which can\underline{not} contain comments).
\item The end of the logical line's containing paragraph.
\item The end of the logical line's containing file.
\end{itemize}
Thus the logical line begun at the paragraph header does \underline{not}
end with a physical line indented to the current paragraph indent
or with a line separator `\TT{;}'.

Also the logical line beginning with the paragraph header is the entire
paragraph: there is only this logical line in the paragraph.  This
paragraph is represented by an object whose type is the paragraph header
{\em bracket-type} (e.g., \TT{p}).
\end{indpar}
Multiple Line \hfill \TT{table}
\begin{indpar}
Multiple line paragraph headers group logical lines into a paragraph
which is an object whose \TT{.type} is the paragraph header
{\em bracket-type} (e.g., \TT{table}).
\end{indpar}
\end{indpar}
