...
number values, and a {\em numeric} lexeme that is the same as
a {\em number} lexeme but with commas every three integer digits
also translates to a number value.
A \key{number} value, a.k.a.~a number, is an IEEE 64-bit floating point number.

The syntax of a {\em numeric} that translates to a number values is the
same as the syntax of a {\em number} lexeme except that the
{\em integer-part} is repaced by:
\begin{indpar}[0.1in]
\emkey{integer-part-with-commas} :::= \\
\hspace*{0.1in}\begin{tabular}[t]{l}
    \{ {\em decimal-digit} $|$
             {\em decimal-digit} {\em decimal-digit} $|$
             {\em decimal-digit} {\em decimal-digit} {\em decimal-digit} \} \\
    \{ \TT{,} {\em decimal-digit} {\em decimal-digit}
	            {\em decimal-digit} \}\PLUS{}
    \end{tabular}
\end{indpar}
