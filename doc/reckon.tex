...
a label representable by a {\em name} not beginning with a {\em number}.
...
There are two kinds of attributes: single valued and double valued.

A \key{single valued} attribute takes as values atoms, special values,
and pointers to object, and is named by an attribute label.

A \key{double values} attribute takes as values double headed pointers
and it named by a pair of labels: the \key{forward label} and the
\key{reverse label}.  A double pointer between objects $X$ and $Y$
...
Both kinds of attributes can take multisets of values.

\ikey{Attribute flags}{attribute flag} may be attached to an
attribute label of an object.  The flags of an attribute label
apply to both a single attribute with that label and to all
double attributes with that label as forward label.
...
is output.
Targets of pointers that are values of attributes with
a maybe flagged label may be garbage collected
