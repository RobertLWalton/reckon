...
Indented paragraphs are contained within a logical line.  It is a semantic
(not parsing) error if they are not the last thing in their logical line.
...
logical line (i.e., the paragraph header).
...
the beginning and go to the end.  This quote is a
                                  paragraph by itself.
...
This it the first paragraph of
    the second example.  It begins with an
implied `{p}' because it was preceeded by a `{section}'.

This is the next paragraph (which also begins
with an implied `{p}') which
   contains a quote:
