...
{\em Meridians} cannot be given in {\em time-names},
...
A {\em unit} can be simplified to a form that contains only \TT{*}'s
and \TT{/}'s, and has no \TT{*} after a \TT{/}.  Examples:
\begin{center}
\tt
...
{\em units} and are additively combined.
\begin{indpar}
...
{\em Units} can be combined by the operators \TT{*}, \TT{/},
and \TT{\CIRCUM} with an integral exponent, using algebraic
computation treating units as unknowns.
Examples are:
\begin{center}
\tt
\begin{tabular}{r@{~$\Longrightarrow$~}l}
1 / sec * sec & /sec/sec \\
1 / sec \CIRCUM{} 2 & /sec/sec \\
sec \CIRCUM{} -2 & /sec/sec \\
lb / ft * sec & lb/ft/sec \\
ft \CIRCUM{} 2 * sec\CIRCUM{} -2 & ft*ft/sec/sec
\end{tabular}
\end{center}
