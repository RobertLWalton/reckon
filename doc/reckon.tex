...
and pointers to objects, and is named by an attribute label.

A \key{double valued} attribute takes as values double headed pointers
and is named by a pair of labels: the \key{forward label} and the
\key{reverse label}.  A double headed pointer between objects $X$ and $Y$
...
Attributes with a hidden flagged are not output when their object
is output.
Targets of pointers may be garbage collected
