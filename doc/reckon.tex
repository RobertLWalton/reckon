...
\newcommand{\rcode}[3]{%
	...
	\hline #1: & #2 \\\hline\end{tabular}\\#3}

\begin{indpar}[0.2in] \tt
\rcode{1}{9}{9} \\
\rcode{2}{9 + 8}{17} \\
\rcode{3}{x = 9}{9} \\
\rcode{4}{y = 9 + 8}{17} \\
\rcode{5}{x + y}{26} \\
\rcode{6}{f (B, C, X) = B * X + C}%
         {f (B, C, X) {\rm evaluates to} (B * X + C)} \\
\rcode{7}{f (10, x, y)}{179}
\end{indpar}

This is a trace of a 7 line input file.  The input lines are in
boxes and are preceeded by line numbers, \TT{1:}, \TT{2:}, etc.
Unboxed lines following a boxed input line represent the output that
results when the input line is executed.
...
\newenvironment{renv}%
	{\begin{tabular}{|r|p{5.3in}|}\hline}%
	{\hline\end{tabular}}

\begin{indpar} \tt
\rcode{~1}{g = `hello'}{{\rm hello}} \\
\rcode{~2}{`[g] there'}{{\rm hello there}} \\
\rcode{~3}{z = `{}`I thought he said `[g]'.'{}'}%
         {{\rm I thought he said `hello'.}} \\
\begin{renv}
4: & {p} // Introduces paragraph. \\
5: & This document is meant to be read. \\
6: & Reading this document is good, but... \\
7: & {br}{br} // {br} is a line break that ends a line. \\
8: & [z] \\
9: & // A {p} paragraph ends with a blank line. \\
10: & \\
\end{renv}
...
\rcode{11}{`When you add [x] and [y] you get [x + y].'}%
          {{\rm When you add 9 and 17 you get 26.}}
\end{indpar}
...
\hspace*{2em}\begin{tabular}[t]{rl}
     ...
     $|$ & {\em number-containing-digit} ~ {\em postfix-unit-name-mark} \\
     $|$ & {\em sign}\QMARK{} ~ {\em prefix-unit-name-mark} ~
           {\em unsigned-number-containing-digit} \\
     \end{tabular} \\
\emkey{number-containing-digit} :::= {\em number}
    that is not a {\em numeric-word} \\
\emkey{unsigned-number-containing-digit} :::= {\em number-containing-digit} that
    has no {\em sign}
\end{indpar}
Examples are:
\begin{center}
...
e.g., `\TT{ft}' by itself names the quantity `\TT{1 ft}'.  For
example:
\begin{center}
\tt
\begin{tabular}{r@{~$\Longrightarrow$~}l}
18 ft / sec & 18 ft/sec \\
5 ft 6 in / sec & 66 in/sec \\
\end{tabular}
\end{center}

Units may be defined by {\em unit-definition} statement:
\begin{indpar}
\emkey{unit-definition} ::= \\
\hspace*{2em}\begin{tabular}{l@{~~}l}
\TT{define unit} & {\em unit-fixity}\QMARK{} ~ {\em unit-name}
                   \{ \TT{,}
		      {\em unit-fixity}\QMARK{} ~ {\em unit-name} \}\STAR{} \\
                 & \{ \TT{=} {\em unit-definition-expression} \}\QMARK{} \\
\end{tabular}
\\
\emkey{unit-fixity} ::= \TT{prefix} $|$ \TT{postfix}
\end{indpar}
...
A unit may have multiple names (e.g., \TT{lb} and \TT{lbs}).
A {\em unit-name} that is a {\em mark} may be \TT{prefix};
all other {\em unit-names} are \TT{postfix} (which is the default).
...
define unit lb, lbs = 16 oz         // pound, pounds
define unit sec                     // second
define unit min = 60 sec            // minute
define unit hr = 60 min             // hour
define unit day, days = 24 hr       // day, days
define unit date                    // date
define unit prefix $                // dollars
\end{verbatim}\end{indpar}
...
\emkey{time-zone}
    ...
    $|$ & {\em missing-time-zone-indicator} \\
    $|$ & {\em defined-time-zone-name}
    \end{tabular}
\\[1ex]
\emkey{universal-time-indicator} :::= \ttkey{UTC} $|$ \ttkey{GMT} $|$ \ttkey{Z}
\\[1ex]
\emkey{missing-time-zone-indicator} :::= \ttkey{??T}
\end{indpar}

A time-of-day with a {\em meridian} but \underline{no} time zone
is given the {\em missing-time-zone-indicator} \TT{??T} as its
time zone.
...
To convert a time-of-day\label{CONVERT-TIME-OF-DAY}
from a source time zone to a target time zone,
the diffence between the source time zone and GMT is first subtracted
from the time interval, and then the differnce between GMT and the
target time zone is added.  Thus to convert \TT{EST} to \TT{NZST},
subtract {\tt -5:00} and then add {\tt +12:00}, or equivalently, just
add {\tt +17:00}.
It is possible to get negative numbers, which indicate a time in the previous
day, or numbers equal to or larger than \TT{24:00}, which indicate
a time in the next day.

Conversions involving the {\em missing-time-zone-indicator} are not
allowed.
...
the date with day value \TT{0}).  \key{UTC seconds} (Coordinated
Universal Time seconds) are such that
each day has \TT{86,400} = 24*60*60 UTC seconds.  However, to make
this true, some UTC seconds, called leap seconds,
have to be lengthened or shortened.

Most UTC seconds are the same as \key{TAI seconds} (International
...
\key{leap seconds}, are double length or zero length.  
These exist to match UTC time with time determined by the rotational
position of the Earth, which in turn determines where the sun and stars
appear in the sky.  The earth rotational time day is not quite
...
are sequences of one or more lexemes, while a {\em date-name} and
{\em time-of-day-name} are single lexemes.
...
A time is a quantity with the {\em time-zone} taken from the
time-of-day as its unit and with a numeric value: \\
\centerline{ day number from date * 86,400 - 12::00 + time-of-day} \\[0.5ex]
Here -12:00 occurs because the time is the number of UTC seconds
from \underline{noon} on the day with day number \TT{0}.

Conversion of times between time zones follows the same rules
as conversion of times-of-day: see \pagref{CONVERT-TIME-OF-DAY}.

The \TT{TAI} function converts a time from UTC seconds to TAI seconds
by adjusting for leap seconds.  If you want the number of TAI seconds between
times {\tt t1} and {\tt t2}, use {\tt TAI(t2) - TAI(t1)}.

Here
{\tt TAI(t) = t + 10 +} adjustment for leap seconds at or before t.

For double length leap seconds, the adjustment is \TT{+1}.  For zero length
leap seconds, the adjustment is \TT{-1}.  Here {\tt + 10} appears because
when the leap second system was created in 1972, TAI was already
10 seconds ahead of UTC.


Leap seconds are always the last second of a month in GMT time,
where so far the only months used are June and December.
The last second of the month is always computed using GMT time:
for example, a leap second at {\tt 23:59GMT} will be at
{\tt 18:59EST}.

GPS satellites use GPS time defined by GPS = TAI - 19.
...
\ttkey{XM} & meridian: \TT{AM} or \TT{PM} \\
\ttkey{hh} & hour: \TT{01} \ldots{} \TT{12}
...
\begin{tabular}{l}
\emkey{raw-vector-element}\label{RAW-ELEMENT}
    \begin{tabular}[t]{rl}
    ::= & {\em word} $|$ {\em numeric} $|$ {\em number} $|$
          {\em natural} $|$ {\em numeric-word} \\
    $|$ & {\em mark} $|$ {\em separator} $|$ {\em quoted-string} \\
    $|$ & {\em bracketed-expression} \\
    \end{tabular} \\
{\em bracketed-expression} ::= see \itemref{BRACKETED-EXPRESSIONS}
\end{tabular}
...
\begin{tabular}{l}
\emkey{single-attribute-representation}%
...
\\\hspace*{0.5in}
    ...
    ::= & {\em attribute-label}
	  ...
	  \end{tabular} \\
    $|$ & {\em attribute-label} \\
    $|$ & \TT{no} {\em attribute-label} \\
    \end{tabular}
\end{tabular}

\begin{tabular}{l}
\emkey{attribute-label}\label{ATTRIBUTE-LABEL}
    ::= {\em name} not beginning with a {\em number} \\
{\em name} ::= see \pagref{NAME} \\
\end{tabular}

\begin{tabular}{l}
\emkey{attribute-label-flags} ::= \TT{[} {\em flag-lexeme} \TT{]} \\
\emkey{flag-lexeme}
    ::= {\em word} or {\em mark} containing just letters and/or
        \TT{*+-/@\&\#=\$\%<>}
\end{tabular}

\begin{tabular}{l}
\emkey{single-attribute-value}\label{SINGLE-ATTRIBUTE-VALUE}
    ::= {\em name} $|$ {\em object-identifier}
                   $|$ {\em bracketed-expression} \\
...
\begin{tabular}{l}
\emkey{double-attribute-value}\label{DOUBLE-ATTRIBUTE-VALUE}
...
\end{tabular}
&
\begin{picture}(3.1,2.0)
\put(0.0,0.2){
    \put(0.3,1.40){\makebox(0.8,0.3){\tt @<Y>}}
    \put(0.3,1.25){\makebox(0.8,0.3){\tt man Jack}}
    \put(0.7,1.45){\oval(1.2,0.5)}
    \put(1.9,1.40){\makebox(0.8,0.3){\tt @<X>}}
    \put(1.9,1.25){\makebox(0.8,0.3){\tt woman Jill}}
    \put(2.3,1.45){\oval(1.2,0.5)}
    \put(0.65,1.20){\vector(0,-1){0.6}}
    \put(0.60,1.00){\makebox(0,0)[r]{\tt child}}
    \put(0.75,0.60){\vector(0,1){0.6}}
    \put(0.80,0.75){\makebox(0,0)[l]{\tt parent}}
    \put(2.25,1.20){\vector(0,-1){0.6}}
    \put(2.20,1.00){\makebox(0,0)[r]{\tt child}}
    \put(2.35,0.60){\vector(0,1){0.6}}
    \put(2.40,0.85){\makebox(0,0)[l]{\tt parent}}
    \put(0.3,0.30){\makebox(2.4,0.3){\tt @<Z>}}
    \put(0.3,0.15){\makebox(2.4,0.3){\tt boy Giddy}}
    \put(1.5,0.35){\oval(2.4,0.5)}
    \put(0,0.0){\framebox(3.1,1.9){}}
}
\end{picture}
\end{tabular}
\end{center}

We say that the value of the \TT{parent} attribute of \TT{@<Z>} is
the set to two elements, \TT{@<X>} and \TT{@<Y>}.  Two lines can be
used to specify the two values, or a single line with the values
separated by commas inside \TT{\{*~*\}} brackets may specify the
two values.

A double headed pointer can be represented in two ways, one way for
each end of the pointer.  Either way can be used, or both.  As
double attribute values are sets, using both ways results in only
one attribute value, and not two identical values.  For example:
\begin{center}
\tt
\begin{tabular}{l}
@<X> :=: \\
~~~~a = @<Y> b \\
@<Y> :=: \\
~~~~b = @<X> a~~~~// This is redundant and does nothing. \\
\end{tabular}
\end{center}
...
The interpretation of brackets within contexts is:
\begin{center}
\begin{tabular}%
  {l@{\hspace*{0.2in}}l@{\hspace*{0.2in}}l@{\hspace*{0.2in}}l}
\bf Brackets & \bf Context & \bf New Context & \bf Usage \\[1ex]
\TT{[<\ldots>]} & all & \TT{label} & {\em label-value} \\
\TT{[\$\ldots\$]} & all & \TT{data} & {\em special-value} \\
\TT{(\ldots)} & all & unchanged & {\em parenthesized-expression} \\
\TT{\{|\ldots|\}} & all & unchanged & {\em purelist} \\
\TT{\{\{\ldots\}\}} & \TT{code}, \TT{text} & \TT{math} & {\em math-value} \\
\TT{\{!\ldots!\}} & \TT{code} & \TT{code} & {\em code-value} \\
\TT{`\ldots'} & \TT{code}, \TT{text} & \TT{text} & {\em phrase-value} \\
\TT{``\ldots''} & \TT{code}, \TT{text} & \TT{text} & {\em sentence-value} \\
\TT{[\ldots]} & \TT{code} & \TT{code} & {\em subscript } \\
\TT{[\ldots]} & \TT{label}, \TT{math}, \TT{text} & \TT{code} & {\em insertion } \\
\TT{\{*\ldots*\}} & \TT{data} & \TT{data} & {\em multi-value } \\
all \begin{tabular}[t]{@{}l@{}}
    but \TT{[<\ldots>]} \\
    and \TT{[\$\ldots\$]} \\
    \end{tabular} & \TT{data} & \TT{data} & {\em object-representation } \\
\end{tabular}
\end{center}

\key{Insertions} contain code that is executed to produce
a value that is inserted in a {\em label-value}, {\em math-value},
or {\em text}.  Their syntax is:

\begin{indpar}
\emkey{insertion} ::= \TT{[} {\em code-expression} \TT{]} \\
{\em code-expression} ::= {\em expression} parsed in the \TT{code} context
    ~~~~ [see \pagref{CODE-EXPRESSION}]
\end{indpar}
...
\begin{center}
\tt
\begin{tabular}{p{2.7in}@{\hspace*{0.3in}}p{3.2in}}
\begin{tabular}[t]{l}
!@53 := \{| X Y Z |\} \\[1ex]
...
Note that in these example \TT{\{|~|\}} purelists have no attributes,
\TT{`{}`} and \TT{'{}'} are single lexemes,
while \TT{\{\{} and \TT{\}\}} are each two-lexeme sequences.
...
\tt
\begin{tabular}{l}
@<Z> := \{\{ integral (x \textasciicircum{} 2, dx) \}\}
\end{tabular}
\end{indpar}
{\rm has the same effect as}
\begin{indpar}
...
contexts.  \TT{`{}`~'{}'} bracketed subexpressions may appear in the
\TT{data} context, but sentences within the brackets will not
be recognized and insertions within the brackets will not be processed.
...
A \key{prefix} is a {\em typed-list}\pagnote{TYPED-LIST}
without any `\TT{|}'s or {\em vector-element-list}:
...
\\
{\em type-mark} ::= see \pagref{TYPE-MARK}
\\
{\em attribute-list} ::= see \pagref{ATTRIBUTE-LIST}

\end{indpar}

However, not every {\em typed-list} of this form is a prefix.
{\em Typed-lists} of this form which are \underline{not}
prefixes just represent objects with \underline{no} vector elements.
...
\{row\}| Name | Height | W | \\
\{row\}-------------------- \\
\{row\}| Jack | 10 ft 3 in | 307 lb | \\
\{row\}| Jill | 11 ft 8 in | 248 lb | \\
\{row\}------------------------- \\
W: Weight \\
\end{tabular}
\end{indpar}
...
\tt
\begin{tabular}{l}
\{table| \begin{tabular}[t]{@{}l}
         \{row|~"--------------------" |row\} \\
         \{row|~"|" Name "|" Height "|" W "|" |row\} \\
         \{row|~"--------------------"|row\} \\
         \{row|~"|" Jack "|" 10 ft 3 in "|" 307 lb "|" |row\} \\
         \{row|~"|" Jill "|" 11 ft 8 in "|" 248 lb "|" |row\} \\
         \{row|~"--------------------"|row\} \\
	 \{"":~.terminator = "<LF>", \\
	 ~~~~~.initiator = [\$ LOGICAL\_LINE \$] \\
	 ~~~~~| W ":" Weight |\} \\
	 |table\} \\
	 \end{tabular}
\end{tabular}
...
Typically the \TT{\{table\}} and \TT{\{row\}} prefixes are
implied: see \itemref{TABLES} for table details.
...
\emkey{primary}\label{PRIMARY} ::=
    {\em primary-element}\PLUS{} ~~~~~ [see \pagref{PRIMARIES}]
\\[0.5ex]
...
\hline
\ttkey{<-{}-} & declare & nofix & declare & 0000
\\\cline{1-4}
\ttkey{if} & conditional & prefix & conditional &
\\\cline{1-1}
\ttkey{else if} & & & &
\\\cline{1-4}
...
\ttkey{times} & iterator & afix   & (none) & \\
               & modifier &        &       &
\\\cline{1-4}
\ttkey{for every} & member & prefix   & unary & \\
		  & declaration & & &
\\\hline
\end{tabular}
\end{center}
...
\begin{boxedfigure}[!p]
\begin{center}
Non-Line Level Code and Math Operators \\
May Occur Outside or Inside Parentheses and Brackets
\\[1ex]
\begin{tabular}{|l|l|l|l|r|}
\hline
Operator & Meaning & Fixity & Format & Precedence \\
...
The following are some examples of parsed expressions:
...
Note that operands are objects that
may or may not have brackets, or equivalently, that
have \TT{\{|~|\}} `implied brackets' if there are no
explicit brackets.  Note that operands that do not
themselves contain operators are {\em primaries}\pagnote{PRIMARY}.
...
Before a primary is parsed, lexeme-splitting
is performed on the parts of the primary outside
brackets.


\subsection{Lexeme Splitting}
\label{LEXEME-SPLITING}

The following lexemes are split if they occur outside brackets
in a primary:
\begin{indpar}
\emkey{splittable-lexeme} :::= {\em word} ~ \TT{'s}
\end{indpar}

Note the use of :::= instead of ::=.  Splitting just inserts
whitespace between the two parts of the
{\em splittable-lexeme} thereby spliting it into two lexemes.

\subsection{Reference Expressions}
\label{REFERENCE-EXPRESSIONS}

A \emkey{reference-expression} names a variable, which may be
a variable allocated by a statement and named by a simple
{\em variable-name}, or may be variable that is an attribute
of another variable which is also named by the {\em reference-expression}.

For example, a statement may create a variable who {\em variable-name}
is `\TT{the person}' and whose value is an object with an attribute
named `\TT{weight}', and this attribute of this particular object
is named by either of the {\em reference-expressions}
`\TT{the person 's weight}' or `\TT{weight of the person}'.
The first of these can be contracted to `\TT{the person's weight}'
as the single lexeme `\TT{person's}' will be split into the
two lexemes `\TT{person 's}': see \pagref{POSSESSIVE-SPLITING}.

In this case the attribute can also be named by
`\TT{the person[{\rm \em subscript}]}'
where {\em subscript} is a {\em code-expression}
that evaluates to \TT{"weight"}.
...
\emkey{reference-expression}\label{REFERENCE-EXPRESSION}
    ::= \begin{tabular}[t]{@{}l@{}}
        \{ {\em attribute-label} ~ {\em subscript-expression}\STAR{} ~ \TT{of}
           \}\STAR{} \\
        {\em reference-primary} \\
	\{ \TT{'s} ~ {\em attribute-label} ~ {\em subscript-expression}\STAR{}
        \}\STAR{}
	\end{tabular}
\\[0.5ex]
\emkey{reference-primary} ::= {\em name} ~ {\em subscript-expression}\STAR{}
\\[0.5ex]
{\em name} ::= see \itemref{NAMES}
\\[0.5ex]
\emkey{subscript-expression} ::=
    \TT{[} {\em subscript} ~ \{ \TT{,} {\em subscript} \}\STAR{}
    \TT{]}
\\[0.5ex]
\emkey{subscript} ::= {\em code-expression}
\\[0.5ex]
{\em code-expression} ::= see \pagref{CODE-EXPRESSION}


\end{indpar}

In a {\em reference-expression} the {\em reference-primary}
is evaluated first, then the `\TT{'s \ldots{}}' clauses are
evaluated left to right, and then the `\TT{\ldots{} of}' clauses
are evaluated right to left.  Thus all the following are equivalent:
\begin{center}
\tt
the man's wife's weight \\
the weight of the man's wife \\
the weight of the wife of the man \\
the man["wife"]'s weight \\
the man's wife["weight"] \\
wife["weight"] of the man \\
weight of the man["wife"] \\
the man["wife"]["weight"]
\end{center}
...
\emkey{call-term-name}\label{CALL-TERM-NAME} ::=
    \begin{tabular}[t]{@{}l}
    {\em name} \underline{not} beginning with a {\em number} \\
    and with quotes \underline{optionally} removed from \\
...
\item
\TT{[~]} bracketed {\em call-argument-lists} cannot be placed
before a {\em call-term-name}.
\end{itemize}
\end{indpar}

Thus a {\em function-call} is a sequence of {\em call-term-names}
...
\ttkey{sentence} \hfill \OP{s} ~~~ \ttkey{.} ~~~ \ttkey{?} ~~~ \ttkey{!}
