...
\emkey{typed-list}\label{TYPED-LIST}
	\begin{tabular}[t]{@{}rl@{}}
        ::= & \TT{\{} {\em type-name}\QMARK{}
	      ...
	      {\em prefix-0-list} \TT{|} {\em type-name}\QMARK{} \TT{\}} \\
	$|$ & \TT{\{} {\em type-mark} ~ {\em prefix-0-list}
	                              ~ {\em type-mark} \TT{\}}
	\end{tabular}
\\[0.5ex]
...
{\em attribute-representation} ::= see \pagref{ATTRIBUTE-REPRESENTATION}
\\[0.5ex]
{\em prefix-0-list} ::= see \pagref{PREFIX-0-LIST}
		        ~~~ [can be {\em vector-element-list}] \\
...
\item The {\em prefix-0-list} (which can be a {\em vector-element-list})
...
user of Reckon.
/bin/bash: date4992: command not found
...
{\bf s} \hfill `Sentence' type; recognized in the \TT{text} context.
\begin{indpar}[0.2in]
If the {\em prefix-0-list} is in fact a {\em vector-element-list}
ending in a \key{sentence terminator},
...
{\bf p}
\\[0.5ex]
{\bf quote} \hfill  Same properties as `\TT{p}'.
\\[0.5ex]
{\bf section} \hfill  Same properties as `\TT{p}'.
\\[0.5ex]
{\bf table}
\\[0.5ex]
{\bf row}
