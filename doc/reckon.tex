...
\emkey{word}\label{WORD} ::=
    {\em middle-lexeme} with a {\em letter} before any {\em digit}
\\[1ex]
\emkey{mark}\label{MARK} ::=
    {\em middle-lexeme} not containing a {\em letter} or {\em digit}
\\[1ex]
...
It is possible to glue several lexemes together by
using the lexeme break indicator \ttkey{\#}.  For example,
\begin{center}
...
Quoted strings can also be split using \TT{\#} as if it were an operator.
For example,

\begin{center}
{\tt "Hi: \# " " \# "There"} ~~~~~ is equivalent to ~~~~~ {\tt "Hi There"}
\end{center}
...
The following are the permitted date formats.  All begin with the
year:

\begin{indpar}
\emkey{date} \begin{tabular}[t]{rl}
	     :::= & {\em long-year} \TT{-} {\em month} \TT{-} {\em day}
	            \{ \TT{-} {\em weekday} \}\QMARK{} \\
	      $|$ & {\em year} {\em era} \TT{-} {\em month} \TT{-} {\em day}
	            \{ \TT{-} {\em weekday} \}\QMARK{} \\
	      $|$ & {\em long-year} \TT{-}\QMARK{} {\em month-name}
	            \TT{-}\QMARK{} {\em day}
	            \{ \TT{-} {\em weekday} \}\QMARK{} \\
	      $|$ & {\em year} {\em era} \TT{-} {\em month-name}
	            \TT{-}\QMARK{} {\em day}
	            \{ \TT{-} {\em weekday} \}\QMARK{} \\
	      $|$ & {\em long-year} \TT{-}\QMARK{} {\em month-name} \\
	      $|$ & {\em year} {\em era} \TT{-} {\em month-name} \\
	      $|$ & {\em year} {\em era} \TT{-} {\em day-of-year} \\
	      $|$ & {\em year} {\em era} \\
	     \end{tabular}
\label{DATE}
\\[1ex]
\emkey{long-year} :::=  {\em digit} {\em digit} {\em digit} {\em digit}
\\[1ex]
\emkey{year} :::= {\em digit}\PLUS{}
\\[1ex]
\emkey{month} :::=  {\em digit} {\em digit}
\\[1ex]
\emkey{day} :::=  {\em digit} {\em digit}
\\[1ex]
\emkey{day-of-year} :::=  {\em digit} {\em digit} {\em digit}
\\[1ex]
\emkey{weekday}
    \begin{tabular}[t]{rl}
    :::= & \TT{Mon} $|$ \TT{Monday} \\
...
Each indented paragraph is a \key{paragraph indent}.
...
begins a new \key{logical line}.  The current logical line ends with
...
a new logical line is begun.  However, empty logical lines are ignored
as if they did not exist.

A logical line can consist of several physical lines, the first and last
of which are non-blank.  All but the first have indents greater than
the first, and are called \skey{continuation line}s.  Blank lines and
comment lexemes within a logical line are ignored, as if they did not
exist.

A logical line beginning with a comment lexeme is only allowed to contain
comment lexemes.  Such a line is called a \key{comment line}.  If a
logical line that is not a comment line begins with a comment, a
warning message is issued.  Aside from rules regarding indentation,
comment lines are ignored as if they did not exist.
...
the current paragraph indent ends the indented paragraph, and this line
...
ends at the next blank line, or the end of its
containing indented paragraph, or the end of file.

TBD


A \key{paragraph header} must appear at the beginning of a logical line
that is just after a blank line, or the beginning of the file, or the
beginning of the containing indented paragraph.
A \key{headed paragraph} is a paragraph header followed by a single
logical line that consists of all physical
lines up to the next non-blank line, or the end of the file, or the end
of the containing indented paragraph.  In this case continuation
lines need not be indented with respect to the beginning of the
logical line (i.e., the paragraph header), but if they are in an
indented paragraph, they should have at least the paragraph indent.

A \key{text paragraph} is a headed paragraph with one of the following
types:
\begin{center}
...
The part of a file outside headed paragraphs contains code in
logical lines.

With one exception, headed text paragraphs do not have to
obey any indentation rules.  The exception is that if the
the headed paragraph is inside an indented paragraph, all
lines of the headed paragraph
must have an indent at least as
great as the indented paragraph's paragraph indent.

The indent of an indented paragraph is the indent of the
first non-blank line after the physical line ending `\TT{:}'
or `\TT{::}' that introduces the indented paragraph.  This
first non-blank line can be a comment line.
...
explicit paragraph header.  For \TT{\{section\}} the implied
paragraph header is \TT{\{p\}}.  For the others, the implied
paragraph header is a copy of the explicit paragraph header:
e.g., for `\TT{\{quote: indent = 5\}}' the implied header will
be `\TT{\{quote: indent = 5\}}'.  To end text, it is necessary
to use `\TT{\{code\}}' as if it were a paragraph header, but
this just deactivates implied headers
...
\end{tabular}
\end{center}
Such are called \slkey{implied paragraph}{header}s.
