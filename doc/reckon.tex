...
a board of size S <-- S is an integer, S >= 5:
    ...
    // length(S) is length of ship with ship ID S.
    //
    length = an array of size (0)

    // hits(S) is the number of hits on the ship with
    // ship ID S.
    //
    hits an array of size (0)
...
for every board B:
    ...
    //
    on test position (DIRECTION, START) on B
    ...
    //
    on find position on B for ship of length L
            <-- L is an integer, 1 <= L <= max ship length:
        value = loop:
            first tries = 1
            direction =
    ...
    //
    on place ship S of length L at (DIRECTION, START) on B
            <-- S is a ship,
                DIRECTION is a pair of integers,
                START is a pair of integers,
                L is an integer, 1 <= L <= max ship length:
