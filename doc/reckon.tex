...
A \key{bracketed expression} represents a label or object and
...
all but \TT{[<\ldots>]} & \TT{data} & \TT{data} & {\em object-representation } \\
\end{tabular}
\end{center}

\ikey{Insertions}{insertion} contain code that is executed to produce
a value that is inserted in a {\em label-value}, {\tt math-value},
or {\tt text}.  Their syntax is:
...
Objects (e.g., those represented by {\em parenthesized-expressions})
and special values (e.g., \TT{[\$ MISSING \$]}) \underline{cannot}
