...
\\[1ex]
\emlkey{isolated-separating-}{character} :::=
    \begin{tabular}[t]{l}
...
\\
\emlkey{trailing-separator-}{character} :::=
	\TT{'} $|$ \TT{!} $|$ \TT{?} $|$ \TT{.} $|$ \TT{:}
	       $|$ \TT{,} $|$ \TT{;}
\\[1ex]
...
and are illegal:
\begin{indpar}
\emlkey{misplaced-horizontal-}{space-character} :::=
    \begin{tabular}[t]{l}
    {\em horizontal-space-character}, other than \\
    ASCII single space
    \end{tabular}
\\[0.3ex]
\emlkey{misplaced-vertical-}{space-character}
		:::= {\em vertical-space-character}
...
A \emlkey{special-character-}{representative} represents on UNICODE character
...
\skey{logical line}s,\label{LOGICAL-LINE} each of which is
...
this top level pseudo-paragraph are called \skey{top level logical lines}.
...
An object can also contain a set of \skey{attribute}s, each with one or two
attribute labels and one or more attribute values which can be atoms,
special values, or pointers to objects.

The elements of a vector are labelled by \ikey{indeces}{index!of vector}
\TT{0}, \TT{1}, \TT{2}, \ldots{}.

An attribute is labeled to one or two \skey{attribute label}s, which are
...
\ikey{Attribute flags}{attribute flag} may be attached to an
...
garbage collector (\secref{GARBAGE-COLLECTION}).
\end{tabular}
\end{center}
...
\emkey{symbolic-object-identifier} :::=
	\ttmkey{@}{in {\em object-identifier}}\TT{<}
...
The interpretation of brackets depends upon the context in whish
...
\ikey{Insertions}{insertion} contain code that is executed to produce
...
The following is a complete list of all the \skey{special value}s:
...
\item If a {\em typed-list} has a {\em type-name} at both its beginning
and end, these must be the same.  This {\em type-name} then becomes the
value of the
\ttdkey{type} attribute of the object, as is also the case if there
is only one {\em type-name} at the beginning.  Thus in \TT{\{b|\ldots|\}},
\TT{\{b|\ldots|b\}}, and \TT{\{b\}}, \TT{b} becomes the value of the \TT{.type}
attribute.
But if the {\em type-name}
is completely missing (as in \TT{\{|\ldots|\}} or \TT{\{\}}), or
...
of the object's \TT{.type} attribute.  Similarly for
\TT{\{}{\em type-mark}\TT{\}}.  If different {\em type-marks}
begin and end the {\em typed-list}, the squence of the two
{\em type-marks}, as a label, becomes the value of the \TT{.type}
attribute.  Thus in \TT{\{+\ldots+\}}, \TT{"+"} is the \TT{.type}
attribute value, whereas is \TT{\{<\ldots>\}}, the label \TT{[< "<" ">" >]}
is the \TT{.type} attribute value.
...
\item A {\em prefixes} is a {\em typed-list} that has
no `\TT{|}'s and is recognized in certain contexts.\pagnote{PREFIXES}
Prefixes are not recognized in {\em typed-lists}, in particular, in the
{\em attributes} and the {\em vector-element-list} of a {\em typed-list}.
Instead they simply represent objects
...
Some {\em type-names} are \mkey{special}{type name} but only in
particular contexts:
...
\skey{sentence}s each ending with a sentence terminator.
...
{\bf p} \hfill `Paragraph' type. \hfill Recognized in \TT{code}
                                    and \TT{text} contexts.
\begin{indpar}[0.2in]
The `\TT{p}' type should only be used in prefixes: see \pagref{P-PREFIX}.
\end{indpar}

\bigskip

{\bf quote} \hfill `Quote' type. \hfill Recognized in \TT{code}
                                    and \TT{text} contexts.
\begin{indpar}[0.2in]
The `\TT{quote}' type should only be used in prefixes:
see \pagref{QUOTE-PREFIX}.
\end{indpar}

\bigskip

{\bf section} \hfill `Section' type. \hfill Recognized in \TT{code}
                                    and \TT{text} contexts.
\begin{indpar}[0.2in]
The `\TT{section}' type should only be used in prefixes:
see \pagref{SECTION-PREFIX}.
\end{indpar}

\bigskip

{\bf table} \hfill `Table' type. \hfill Recognized in \TT{code}
                                    and \TT{text} contexts.
\begin{indpar}[0.2in]
An object with `\TT{table}' type represents a table with columns
and rows.  The {\em vector-element-list} of the object is a sequence
of `\TT{row}' objects and logical lines.  See \itemref{TABLES} for details.
\end{indpar}

\bigskip

{\bf row} \hfill `Row' type. \hfill Recognized in
                                    \TT{text} context.
\begin{indpar}[0.2in]
An object with `\TT{row}' type represents a row in a table.
See \itemref{TABLES} for details.
\end{indpar}

\bigskip

{\bf code} \hfill `Code' reset. \hfill Recognized in \TT{code}
                                    and \TT{text} contexts.
\begin{indpar}[0.2in]
The `\TT{code}' type is only be used as a prefix to end a sequence
of paragraphs: see \pagref{CODE-PREFIX}.
\end{indpar}
...
\mkey{logical-line}{prefix-0-list} \hfill \TT{code} or \TT{text} context
...
\ttmkey{`{}`\ldots'{}'}{prefix-0-list} \hfill \TT{code} or \TT{text} context
...
\ttmkey{(\ldots)}{prefix-0-list} \hfill inside a {\em prefix-n-list}
...
\ttkey{p}	& \itemref{PARAGRAPHS} & paragraph
	& \TT{code} or \TT{text},
\\\cline{1-1}\cline{3-3}
\ttkey{quote}	& & quoted paragraph & at beginning of logical line
\\\cline{1-1}\cline{3-3}
\ttkey{section}	& & section header &
\\\cline{1-3}
